...
def loadphecodes():
    phecodes9 = pd.read_csv(pkg_resources.resource_filename(__name__,'phecode_icd9_rolled.csv'),dtype=str).iloc[:,0:4]
    phecodes10 = pd.read_csv(pkg_resources.resource_filename(__name__,'Phecode_map_v1_2_icd10cm_beta.csv'),dtype=str).iloc[:,0:4]
...
def icdtophecode(df,col_icd):
    phecodes = loadphecodes()
    df = df.copy()
    df[col_icd] = df[col_icd].str.replace(' ','').str.replace('.','')
    df.merge(phecode,how='left',left_on=col_icd,right_on='ICD Code')
    return df
