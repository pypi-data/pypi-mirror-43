...
time_pt = r"\d{1,2}(时|:)?\d{1,2}(分)?"

date_pt = r"\d{4}(年|-|/|\.)\d{1,2}(月|-|/|\.)\d{1,2}(日|\s)"
...
datetime_extract_pt = r"(?P<date>(?P<year>\d{4})(年|-|/|\.)(?P<month>\d{1,2})(月|-|/|\.)(?P<day>\d{1,2})(日|\s)?)(?P<time>(?P<hour>\d{1,2})(时|:)?(?P<minute>\d{1,2})(分)?)?"

datetime_board_extract_pt = r"((?P<date>(?P<year>\d{4})(年|-|/|\.))?(?P<month>\d{1,2})(月|-|/|\.)(?P<day>\d{1,2})(日|\s)?)(?P<time>(?P<hour>\d{1,2})(时|:)?(?P<minute>\d{1,2})(分)?)?"


mdHM_pt = r"\d{1,2}(月|-|/|\.)\d{1,2}(日|\s)?(\d{1,2}(时|:)?\d{1,2}(分)?)?"

buy_date_pt = r"(?P<start_date>%s)[^至]{0,20}?至(?P<end_date>%s)" % (
    datetime_pt,
    datetime_pt,
)
...
buy_date_pt_2 = r"(?P<start_date>%s)[^至]{0,20}?至(?P<end_date>%s)" % (
    datetime_pt,
    mdHM_pt,
)
...
buy_date_pt_3 = r"(售卖|购买|发售|文件获取|文件的获取)\D{0,30}(?P<date>%s)" % datetime_pt

# 购买截止时间

limit_date_pt = r"(截止|截标)\D{0,40}(?P<date>%s)" % datetime_pt

open_date_pt = r"开标\D{0,40}(?P<date>%s)" % datetime_pt

publish_date_pt = r"(发布时间|发布日期|公告时间|公告日期)\D{0,20}(?P<date>%s)" % datetime_pt
...
def parse_date(d, default=None):
    ...
    try:
        ...
        print(e)
        print("dateparser.parse %s error" % d)
    ...
    for m in re.finditer(buy_date_pt_2, text):
        ...
        if em and em.group("year") is None:
            try:
                ...
                print(e)
                print("board_extract buy_end_date fail: %s" % m.group("end_date"))
