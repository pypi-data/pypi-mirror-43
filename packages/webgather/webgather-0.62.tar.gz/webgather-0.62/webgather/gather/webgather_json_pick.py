...
from webgather.gather.webgather_base import webgather_base, Item
    ...
    def picker_content(self, **kwargs):
        ...
        for item in test(dic, 0, selecters):
            ...
            if kwargs.get("merge", ""):
                urlitem.href = (
                    kwargs["merge"]
                    .replace(r"(url)", str(item[selecters[-1]]))
                    .strip('"')
                )
            elif kwargs.get("merge_detail_page_format", ""):
                urlitem.href = kwargs["merge_detail_page_format"] % str(
                    item[selecters[-1]]
                )
            elif kwargs.get("detailpageformat", ""):
                # urlitem.href = kwargs['detailpageformat'] % str(item[selecters[-1]].encode('utf-8'))
                urlitem.href = kwargs["detailpageformat"].format(
                    str(item[selecters[-1]].encode("utf-8"))
                )
            else:
