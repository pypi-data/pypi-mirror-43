...
def LoadSharedLibrary( dllname = None ):
	...
	if dllname is None:
		...
		if platform.architecture()[ 0 ].startswith( '32' ): arch = '32bit'  # NB: we override platform.machine() here because for example, on 32-bit Python for 64-bit Windows, platform.machine() can return 'AMD64'
		if platform.architecture()[ 0 ].startswith( '64' ): arch = '64bit'
		if   arch == 'i386':   arch = '32bit'
		elif arch == 'x86_64': arch = '64bit'
		elif arch == 'AMD64':  arch = '64bit'
