...
that employs a bit-stealing technique (after Tyler 1997), or
for creating LUTs that can be used with specific dynamic-range-
enhancing hardware such as the Bits++ or Viewpixx
(`BitCombiningLUT`).
...
def BitCombiningLUT( nbits=16, channelOrder='RG', gamma='sRGB', cache_dir=None, DACbits=8 ):
	"""
	...
	range by using more than one DAC to specify the (monochromatic) intensity
	of each pixel.  The resulting DAC combinations must be decoded by
	custom hardware (Bits++/Bits#, DataPixx, etc).
	
	Args:
	    nbits (int):
	        The look-up table will have `2 ** nbits` entries. The minimum
	        value will be 0 and the maximum will be `2 ** nbits - 1`, but
	        the intervening values will the appropriate gamma-correction
	        curve according to the specified `gamma` value.

	    channelOrder (str):
	        Specifies which DACs are used, in what order of significance
	        (most-significant first) to represent each pixel value.
	...
	"""
vec4 convertToBitsMonoMode(vec4 fboFrag)
{
    index = fboFrag.r * 65535.0 + 0.01;
    fboFrag.r = floor( index / 256.0 ) / 255.0;
    fboFrag.g = mod( index, 256.0 ) / 255.0;
    fboFrag.b = 0.0;
	...
	if cache_dir:
		...
		filename = 'BitConcatenationLUT_channelOrder=%s_nbits=%s_gamma=%s' % ( channelOrder, nbits, gamma )
	...
	maxVal = 2 ** nbits - 1
	a = numpy.linspace( 0.0, 1.0, 2 ** nbits, endpoint=1 )[ :, None ]
	a = Linearize( a, gamma=gamma )
	a = numpy.round( a * maxVal ).astype( 'uint32' )
	
	dacs = []
	coefft = 2 ** ( DACbits * 2 )
	dacs.insert( 0, a // coefft )
	a %= coefft
	coefft = 2 ** ( DACbits * 1 )
	dacs.insert( 0, a // coefft )
	a %= coefft
	...
	dtype = 'uint8' if DACbits <= 8 else 'uint16' if DACbits <= 16 else 'uint32'
	lut = numpy.zeros( [ a.size, 1, 3 ], dtype=dtype )
	for srcIndex, ch in enumerate( channelOrder[ ::-1 ] ):
		dstIndex = 'RGB'.index( ch.upper() )
		lut[ :, dstIndex ].flat = dacs[ srcIndex ]	
