...
def chi_mapping_tools():
	...
	if default_param["map"]:
		...
		ax.scatter(mydem.df_base_river["x"],mydem.df_base_river["y"], s = normalize(mydem.df_base_river["drainage_area"].values), c = mydem.df_base_river["chi"], vmin = normalize(mydem.df_base_river["chi"].quantile(0.10)), vmax = normalize(mydem.df_base_river["chi"].quantile(0.90)), zorder = 5)
