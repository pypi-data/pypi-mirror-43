...
class SeleniumDeviceFactory:
    ...
    def build(self, key, profile_dir=None):
        sys.stderr.write("BUILD DEVICE STARTED")
        sys.exit(-1)
        ...
        if definition['platform'] == 'selenium':
            ...
            if definition['kind'] == 'driver':
                if definition['device'] == 'firefox':
                    ...
                    sys.exit(-1)
                    print("definition: %s" % str(definition))
                    if 'options' in definition.keys():
                        if 'headless' in definition['options']:
                            print("HEADLESS SET")
                            options.headless = True
                    return FirefoxDriver(firefox_profile=profile, firefox_binary=binary, firefox_options=options)
