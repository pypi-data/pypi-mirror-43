...
#  -*- Mode: Python; -*-                                              
# 
#  internal.py     Low level interface to librosie from Python 2.7 and 3.6
# 
...
def _new_cstr(bstring=None):
    ...
    else: #if isinstance(bstring, bytes):
        obj = _lib.rosie_string_ptr_from(bstring, len(bstring))
        return ffi.gc(obj, _lib.free)
#    else:
#        raise ValueError("Unsupported argument type: " + str(type(bstring)))
...
class engine (object):
    ...
    # not currently implented in rosie.py
