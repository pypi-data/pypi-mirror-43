...
class ActiveDirectoryConfig(AzureBaseConfig):
    targets = (
        ('permissions', 'Directory', 'list', {}, False),
    )
    ...
    def __init__(self, thread_config):
        ...
        super(ActiveDirectoryConfig, self).__init__(thread_config)

    def parse_directories(self, directory, params):
        active_directory_dict = {}
        #active_directory_dict['id'] = self.get_non_provider_id(directory.id)
        #active_directory_dict['name'] = directory.name
        #active_directory_dict['ad_admin_configured'] = self._is_ad_admin_configured(directory)
        #active_directory_dict['databases'] = self._parse_databases(directory)

        self.directories[active_directory_dict['id']] = active_directory_dict
