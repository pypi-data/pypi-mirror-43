...
class Jwk(Jose):
    ...
    def get_instance(
        ...
        :param enc: Specific encryption indicator (default: None).
        ...
        """
        if enc is None and crv is None:
            if from_dict:
                enc = from_dict.get('enc')
        ...
        klass = _CLASS_MAPPING.get((enc, crv))
        if klass:
            if (from_secret
                    and enc in [ChaCha20Poly1305Jwk._DEFAULT_HEADER['enc'],
                                AES256GCMJwk._DEFAULT_HEADER['enc']]):
                ...
                return klass(from_json=from_json, from_dict=from_dict,
                             generate=generate)
        raise RuntimeError(
            'No suitable JWK type implemented for enc={} and crv={}.'
            .format(enc, crv))
...
_CLASS_MAPPING = {
    (ChaCha20Poly1305Jwk._DEFAULT_HEADER['enc'], None): ChaCha20Poly1305Jwk,
    (AES256GCMJwk._DEFAULT_HEADER['enc'], None): AES256GCMJwk,
