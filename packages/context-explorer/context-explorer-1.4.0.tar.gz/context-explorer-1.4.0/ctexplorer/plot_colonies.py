...
#    self.pushButton_identify_colonies.setEnabled(False)
    ...
    fig_plot_wells.suptitle(os.path.basename(self.file_path), fontsize=6,
                            fontstyle='normal', color='grey')
    ...
    for well_name, well_df in self.data.groupby('Well'):
        ...
        fig_plot_wells = plot_wells(
            self.well_labels, well_name, well_df, fig_plot_wells, xmin, xmax,
            ymin, ymax, self.well_colony_vertices,
    ...
    fig_path = os.path.join(
        self.ce_dir_name, 'PlateOverview{}{}'.format(date, ext))
    ...
    with plt.rc_context({'font.size': 5, 'axes.linewidth': 0.3}):
        ...
        if well_colony_vertices:
            for colony_name in well_df['Colony'].unique():
                if colony_name != '-1':
                    colony_vertices = well_colony_vertices[1]['{}-{}'.format(
                        well_name, colony_name)]
                    ax.plot(colony_vertices[:, 0], colony_vertices[:, 1],
                            lw=0.2)
...
#    self.fig_well_patterning = plt.figure('WellPatterning',
#         figsize=(1920/my_dpi, 1080/my_dpi), dpi=my_dpi)
    # The subplot must be created prior to setting the tight layout
    ax_subplot = self.fig_well_patterning.add_subplot(
        8, 12, self.well_labels.index(self.well_name)+1)
    # This is to be able to access all the sublots afterwards. Not needed right
    # now.
...
#    ax_subplot = self.axes_well_patterning.flat[self.well_num]
    ...
    self.fig_well_patterning.tight_layout(rect=[0.05, 0, 0.95, 0.97])
    # Maybe make the subplots flexible. Then ii cannot use a list for the
    # wellabels
    ax_subplot.set_title(
        self.well_name + ' - ' + self.plate_layout[self.well_num] +
    ...
    if self.radioButton_thresholds.isChecked():
        ...
        if (self.checkBox_threshold_channel2.isChecked() and
            ...
            self.fig_well_patterning.suptitle(
                os.path.basename(self.file_path), fontsize=6,
                fontstyle='normal', color='grey')
            # Find the indexes for all the different combinations of the
            # thresholds
            ...
            double_negative = np.where(
                (np.array(self.data.AvgIntenCh2.iloc[self.idx_cells_well]) < self.threshold2) &
                (np.array(self.data.AvgIntenCh3.iloc[self.idx_cells_well]) < self.threshold3))
            # Create a color matrix that holds the colors to the cells of the
            ...
            # Plot the cells at different transparency levels and with the
            # color specified above
            plot_clustered = plt.scatter(
                self.data.LeftPixsize.values[
                    self.idx_cells_well[self.labels != -1]],
                self.data.TopPixsize.values[
                    self.idx_cells_well[self.labels != -1]],
            ...
            artist_pospos = plt.Line2D(
                (0, 0), (0, 0), color=[1, 0.2, 0], marker='o', markersize=1,
                linestyle='', markeredgecolor='none', alpha=0.5)
            artist_posneg = plt.Line2D(
                (0, 0), (0, 0), color=[0.8, 0, 0.8], marker='o', markersize=1,
                linestyle='', markeredgecolor='none', alpha=0.5)
            artist_negpos = plt.Line2D(
                (0, 0), (0, 0), color=[0, 0.75, 1], marker='o', markersize=1,
                linestyle='', markeredgecolor='none', alpha=0.5)
            artist_negneg = plt.Line2D(
                    (0, 0), (0, 0), color=[0.1, 0.1, 0.1], marker='o',
                    markersize=1, linestyle='', markeredgecolor='none',
                    alpha=0.5)
        ...
        # Thresholding with one channel
            ...
            self.fig_well_patterning.suptitle(
                os.path.basename(self.file_path), color='grey')
            # Find the indexes for all the different combinations of the
            # thresholds
            positive = np.where(
                np.array(self.data.AvgIntenCh2
                         .iloc[self.idx_cells_well] >= self.threshold2))
            negative = np.where(
                np.array(self.data.AvgIntenCh2
                         .iloc[self.idx_cells_well] < self.threshold2))
            # Create a matrix of the colors to the cells of the corresponding
            # indices
            color = np.zeros((len(self.idx_cells_well), 3))
            ...
            # Create custom artists to be shown in the legend
            artist_pos = plt.Line2D(
                (0, 0), (0, 0), color=[1, 0.2, 0], marker='o', markersize=1,
                linestyle='', markeredgecolor='none', alpha=0.5)
            artist_neg = plt.Line2D(
                (0, 0), (0, 0), color=[0.1, 0.1, 0.1], marker='o',
                markersize=1, linestyle='', markeredgecolor='none', alpha=0.5)
            # Create legend from custom artist/label lists
            leg = ax_subplot.legend(
                [artist_pos, artist_neg], ['Ch2+', 'Ch2-'], numpoints=1,
                ncol=2, fontsize=1.1, markerscale=0.75, loc='upper center',
                bbox_to_anchor=(0.5, 1.02), borderpad=0.5, handletextpad=0.1)
            fr = leg.get_frame()
            fr.set_lw(0.3)
            ...
            # thresholds
            positive = np.where(
                np.array(self.data.AvgIntenCh3
                         .iloc[self.idx_cells_well] >= self.threshold3))
            negative = np.where(
                np.array(self.data.AvgIntenCh3
                         .iloc[self.idx_cells_well] < self.threshold3))
            # Create a matrix of the colors to the cells of the corresponding
            ...
            # Create legend from custom artist/label lists
            leg = ax_subplot.legend(
                [artist_pos, artist_neg], ['Ch3+', 'Ch3-'], numpoints=1,
                ncol=2, fontsize=1.1, markerscale=0.75, loc='upper center',
                bbox_to_anchor=(0.5, 1.02), borderpad=0.5, handletextpad=0.1)
            fr = leg.get_frame()
            fr.set_lw(0.3)
    # Color cells after channel 2 intensity
        ...
        self.fig_well_patterning.suptitle(
            os.path.basename(self.file_path), fontsize=4, color='grey')
        self.fig_well_patterning.suptitle(
            '\n\nEncircled cells have protein intensities above the channel2 '
            'threshold', fontsize=3, color='grey')
        # For clustered cells
        # Set the edgecolor to black if above the threshold and white if below
        edgecolor = 'none'  # np.where(
#            self.data.AvgIntenCh2.iloc[self.idx_cells_well[self.labels != -1]]
#                > self.threshold2, 'k', 'w')
        color = self.data.AvgIntenCh2.iloc[
            self.idx_cells_well[self.labels != -1]]
        # Plot cells in colonies with a opacity of 0.7 and a color value
        # corresponding to their channel intensity in a hot colormap
        # Use vmin and vmax to have the threshold always represent the same
        # color, preferebly in the shift from orange to yellow so that it is
        # easy to detect
        plot_clustered = plt.scatter(
            self.data.LeftPixsize.values[
                self.idx_cells_well[self.labels != -1]],
            ...
            vmin=self.colorbar_min_array[self.file_num, 0],
            vmax=self.colorbar_max_array[self.file_num, 0],
            edgecolor=edgecolor, alpha=0.6)
        # For unclustered cells, opacity = 0.3
        edgecolor = np.where(
            self.data.AvgIntenCh2.iloc[
                self.idx_cells_well[self.labels == -1]] >
            self.threshold2, 'k', 'w')
        color = self.data.AvgIntenCh2.iloc[
            self.idx_cells_well[self.labels == -1]]
        ...
        axins1 = inset_axes(
            ax_subplot, width="50%",  # of parent box width
            height="2.5%", loc=1, bbox_to_anchor=(-.01, .012, .7, 1),
            bbox_transform=ax_subplot.transAxes, borderpad=0)
        # Create the colorbar
        cbar = plt.colorbar(
            plot_clustered, cax=axins1, orientation="horizontal",
            ticks=[self.colorbar_min_array[self.file_num, 0], self.threshold2,
                   self.colorbar_max_array[self.file_num, 0]])
        ...
        cbar.ax.set_xticklabels(
            [str(int(self.threshold2/20)), 'Threshold\n' +
    ...
    # Color cells after channel 2 intensity
        ...
        self.fig_well_patterning.suptitle(os.path.basename(self.file_path),
                                          color='grey')
        self.fig_well_patterning.suptitle(
            '\nEncircled cells have protein intensities above the channel3 '
            'threshold', color='grey')
        # Plot cells in colonies with a opacity of 0.7 and a color value
        # corresponding to their channel intensity in a hot colormap
        edgecolor = np.where(self.data.AvgIntenCh3.iloc[self.idx_cells_well[
            self.labels != -1]] > self.threshold3, 'k', 'w')
        color = self.data.AvgIntenCh3.iloc[
            self.idx_cells_well[self.labels != -1]]
        plot_clustered = plt.scatter(
            self.data.LeftPixsize.values[
                self.idx_cells_well[self.labels != -1]],
            self.data.TopPixsize.values[
                self.idx_cells_well[self.labels != -1]],
            s=0.09, c=color, cmap=plt.cm.hot, marker='o', linewidths=0.05,
            vmin=self.colorbar_min_array[self.file_num, 1],
            vmax=self.colorbar_max_array[self.file_num, 1],
            edgecolor=edgecolor, alpha=0.6)
        # Plot cells in colonies with a opacity of 0.7 and a color value
        # corresponding to their channel intensity in a hot colormap
        edgecolor = np.where(
            self.data.AvgIntenCh3.iloc[self.idx_cells_well[
                self.labels == -1]] > self.threshold3, 'k', 'w')
        color = self.data.AvgIntenCh3.iloc[
            self.idx_cells_well[self.labels == -1]]
        plt.scatter(
        ...
        cbar = plt.colorbar(
            plot_clustered, cax=axins1, orientation="horizontal",
            ticks=[self.colorbar_min_array[self.file_num, 1], self.threshold3,
                   self.colorbar_max_array[self.file_num, 1]])
        # Set the properties of the ticks and their labels
        axins1.xaxis.set_ticks_position("top")
        cbar.ax.tick_params(axis='x', direction='inout', length=1, pad=-1,
                            labelsize=1, width=0.2)
        cbar.ax.set_xticklabels(
            [str(int(self.threshold3/20)), 'Threshold\n' +
                str(int(self.threshold3)), str(int(self.threshold3/0.35))])

    # Calculate the convex hull points of each colony to be able to plot the
    # perimeter outline later on in the parent function
    if self.checkBox_convex_hull.isChecked():
        for hull_points_idx, hull_points_array in (
                enumerate(self.cells_in_colonies2)):
            # ConvexHull and Delauny needs at least three values
            # Also, all cells cannot be of the same x or y coordinate. The
            # convex hull can only be drawn if there are 2 dimensions
            if len(hull_points_array) > 2 and np.all([
                    min(hull_points_array[:, 0]) !=
                    max(hull_points_array[:, 0]),
                    min(hull_points_array[:, 1]) !=
                    max(hull_points_array[:, 1])]):
                hull = ConvexHull(hull_points_array)
                # New scipy syntax
#                ax_subplot.plot(hull_points_array[hull.vertices,0],
#                hull_points_array[hull.vertices,1], 'k-', lw=0.1)
                # Hull simplices are the indices for -cells_in_colonier- of the
                # cells incuded in the convex hull
                for simplex in hull.simplices:
                    ax_subplot.plot(
                        hull_points_array[simplex, 0],
