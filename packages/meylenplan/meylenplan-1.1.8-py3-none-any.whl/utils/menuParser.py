...
def create_meals(lines):
    ...
    # Match friday's dessert which doesn't have a price and is for sure without any allergens ;)
    ...
    for line in lines:
        ...
        if match:
            ...
            if match:
                meals.append(Meal(name=match[1], allergens=match[2]))
                continue
            match = re.match(free_dessert_pattern, line)
            if match:
                meals.append(Meal(name=match[0], allergens=None))
                continue
            match = re.match(arbitrary_pattern, line)
            if match:
                meals.append(Meal(name = match[0], allergens=None, potentially_not_a_meal=True))
                continue
