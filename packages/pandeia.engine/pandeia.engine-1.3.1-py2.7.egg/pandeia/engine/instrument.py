...
class Instrument(InstrumentConfig):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _get_psf_library_version(self):
        ...
        if "V2 Library" in version:
            self.psf_library_version = 2
        else:
            self.psf_library_version = 1
            warn('The PSF library organization has changed. Support for older pandeia_refdata packages will be removed '
                 'in v1.5', category=FutureWarning)
    ...
    def get_detector_pars(self):
        ...
        """
        return self.read_detector_pars('detector')
    ...
    def read_detector_pars(self,detector_key):
        """
        ...
        for parameter_key in self.det_pars.keys():
            if not isinstance(self.det_pars[parameter_key], dict):
                # If the field exists and is not a dict then just return the value
                parameter_value = self.det_pars[parameter_key]

            elif isinstance(self.det_pars[parameter_key], dict) and 'key_format' in self.det_pars[parameter_key].keys():
                ...
                key_lookup = []
                for kf in self.det_pars[parameter_key]['key_format']:
                    kfelement = kf.split('__')
                    for kfe in kfelement:
                        try:
                            key_lookup.append(value_lookup[kfe])
                        except Exception as e:
                            raise DataError("Unknown key format {} value {} listed in {}".format(kfe, value_lookup[kfe], self.det_pars[parameter_key]))
                ...
                if not any([x is None for x in key_lookup]):
                    ...
                    if key_lookup in self.det_pars[parameter_key]:
                        parameter_value = self.det_pars[parameter_key][key_lookup]

                    # If they key_lookup was not present or some of the required
                    # data was not present then just return the default.
                    elif "default" in self.det_pars[parameter_key]:
                        parameter_value = self.det_pars[parameter_key]['default']
                    else:
                        raise DataError("There is no default value in {}".format(self.det_pars[parameter_key]))
