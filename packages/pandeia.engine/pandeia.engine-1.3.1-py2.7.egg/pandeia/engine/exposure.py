...
class ExposureSpecification:
    ...
    def get_times_mir(self):
        """
        The time formulae are defined in Valenti et al. 2017, JWST-STScI-006013. Note that we have to subtract the groups that
    ...
    def get_groups_before_sat(self, slope, fullwell):
        ...
        time_to_saturation = fullwell / slope.clip(1e-10, np.max(slope))
        if self.det_type == 'sias':
            groups_before_sat = (time_to_saturation - self.tfffr) / (self.nframe * self.tframe)
        elif self.det_type == 'h2rg':
            groups_before_sat = (((time_to_saturation - self.tfffr) / self.tframe) - self.nframe) / (self.nframe+self.nskip) + 1.
        else:
    ...
    def get_saturation_fraction(self, slope, fullwell):
        ...
        """
        return self.saturation_time / (fullwell / slope)
