...
class Shell(bot.Bot):
    ...
    def start(self):
        logging.warning("starting shell")
        if not bot.cfg.workdir:
            bot.cfg.workdir = hd(".bot")
            cdir(bot.cfg.workdir)
        if bot.cfg.resume:
            self.resume()
        super().start()
...
def parse_cli(name="bot", options=None, usage="", version="", workdir=""):
    global histfile
    ...
    bot.cfg.logdir = os.path.abspath(os.path.join(os.path.expanduser("~"), ".%slogs" % name))
    bot.cfg.name = name
    bot.cfg.version = version
    bot.cfg.args = arguments
    if workdir:
        bot.cfg.workdir = bot.cfg.workdir or workdir
    if not bot.cfg.workdir:
        bot.cfg.workdir = os.path.abspath(os.path.join(os.path.expanduser("~"), ".%s" % cfg.name))
    if not os.path.exists(bot.cfg.workdir):
        cdir(bot.cfg.workdir)
    if not os.path.exists(bot.cfg.logdir):
        cdir(bot.cfg.logdir)
    histfile = ld("history")
