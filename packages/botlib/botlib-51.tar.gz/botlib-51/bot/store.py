...
from bot.base import construct, get_cls, get_type, get_types, fn_time
from bot.base import starttime
...
def filenames(ftype, delta=0):
    p = os.path.join(bot.cfg.workdir, ftype)
    now = time.time()
    past = now + delta
    for rootdir, dirs, files in os.walk(p, topdown=True):
        if p not in rootdir:
            continue
        for fn in files:
            fnn = os.path.join(rootdir, fn).split(bot.cfg.workdir)[-1][1:]
            if delta:
                if fn_time(fnn) < past:
                    continue
            yield fnn
...
def find(event):
    res = {x.split(".")[-1].lower() for x in os.listdir(bot.cfg.workdir)}
    if not event.args:
