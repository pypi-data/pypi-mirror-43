...
def compose_RAVEN(gracedb_id, authors=(),
    ...
    if GRB:
        ...
        voevents = client.voevents(em_event_id).json()['voevents']
        for voevent in voevents:
            voevent_text = client.files(
                em_event_id, voevent['filename']).read()
            root = lxml.etree.fromstring(voevent_text)
...
def compose_grb_medium_latency(
    ...
    external_trigger_id = event['extra_attributes']['GRB']['trigger_id']
    # FIXME: Currently we are using the VOEvent ivorn in place of the
    # GCN circular that made the first announcement of the GRB.
    # When this is fixed, the gcn_citation should generate a string of
    # the form (Sbarufatti et al. 2019, GCN 23670) in the template
    gcn_citation = event['extra_attributes']['GRB']['ivorn']
