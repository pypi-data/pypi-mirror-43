...
class GraphData(gensim.utils.SaveLoad):
    ...
    def find_one_node_by_property(self, property_name, property_value):
        for node_id, node_json in self.id_to_node_map.items():
            if property_name in node_json.keys() and node_json[property_name] == property_value:
                return node_json
        return None
    ...
    def find_one_node_by_properties(self, **properties):
        for node_id, node_json in self.id_to_node_map.items():
            ...
            for property_name, property_value in properties.items():
                if property_name not in node_json.keys() or node_json[property_name] != property_value:
                    is_match = False
                    break
            if is_match:
                return node_json
