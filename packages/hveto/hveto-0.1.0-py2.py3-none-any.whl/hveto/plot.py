...
def before_after_histogram(
    ...
    # make histogram
    if range is None:
        range = min(list(map(numpy.min, (x, y)))), max(list(map(numpy.max, (x, y))))
...
def veto_scatter(
    ...
    if ax.get_legend_handles_labels()[0]:
        ...
        legargs.update(dict((x[7:], axargs.pop(x)) for x in list(axargs.keys())
...
def significance_drop(outfile, old, new, show_channel_names=None, **kwargs):
    ...
    winner = sorted(list(old.items()), key=lambda x: x[1])[-1][0]
    ...
    # or just show systems of channels
        ...
        for i, c in enumerate(channels):
            ...
            try:
                ...
                systems[sys] = [i, 1]
        systems = sorted(list(systems.items()), key=lambda x: x[1][0])
        labels, counts = list(zip(*systems))
        xticks, xmticks = list(zip(*[(a, a+b/2.) for (a, b) in counts]))
