    ...
    for i, x in enumerate(triggers):
        ...
        def add_if_coinc(event):
            if event['channel'] == channel:
                return
            in_seg = [s for s in segs if s[0] <= event['time'] <= s[1]]
            if not in_seg:  # no triggers in window
                return
            for k, w in enumerate(in_seg):
                for snr in [s for s in snrs if event['snr'] >= s]:
        ...
        # count 'em up
        for p, cset in list(channels.items()):
            for c in cset:
                try:
                    coincs[p][c] += 1
                except KeyError:
...
def find_max_significance(primary, auxiliary, channel, snrs, windows, livetime):
    ...
    for p, cdict in list(coincs.items()):
        dt, snr = p
        for chan in cdict:
