...
def main():
    ...
    all_files = list(itertools.chain(*[glob(f) if '*' in f else [f] for f in args.files]))
    ...
    if not private_token:
        if re.match(r'[A-Za-z0-9]', all_files[0]):
            ...
            print("PRIVATE-TOKEN not available, must be in env var 'PRIVATE_TOKEN' or provided as arg")
            exit(1)

    if private_token == all_files[0]:
