...
def main():
    ...
    parser.add_argument('--artifact-file', action='append', help='Link individual artifact file from the current job')
    ...
    if args.artifact_zip or args.artifact_file:
        if not artifact_job:
            print("Must provide --artifact-job <id> for artifact files")
            exit(-1)

        for fname in args.artifact_file:
            if fname.startswith('./'):
                fname = fname[2:]
