...
__all__ = ["mount", "tkmount", "download_large_file", "unzip", "zip", "open_dir"]

def mount(path="/content/gdrive/My Drive/", mountpoint='/content/gdrive', force_remount=False, timeout_ms=15000):
...
def open_dir(path="/content/gdrive/My Drive/"):
  print("Opening directory {}".format(path))
  try:
      os.chdir(path)
      print("Done!")
  except:
...
def tkmount(TOKEN=None, mountpoint='/content/gdrive', force_remount=False):
  ...
  while True:
    ...
    if case == 0:
      ...
      d.terminate(force=True)
      raise ValueError('mount failed (invalid token?)')
    elif case == 2:
      if TOKEN is None:
