...
device name so that Quisk can read the available settings.",
  ...
  def MakeSoapyControls(self):
    ...
    name = 'soapy_gain_mode_rx'
    self.gain_mode = radio_dict[name]
    choices = ['total']
    if len_gain_names >= 3:
      choices.append('detailed')
    if radio_dict.get('soapy_hasGainMode_rx', 0):
      choices.append('automatic')
    if self.gain_mode not in choices:
      self.gain_mode = radio_dict[name] = 'total'
      local_conf.settings_changed = True
    txt, cb, btn = self.AddTextComboHelp(self.col, self.name_text[name], self.gain_mode, choices, self.help_text[name], True, border=self.border)
    cb.handler = self.OnChange
    cb.quisk_data_name = name
    self.NextCol()

    values = radio_dict['soapy_gain_values_rx']
    ...
    self.FitInside()
  def FixGainButtons(self):
    for i in range(len(self.gains_rx)):
      ctrl = self.gains_rx[i]
      if self.gain_mode == "automatic":
        ctrl.Enable(False)
      elif self.gain_mode == "total":
        if i == 0:
          ctrl.Enable(True)
        else:
          ctrl.Enable(False)
  ...
  def OnChange(self, ctrl):
    ...
    local_conf.settings_changed = True
    # Immediate changes
    if name == 'soapy_gain_mode_rx':
      self.gain_mode = value
      self.FixGainButtons()
    if soapy and self.radio_name == Settings[1]:	# changed for current radio
      if name == 'soapy_gain_mode_rx':
        application.Hardware.ChangeRxGain()
      elif name in ('soapy_setAntenna_rx', ):
        soapy.set_parameter(name, value, 0.0)	# string values
  def OnGain(self, event):
    radio_dict = local_conf.GetRadioDict(self.radio_name)
    obj = event.GetEventObject()
    value = obj.GetValue()
    radio_dict['soapy_gain_values_rx'][obj.quisk_data_name] = value
    local_conf.settings_changed = True
    if soapy and self.radio_name == Settings[1]:	# changed for current radio
      application.Hardware.ChangeRxGain()
