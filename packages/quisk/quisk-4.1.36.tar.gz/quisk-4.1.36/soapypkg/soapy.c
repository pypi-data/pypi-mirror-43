...
static int quisk_read_samples(complex double * cSamples)
{
	...
	if (shutdown_sample_device) {
		...
		nSamples = SoapySDRDevice_readStream(soapy_sample_device, rxStream, stream_buffs, num_samp, &flags, &timeNs, data_poll_usec * 2);
		if (nSamples < 0) {
...
{  // Parameter name can end in "_rx" or "_tx" to specify direction.
	...
	if (soapy_sample_device) {
		if ( ! strncmp(param, "soapy_setAntenna", length)) {	// do not set empty string
