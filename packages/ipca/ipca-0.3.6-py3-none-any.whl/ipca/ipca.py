...
class IPCARegressor:
    ...
    def _nan_check(self, Z, Y):
        ...
        Z_nan = np.isnan(Z, dtype=bool)
        Y_nan = np.isnan(Y, dtype=bool)
    ...
    def _unpack_panel_XY(self, P):
        ...
        """
        # Transform to DataFrame for tensor generation
        P = pd.DataFrame(P)
        P.rename(columns={P.columns[0]: 'id', P.columns[1]: 'date'},
                 inplace=True)
        ids = np.sort(P['id'].unique())
        ...
        # Create placeholder panel for all observations
        balanced_panel = pd.DataFrame({'id': np.array([[x]*len(dates) for x in ids]).flatten(),
                                       'date': list(dates) * len(ids)})
        # Merge panel onto placeholder and fill missing with nan
        P = pd.merge(balanced_panel, P, how='left',
                     left_on=['id', 'date'],
                     right_on=['id', 'date']).fillna(np.nan)

        Y = np.reshape(P.iloc[:, 2].values, (N, T))
        P.drop([P.columns[2]], axis=1, inplace=True)
        P = np.array(list(P.groupby('id').apply(lambda df: df.to_numpy())))
        P = np.swapaxes(P, 1, 2)
        P = P[:, 2:, :]  # remove id and date
        print(np.shape(P))
