...
class bert_sent_encoding:
	...
	def get_vector(self, texts, word_vector=False, layer=-1):
		...
		for input_ids, input_mask, example_indices in eval_dataloader:
			...
			last_encoder_layer = all_encoder_layers[layer]

			for b, example_index in enumerate(example_indices):
				if word_vector:
					vector = last_encoder_layer[b].detach().cpu().numpy().tolist()
				else:
...
if __name__ == '__main__':
	...
	text = ['你好', '吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗吃饭了吗', '你好'] * 16
	start = time.time()
