...
class DownloadWidget(BaseWidget):
    ...
    def select_platform(self, b):
        ...
        supported_source_keys = list(supported_source_drivers.keys())
        ...
        if default_source_driver is not None:
            ...
            supported_source_keys.remove("Default")

        self._widget_source.options = supported_source_keys
        if len(self._widget_source.options) > 0:
            self._widget_source.value = self._widget_source.options[
                0] if default_source_driver is None else default_source_driver[0]
        else:
            self._widget_source.options = ['Custom']
        ...
        if len(self._widget_source_rate.options) > 0:
            self._widget_source_rate.value = self._widget_source_rate.options[
                0] if default_source_driver is None else default_source_driver[1]
        else:
    ...
    def get_platform_names(self):
        ...
        for platform in self._dsk.platforms.platform_list:
            pf['{} {}'.format(platform.platform,
                              platform.platform_version)] = platform.id
    ...
    def create_widget(self):
        ...
        self._widget_source = Dropdown(description="Source", options=[])
