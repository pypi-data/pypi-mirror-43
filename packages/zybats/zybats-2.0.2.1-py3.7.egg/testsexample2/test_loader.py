...
from zybats import exceptions
from zybats.core import loader
...
class TestFileLoader(unittest.TestCase):
    ...
    def test_load_dot_env_file(self):
        ...
        self.assertEqual(env_variables_mapping["UserName"], "atsfunc")
    ...
    def test_locate_file(self):
        with self.assertRaises(exceptions.FileNotFound):
            loader.locate_file(os.getcwd(), "atsfunc.py")

        with self.assertRaises(exceptions.FileNotFound):
            loader.locate_file("", "atsfunc.py")

        start_path = os.path.join(os.getcwd(), "tests")
        self.assertEqual(
            loader.locate_file(start_path, "atsfunc.py"),
            os.path.join(
                os.getcwd(), "tests/atsfunc.py"
            )
        )
        self.assertEqual(
            loader.locate_file("tests/", "atsfunc.py"),
            os.path.join(os.getcwd(), "tests", "atsfunc.py")
        )
        self.assertEqual(
            loader.locate_file("tests", "atsfunc.py"),
            os.path.join(os.getcwd(), "tests", "atsfunc.py")
        ...
        self.assertEqual(
            loader.locate_file("tests/base.py", "atsfunc.py"),
            os.path.join(os.getcwd(), "tests", "atsfunc.py")
        )
        self.assertEqual(
            loader.locate_file("tests/data/demo_testcase.yml", "atsfunc.py"),
            os.path.join(os.getcwd(), "tests", "atsfunc.py")
        )
...
class TestModuleLoader(unittest.TestCase):
    ...
    def test_load_atsfunc_module(self):
        loader.load_project_tests(os.path.join(os.getcwd(), "zybats"))
    ...
    def test_load_atsfunc_py(self):
        loader.load_project_tests("tests/data/demo_testcase.yml")
        project_working_directory = loader.project_mapping["PWD"]
        atsfunc_functions = loader.project_mapping["functions"]
        self.assertEqual(
            project_working_directory,
            os.path.join(os.getcwd(), "tests")
        )
        self.assertIn("gen_md5", atsfunc_functions)
        ...
        loader.load_project_tests("zybats/__init__.py")
        project_working_directory = loader.project_mapping["PWD"]
        atsfunc_functions = loader.project_mapping["functions"]
        self.assertEqual(
            project_working_directory,
        ...
        self.assertEqual(atsfunc_functions, {})
