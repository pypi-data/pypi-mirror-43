...
from zybats import exceptions, utils
from zybats.compat import basestring, builtin_str, numeric_types, str
    ...
    Args:
        parameters (list) parameters: parameter name and value in list
            parameter value may be in three types:
                ...
                (3) call custom function in atsfunc.py, "${gen_app_version()}"

        variables_mapping (dict): variables mapping loaded from testcase config
        functions_mapping (dict): functions mapping loaded from atsfunc.py
...
def get_mapping_function(function_name, functions_mapping):
    ...
    Raises:
        exceptions.FunctionNotFound: function is neither defined in atsfunc.py nor builtin.
    ...
    try:
        # check if zybats builtin functions
...
def parse_string_functions(content, variables_mapping, functions_mapping):
    ...
    for func_content in functions_list:
        ...
        if func_name in ["parameterize", "P"]:
            ...
            project_working_dir = utils.get_os_environ("pwd")
            data_path = os.path.join(project_working_dir, 'data')
            if not os.path.exists(data_path):
                raise exceptions.ParamsError("Data dir should be exited when you use csv file!")
            csv_file_path = os.path.join(data_path, args[0])
            from zybats.core import loader
            eval_value = loader.load_csv_file(csv_file_path)
