...
from zybats import (exceptions, parser, report, runner,
                    utils, validator)
from zybats.core import loader, logger
...
class zybats(object):
    ...
    def __init__(self, failfast=False, save_tests=False, report_template=None, report_dir=None,
        ...
        """
        self.exception_stage = "initialize zybats()"
        kwargs = {
            "failfast": failfast,
    ...
    def run_tests(self, tests_mapping):
        ...
        self._summary['ExecScenario'] = os.environ["ExecScenario"] if "ExecScenario" in os.environ else None
        self._summary['CaseSum'] = os.environ["CaseSum"] if "CaseSum" in os.environ else None
        ...
        if not self.report_dir:
            test_case_path = parsed_tests_mapping.get("project_mapping").get("PWD")
            self.__generate_report_path(test_case_path)
    ...
    def __generate_report_path(self, test_case_path):
        if test_case_path:
            self.report_dir = os.path.join(test_case_path, "reports")
