...
import yaml
from zybats import exceptions, utils, validator
from zybats.core import logger
    ...
    with io.open(dot_env_path, 'r', encoding='utf-8') as fp:
        for line in fp:
            ...
            if "=" in line:
                ...
                variable, value = line.split(":", 1)
            elif not len(line.strip()) or line.startswith('#'):
                continue
            else:
...
def load_builtin_functions():
    ...
    """
    from zybats import built_in
    return load_module_functions(built_in)


def load_atsfunc_functions():
    """ load project atsfunc.py module functions
        atsfunc.py should be located in project working directory.

    Returns:
        dict: atsfunc module functions mapping
            {
                "func1_name": func1,
                "func2_name": func2
            }
    ...
    """
    # load atsfunc.py module
    imported_module = importlib.import_module("atsfunc")
...
def locate_atsfunc_py(start_path):
    """ locate atsfunc.py file

    Args:
        start_path (str): start locating path, maybe testcase file path or directory path

    Returns:
        str: atsfunc.py file path, None if not found

    """
    try:
        # locate atsfunc.py file.
        atsfunc_path = locate_file(start_path, "atsfunc.py")
...
def load_project_tests(test_path, dot_env_path=None):
    """ load api, testcases, .env, atsfunc.py functions.
    ...
    Returns:
        dict: project loaded api/testcases definitions, environments and atsfunc.py functions.

    """
    # locate atsfunc.py file
    atsfunc_path = locate_atsfunc_py(test_path)

    if atsfunc_path:
        # The folder contains atsfunc.py will be treated as PWD.
        project_working_directory = os.path.dirname(atsfunc_path)
    else:
        # atsfunc.py not found, use os.getcwd() as PWD.
        project_working_directory = os.getcwd()

    # add PWD to sys.path
    sys.path.insert(0, project_working_directory)
    utils.set_os_environ({"pwd" : project_working_directory})

    # load .env file
    # NOTICE:
    # environment variable maybe loaded in atsfunc.py
    # thus .env file should be loaded before loading atsfunc.py
    ...
    if atsfunc_path:
        # load atsfunc.py functions
        atsfunc_functions = load_atsfunc_functions()
    else:
        atsfunc_functions = {}

    # locate PWD and load atsfunc.py functions

    project_mapping["PWD"] = project_working_directory
    project_mapping["functions"] = atsfunc_functions
