...
class Amplitude(Grid):
    ...
    def save(self, filename):
        ...
        info=json.dumps(
            {
                "qmax":self.q_max,
                "qmin": 0.0, #not currently used, but adding for future compatibility
...
def ampj_to_amp_converter(amp_filename):
    ampj=Amplitude.load(amp_filename)
    new_filename=amp_filename[:-1]
