...
def pixelSearch(im1, im2, nodePositions, halfWindowSize, searchRange,
    ...
    def getImagettes(nodeNumber, nodePositions, Ffield, searchRange, halfWindowSize, im1, im2, minSubVolume, greyThreshold):
        returnStatus = 2

        initialDisplacement = Ffield[nodeNumber,0:3,3].astype(int)
    ...
    if Ffield is None:
        ...
        for nodeNumber in range(numberOfNodes): Ffield[nodeNumber] = numpy.eye(4)
    ...
    while finishedNodes != numberOfNodes:
        ...
        if workersActive.sum() < numberOfWorkers and not q.empty():
            ...
            imagetteReturns = getImagettes(nodeNumber, nodePositions, Ffield, searchRange, halfWindowSize, im1, im2, minSubVolume, greyThreshold)
            ...
            if imagetteReturns['returnStatus'] == 2:
                ...
                finishedNodes                    += 1
                Ffield[ nodeNumber, 0:3, 0:3 ]    = numpy.eye(3) 
        ...
        if writeReturns:
                ...
                Ffield[nodeNumber, 0:3, 3 ] = numpy.array(returns['transformation']['t'])
...
def subPixel(im1, im2,
    ...
    def getImagettes(nodeNumber, im1, im2, Ffield, nodePositions, margin, halfWindowSize, greyThreshold, im1mask):
        # Initialise defaults
        imagette1mask = None
        Finit = None
    ...
    while finishedNodes != numberOfNodes:
        ...
        if workersActive.sum() < numberOfWorkers and not q.empty():
            ...
            imagetteReturns = getImagettes(nodeNumber, im1, im2, Ffield, nodePositions, margin, halfWindowSize, greyThreshold, im1mask)
