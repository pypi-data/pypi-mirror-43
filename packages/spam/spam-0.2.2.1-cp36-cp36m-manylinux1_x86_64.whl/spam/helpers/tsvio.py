...
def writeRegistrationTSV(fileName, regCentre, regReturns):
    ...
    ----------
        ...
        regCentre : 3-component list
            A list containing the point at which `Fcentre` has been measured.
...
def readTSV(fileName, fieldBinRatio=1.0, readDisplacements=False, readFs=False, readConvergence=False, readPixelSearchCC=False, readEverything=True, returnFs=True):
    ...
    ----------
        ...
        readDisplacements : bool, optional
            Read "zDisp", "yDisp", "xDisp", displacements from the TSV file, and not the rest of the F matrix?
            Default = False

        readFs : bool, optional
            Read full F matrices from the TSV file?
            Same as displacements with also "F11" to "F33" columns
            Default = False

        readConvergence : bool, optional
            Read "SubPixReturnStat", "SubPixDeltaFnorm", "SubPixIterations" from file
            Default = False
    ...
    if readDisplacements and not returnFs:
        output.update({"Displacements": Ffield[:,0:3,-1]})
    else:
        output.update({"Ffield": Ffield})
        ...
        if lab:
            ...
            for component in Fcomponents:
                tifffile.imsave("{}/{}-{}.tif".format(outDir, prefix, component[0]),
                                ltk.convertLabelToFloat(labelled, Ffield[:, component[1], component[2]]).astype('<f4'))
            if returnRS:
...
def TSVtoVTK(fileName, fieldBinRatio=1.0, pixelSize=1.0, returnRS=False, outDir=None, prefix=None):
    ...
    if fi["numberOfLabels"] != 0:
        ...
        if not returnRS:
            pointData = {"XYZdisp":  Ffield[1:, :-1, 3] * pixelSize}
    ...
    # if not, is a DICregularGrid result
        ...
        if not returnRS:
            cellData = {"XYZdisp":  (Ffield[:, :-1, 3] * pixelSize).reshape((dims[0], dims[1], dims[2], 3))}
