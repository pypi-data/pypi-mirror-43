...
def projectBagiStrainToGrains(delaunay, tet_strain, com2):
    """
    ...
    for tet in pbar(range(indxuniq.shape[0])):
        ...
        for jj in range(indx[0].shape[0]):
            ...
            strain_tot = strain_tot + tet_strain[indx[0][jj]]*vol
            vol_tot = vol_tot + vol

        # Divide strain_tot by vol_tot
        strain_tot = strain_tot/vol_tot

        # Store in particles
        grainStrain.append(numpy.array(strain_tot))
        # grainStrainVoigt.append(numpy.array([strain_tot[0, 0], strain_tot[1, 1], strain_tot[2, 2],
        # strain_tot[1, 2], strain_tot[0, 2], strain_tot[0, 1]]))
    # return(grainStrain, grainStrainVoigt)
    return(grainStrain)


def projectBagiStrainToGrid(delaunay, tet_strain, com2, nx=8, ny=8, nz=10):
    """
    ...
    print("spam.mesh.projectBagiStrainToGrid(): Projecting strains to grid...\n")
    for ii in pbar(range(grid_bounds.shape[0])):
        ...
        if (indtet.shape[0] > 0):
            for jj in range(indtet.shape[0]):
                ...
                # Add volume and strain
                strain_tot = strain_tot + tet_strain[indtet[jj]]*vol
                vol_tot = vol_tot + vol
        ...
        gridStrain.append(numpy.array(strain_tot))
        # gridStrainVoigt.append(numpy.array([strain_tot[0, 0], strain_tot[1, 1], strain_tot[2, 2],
        # strain_tot[1, 2], strain_tot[0, 2], strain_tot[0, 1]]))
