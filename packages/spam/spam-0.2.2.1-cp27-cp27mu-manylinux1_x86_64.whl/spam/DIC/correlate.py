...
from . import transformationOperator
...
def lucasKanade(im1, im2,
    ...
    ###########################################################
    ### Important -- since we're moving im2, initial Fs will be
    ...
    if Finit is None:
        ...
        Finit[0:3,-1] *= FinitBinRatio

        # Make a copy, otherwise it's updated outside, could be a problem...
        F = Finit.copy()

        # invert Finit to apply it to im2
        try:
            Finv = numpy.linalg.inv(Finit.copy())
        ...
        if interpolator == 'C':
            im2defCrop = transformationOperator.applyTransformationOperator(im2, F=Finv, interpolationOrder=interpolationOrder)[crop2]

        elif interpolator == 'python':
            im2defCrop = transformationOperator.applyTransformationOperatorPython(im2, F=Finv, interpolationOrder=interpolationOrder)[crop2]
    ...
    while iterationNumber < maxIterations and deltaFnorm > minFchange:
        ...
        if interpolator == 'C':
            #im2defCrop = transformationOperator.applyTransformationOperator(im2, F=F, interpolationOrder=interpolationOrder)[crop2]
            im2defCrop = transformationOperator.applyTransformationOperator(im2, F=Finv, interpolationOrder=interpolationOrder)[crop2]

        elif interpolator == 'python':
            #im2defCrop = transformationOperator.applyTransformationOperatorPython(im2, F=F, interpolationOrder=interpolationOrder)[crop2]
            im2defCrop = transformationOperator.applyTransformationOperatorPython(im2, F=Finv, interpolationOrder=interpolationOrder)[crop2]
        ...
        if verbose:
            ...
            #print("F = \n", F, "\n\n")
            r = transformationOperator.FtoTransformation(F.copy())["r"]
            widgets[3] = progressbar.FormatLabel("  dFnorm={:0>7.5f}   error={:0>4.2f}   t=[{:0>5.3f} {:0>5.3f} {:0>5.3f}]   r=[{:0>5.3f} {:0>5.3f} {:0>5.3f}]   z=[{:0>5.3f} {:0>5.3f} {:0>5.3f}]".format(
                deltaFnorm,
                error,
                F[0,-1],
                F[1,-1],
    ...
    # To respect F gpoing from im1 -> im2, inverse it
    #F = numpy.linalg.inv(F)
    ...
    if im1mask is not None:
        # If a mask on im1 is defined, return an F at the centre of the mass
        maskCOM = ltk.centresOfMass(im1mask[crop1])[-1]
        #print("Mask COM", maskCOM)
        #print( "\nNormal F:\n", F)
        F[0:3, -1] = transformationOperator.FtoTransformation(F.copy(), Fcentre=(numpy.array(im1crop.shape)-1)/2.0, Fpoint=maskCOM)["t"]
        #print( "\nF in mask:\n", F)
...
def pixelSearch(imagette1, imagette2,
    ...
    if type(searchRange) is dict:
        searchRange = numpy.array([searchRange['zRange'], searchRange['yRange'], searchRange['xRange']], dtype='<f4')
    else:
