...
void applyTransformationOperatorC(  int nz1,   int ny1,  int nx1, float* im,   
            ...
            for (int x=0; x<nx2+0; x++ )
            {
                ...
                *   imDef should be initialised to zero, so we can drop it */
                     ...
                     xTransf >= interpOrder && xTransf < nx1 - interpOrder )
                {
                    ...
                    {
                        std::cout << "applyTransformationOperatorC(): Interpolation Order = " << interpOrder << " Not implemented, exiting.\n";
//                         return;
                    }
                }
