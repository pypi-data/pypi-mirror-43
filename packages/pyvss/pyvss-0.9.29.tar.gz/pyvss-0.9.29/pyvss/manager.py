...
class VssManager(object):
    ...
    """
    def __init__(
            self, tk=None, api_endpoint=None,
    ...
    def create_vm(
            self, os, built, bill_dept,
            description, folder, networks, disks,
            name=None, iso=None, notes=None,
            usage='Test', cpu=1, memoryGB=1,
            high_io=False, vss_service=None,
            **kwargs
    ):
        """
        ...
        # generating payload
        json_payload = dict(
            os=os, built_from=built_from,
            bill_dept=bill_dept,
            cpu=cpu, memory=memoryGB, usage=usage,
            high_io=high_io, description=description, folder=folder,
    ...
    def create_vms(
            self, count, name, os, built, bill_dept,
        ...
        # generating payload
        json_payload = dict(
            os=os, built_from=built_from, bill_dept=bill_dept,
            cpu=cpu, memory=memoryGB, usage=usage,
            high_io=high_io,
    ...
    def create_vm_from_image(
            self, os, image, bill_dept,
            description, folder, networks, disks,
            notes=None, usage='Test', name=None,
            cpu=1, memoryGB=1, vss_service=None,
    ...
    def deploy_vm_from_template(
            self, source_template, description, name=None,
            os=None, bill_dept=None,
            folder=None, networks=None,
            disks=None, notes=None, usage=None,
            cpu=None, memoryGB=None, high_io=None,
    ...
    def deploy_vms_from_template(
            self, source_template, description, count=1,
        ...
        new_vms_spec = dict(
            description=description, built_from='template',
    ...
    def get_vm_vss_service(self, uuid):
        ...
        :return: obj
        """
        json = self.request('/vm/{uuid}/vss/service'.format(
            uuid=uuid)
        )
        return json.get('data')
    ...
    def validate_usage(usage):
        ...
        if valid_usage:
            ...
            raise VssError(
                'Usage {0} not supported'.format(usage)
            )
        return usage
    ...
    def validate_build_process(built):
        if built not in VALID_VM_BUILD_PROCESS:
            raise VssError(
                'Built process {0} not supported'.format(built)
            )
        return built
...
def run_main():
    ...
    if not api_token:
        raise VssError(
            'Missing environment variable VSS_API_TOKEN'
        )
