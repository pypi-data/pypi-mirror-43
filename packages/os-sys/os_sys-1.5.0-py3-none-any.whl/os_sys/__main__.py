...
def install_zip(zip_file, end_dir, rpath):
    ...
    with ZipFile(zip_file, 'r') as zipObj:
        ...
        Bar.close()
        print('installing...')
        copytree(rpath + '\\' + os.path.dirname(first).replace('/', '\\'), end_dir)
...
def install(path):
    ...
    with open(file, mode='rb') as fh:
        mystr = str(str(str(to_str(fh.read())).replace('\n', '')).replace('\\r', '\r')).replace('\\n', '')
    e = mystr.split('\r')
    ...
    print('looking for older installation')
    print('if is found un installing...')
    try:
            ...
            try:
                ...
                try:
                    with open(os.path.join(path2, path), mode='w+') as kk:
                        kk.write(str(str(data).replace('\\r', '\r')).replace('\\n', ''))
                except Exception:
                    pass
                
    pbar.close()
    install_zip(the, base, base)
