...
for check in osm:
    ...
    for num in range(0, len(c)):
        c_list.append(c[num])
    for check_num in c_list:
        if check_num in check:
            etc, etc2 = check.rsplit('-')
            num1, num2, num3, etc = check.rsplit('.')
            num1 = int(num1)
            num2 = int(num2)
            num3 = int(num3)
        break
...
if num1 < num_list[0] or num2 < num_list[1] or num3 < num_list[2]:
    print('version %s[0].%s[1].%s[2] is availble you have version %s[3].%s[4].%s[5]' % [num_list[0], num_list[1], num_list[2], num1, num2, num3], file=sys.stderr)
'''
...
run_cmds = run_cmds_()
def docs():
    input('you need to press reload if you get a error then it will work(press enter to close this message)')
...
if __name__ == '__main__':
    print(_g)
from . import *
_m = ['_code', '_download', 'all_dict', 'docs', 'download', 'get_newest_version', 'gpl', 'make_text', 'more_input', 'obj_type', 'object_type', 'os_sys_lib', 'print_all_dirs', 'show_progres', 'test', 'update_progress',
      'os_sys', 'wifi',
           'commands', 'core', 'programs', 'discription', 'errors', 'fail', 'progress_bars',
           'system', 'upload', 'wifi', 'html_text', 'installers', 'py_install']
__all__ = _g + _m
