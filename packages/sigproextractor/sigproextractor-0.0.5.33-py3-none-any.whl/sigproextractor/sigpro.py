...
def sigProfilerExtractor(input_type, out_put, project, refgen="GRCh37", startProcess=1, endProcess=10, totalIterations=8, cpu=-1, hierarchy = False, mtype = ["default"],exome = False, par_h=0.90): 
    ...
    sysdata.write("Date and Clock time when the excecution started: "+str(tic)+"\n")
    sysdata.close()
    ...
    for m in mtypes:
        ...
        while flag:
            ...
            fh = open(layer_directory+"/All_solution_stat.csv", "w")   
            fh.write("Total Signatures,Stability,Frobenius Percentage Error\n") 
            fh.close()
            ...
            for i in range(startProcess,endProcess+1):
                ...
                minimum_stabilities.append(clusterSilhouetteCoefficients.min())
            ...
            solution, all_stats = sub.stabVsRError(layer_directory+"/All_solution_stat.csv", layer_directory, title, all_similirities_list, mutation_type)
            all_stats.insert(1, "Minimum Stability", minimum_stabilities)
            all_stats.to_csv(layer_directory+"/All_solution_stat.csv", sep = ",")
...
# =============================================================================
    ...
    sysdata.write("Date and Clock time when the excecution ended: "+str(toc)+"\n")
    sysdata.write("Total time taken to execute the experiment: "+str(toc-tic)+"\n\n")
