...
def sigProfilerExtractor(input_type, out_put, project, refgen="GRCh37", startProcess=1, endProcess=10, totalIterations=8, cpu=-1, hierarchy = False, mtype = ["default"],exome = False, par_h=0.90): 
    ...
    sysdata.write("Scipy version: "+sklearn.__version__+"\n")
    sysdata.write("Nimfa version: "+nimfa.__version__+"\n")
    
    sysdata.write("\n-------Data Vital Parameters Used for the execution -------\n")
    sysdata.write("input_type: {}\nstartProcess: {}\nendProcess: {}\ntotalIterations: {}\ncpu: {}\nhierarchy: {}\n".format(input_type, startProcess, endProcess, totalIterations, cpu,  hierarchy))
...
# =============================================================================
                # record the samples
                layer_genome.to_csv(output+"/Samples.text", sep = "\t", index_label=[layer_genome.columns.name])
                #similarity_dataframe.to_csv(data_stat_folder+"/Similatiry_Data_All_Sigs"+str(H_iteration)+".text", sep = "\t")
                del layer_genome
