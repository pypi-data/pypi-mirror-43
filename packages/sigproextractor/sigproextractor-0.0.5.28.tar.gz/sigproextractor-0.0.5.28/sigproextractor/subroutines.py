...
def export_information(loopResults, mutation_context, output, index, colnames):
    ...
    processSTE.to_csv(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Signatures_STD_Error"+".txt", "\t", float_format='%.2E', index_label=[processes.columns.name]) 
    ...
    exposureSTE.to_csv(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Activities_STD_Error.txt", "\t", float_format='%.2E', index_label=[exposures.columns.name]) 
    
    all_similarities = loopResults[8] 
    all_similarities.to_csv(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Sample_Similatiry.text", sep="\t")
    ...
    signature_stats.to_csv(subdirectory+"/"+mutation_type+"_S"+str(i)+"_"+"Signature_stats.txt", "\t", index_label=[exposures.columns.name]) 
    ...
    if m=="DINUC" or m=="78":        
        plot.plotDBS(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Signatures"+".txt", subdirectory+"/Signature_plot" , "_S"+str(i), "78", True, custom_text_upper=stability_list, custom_text_bottom=total_mutation_list)
    elif m=="INDEL" or m=="83":
        plot.plotID(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Signatures"+".txt", subdirectory+"/Signature_plot" , "_S"+str(i), "94", True, custom_text_upper=stability_list, custom_text_bottom=total_mutation_list)
    else:
        plot.plotSBS(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Signatures"+".txt", subdirectory+"/Signature_plot", "_S"+str(i), m, True, custom_text_upper=stability_list, custom_text_bottom=total_mutation_list)
...
#---------------------------- alternative solution start -------------------------------------------------#
    ...
    new_data=data
    new_data = new_data.set_index("Number of signature")
    x = np.array(new_data)
    ...
    scaledRE = np.zeros(len(x[:,0]))
    for i in range(len(x[:,0])):
        #print(x[i,0])
        if x[i,0]>0.1:
            scaledRE[i] = change_scale(x[i,0], cul = 1, cll = 0.1, nul = 0.5, nll = 0.1)
        elif x[i,0]<0.1:
            scaledRE[i] = change_scale(x[i,0], cul = 0.1, cll = 0.0, nul = 0.1, nll = 0.085)
            
    scaledStability = np.zeros(len(x[:,1]))
    for i in range(len(x[:,1])):
        #print(x[i,1])
        if x[i,1]>0.8:
            scaledStability[i] = change_scale(x[i,1], cul = 1, cll = 0.8, nul = 0.5, nll = 0.2)
        elif x[i,1]<0.8:
            scaledStability[i] = change_scale(x[i,1], cul = 0.8, cll = -1.0, nul = 0.05, nll = 0.001)
    ...
    alternative_solution  = np.argmax(results)+1
    
    #print("\n\nThe selected number of signature is: ",  selected_sigs)
#---------------------------- alternative solution end -------------------------------------------------#
    ...
    try:
        ...
        datanew = data[data[' Process stability']>0.90]
    ...
    #plt.show()
    plt.savefig(output+'/RE_vs_stabiliy_plot.pdf')    
...
def make_final_solution(processAvg, allgenomes, allsigids, layer_directory, m, index, allcolnames, process_std_error = "none", signature_stabilities = " ", signature_total_mutations= " ", signature_stats = "none"):
    ...
    if type(process_std_error) != str:
        ...
        processSTE = processSTE.rename_axis("MutationType", axis="columns")
        processSTE.to_csv(layer_directory+"/"+solution_type+"_"+"Signatures_STD_Error.txt", "\t", float_format='%.2E', index_label=[processes.columns.name]) 
    
    if type(signature_stats)!=str:
        signature_stats = signature_stats.set_index(allsigids)
        signature_stats = signature_stats.rename_axis("Signatures", axis="columns")
        signature_stats.to_csv(layer_directory+"/"+solution_type+"_"+"Signature_stats.txt", "\t", index_label=[exposures.columns.name]) 
    ...
    try:
        clusters = dendrogram(exposureAvg.T, 0.05, layer_directory)
