...
def export_information(loopResults, mutation_context, output, index, colnames):
    ...
    fh = open(output+"/All_solution_stat.csv", "a") 
    print ('The reconstruction error is {} and the process stability is {} for {} signatures\n\n'.format(reconstruction_error, round(processStabityAvg,4), i))
    fh.write('{}, {}, {}\n'.format(i, round(processStabityAvg, 3), round(reconstruction_error,3)))
    fh.close()
    ...
    if m=="DINUC" or m=="78":        
        plot.plotDBS(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Signatures"+".txt", subdirectory+"/Signature_plot" , "S"+str(i), "78", True, custom_text_upper=stability_list, custom_text_bottom=total_mutation_list)
    elif m=="INDEL" or m=="83":
        plot.plotID(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Signatures"+".txt", subdirectory+"/Signature_plot" , "S"+str(i), "94", True, custom_text_upper=stability_list, custom_text_bottom=total_mutation_list)
    else:
        plot.plotSBS(subdirectory+"/"+mutation_type+"_S"+str(i)+"_Signatures"+".txt", subdirectory+"/Signature_plot", "S"+str(i), m, True, custom_text_upper=stability_list, custom_text_bottom=total_mutation_list)
...
#---------------------------- alternative solution end -------------------------------------------------#
    ...
    ax1.set_ylabel('Frobenius Percentage Error', color=color)
    ax1.set_title(title)
    ...
    data = data.assign(**{'Median Sample L1%': median_l1, 'Maximun Sample L1:': maximum_l1, 'Median Sample L2%': median_l2, 'Maximum Sample L2%': maximum_l2, 'Median Sample KL%': median_kl, 'Maximum Sample KL%': maximum_kl})  
