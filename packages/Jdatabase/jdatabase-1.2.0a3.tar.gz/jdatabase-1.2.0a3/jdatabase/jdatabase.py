...
class Jdatabase:
    ...
    def __init__(self, **kwargs):
        ...
        """
        self.__conf = kwargs
        # whether or not to track the database's tables created through the jdatabase object, defaults False to not
        self.__conf["track"] = kwargs.get("track", False)
        # log the database changes using the logger module, defaults to False to not
        self.__conf["log"] = kwargs.get("log", False)
        # the log level to log if log is set to True, defaults to info level
