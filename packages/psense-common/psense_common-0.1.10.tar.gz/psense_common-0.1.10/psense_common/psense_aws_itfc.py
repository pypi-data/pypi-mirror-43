...
class PSenseAWSInterface(object):
    ...
    def add_experiment(self, device_version='unknown', sample_period=30):
        ...
        if len(response['Items']) > 0:
            self.logm(
                'error', 'add_experiment() failed. [{}] already exists.'.format(expid))
            return False, 'exists'
        else:
            item = {
                'experiment':   expid,
            ...
            try:
                ...
                self.logm(
                    'error', 'add_experiment() failed. [{}] could not be added.'.format(expid))
                return False, 'error'

        if self.debug:
            self.logm(
                'info', 'add_experiment() success. [{}] added to dynamodb'.format(expid))
        return True, ''

    def add_sensordata(self, timestamp=[], vout=[0.5], signal=[0]):
        ...
        if vout is None:
            vout = self.vset.get(
                self.experiment_info['sensor']['type'], self.vset['default'])
