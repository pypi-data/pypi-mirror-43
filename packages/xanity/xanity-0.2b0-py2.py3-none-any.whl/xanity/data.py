...
def scan_data(experiment=False, print_to_std=False, sort='composite_score',
    ...
    for i, d in enumerate(rundirs):
        ...
        if not '-debug' in name:
            score += 1
            score += np.clip(len(c1) - 3, 0, None)  # three files are created in non-debugging
        else:
            score += len(c1)

        # point for  log and another if it's big
        if DIRNAMES.ROOT_LOG in c1:
            score += 1
            if os.stat(os.path.join(d, DIRNAMES.ROOT_LOG)).st_size > 1000:
                score += 1

        if 'xanity_metadata.dill' in c1:
            score += 1

        for item in c1:
            if os.path.isdir(item):
                # examine individual exp directories
                d = os.path.join(d, item)
                score += score_exp_dir(d)
    ...
    if print_to_std:
        ...
        for i, index in enumerate(ranks):
            if headings and headings > 0 and not i % headings:
                ...
                print("########################################################################")
                #         2019-02-28-180646-debug    -1.09    -2.69    -4.25      3.23
            if not normalize:
                print("   {:23s}    {:5.2f}    {:8s}    {:5.2f}    {:5.2f}".format(
                    names[index], scores[index], sizeof_fmt(sizes[index]), logsizes[index], composite_scores[index]
                ))
            else:
                print("   {:23s}    {:5.2f}    {:5.1f} %    {:5.1f} %    {:5.1f} %".format(
                    names[index], scores[index], sizes[index], logsizes[index], composite_scores[index]
                ))

    return np.array(rundirs)[ranks], scores[ranks], sizes[ranks], logsizes[ranks]


def summarize_data(log=True, top=5, cols=2):
    root = common.find_xanity_root()
    rundirs, scores, sizes, logsizes = scan_data(sort='size', order='increasing', normalize=False)
    ...
    print('\n\nBy experiment:')
    print('\n'.join(['    {}: {}'.format(exp, len(os.listdir(os.path.join(root, 'data', 'runs', exp)))) for exp in exps]))
...
def prune(threshold=0.001, metric='size', unit='fraction', print_sizes=False, human_readable=False, full_path=False,
          broken_links=False, full_path_only=False, delete=False, count_only=False):
    ...
    rundirs, scores, sizes, logsizes = scan_data(sort='size', order='ascending', normalize=False, include_saved=False)
    ...
    if delete:
        try:
            confirm('you are about to delete {} runs worth of precious data. are you sure?'.format(del_upto), resp=False)
        except KeyboardInterrupt:
            print('\n')
            raise SystemExit(1)
...
def fetch_timer_data(experiment=None):
...
def list_xanity_variables(experiment=None, path=None):
...
def fetch_variable(experiment, variable, most_recent=False, verbose=False):
    ...
    return pd.DataFrame(results).set_index('UID', drop=True) if results else pd.DataFrame()


def load_data(experiments=None, variables=None, reduce=None, most_recent=False):
    """
    ...
    if not experiments and not variables:
        try:
            confirm('you\'re about to load all data from all experiments... this could be bad news for your system memory.')
        except KeyboardInterrupt:
            print('\n')
            raise SystemExit
...
def score_entry(args=None):
    ...
    scan_data(print_to_std=True, sort=sort, order=order, top=kn.top, headings=kn.headings)
    raise SystemExit
