...
def load_ens_fcsts(issued_dates, fhrs, members, file_template, data_type, geogrid,
                   ...
                   debug=False, accum_over_fhr=False, nc_var=None):
    """
    ...
    if data_type in ('grib1', 'grib2'):
        ...
        for d, date in enumerate(issued_dates):
            ...
            for m, member in enumerate(members):
                ...
                for f, fhr in enumerate(fhrs):
                    kwargs = {'yyyy':yyyy,
                     'mm':mm,  'dd':dd,  'cc':cc,
                     'cycle':f'''{cc}z''',  'cycle_num':cc,  'fhr':fhr,
                     'member':member}
                ...
                if unit_conversion:
                    if unit_conversion == 'prate-to-mm':
                        if len(fhrs) < 2:
                            raise ValueError(f'''Cannot apply a unit conversion of {unit_conversion} with only a single fhr''')
                        else:
        ...
        if data_type == 'netcdf':
            for issued_date in issued_dates:
                ...
                kwargs = {'yyyy':yyyy,
                 'mm':mm,  'dd':dd,  'cc':cc,  'cycle':f'''{cc}z''',  'cycle_num':cc}
                file = jinja2.Template(os.path.expandvars(file_template)).render(**kwargs)
...
def load_dtrm_fcsts(issued_dates, fhrs, file_template, data_type, geogrid, fhr_stat='mean', yrev=False, grib_var=None, grib_level=None, remove_dup_grib_fhrs=False, unit_conversion=None, log=False, transform=None, debug=False):
    """
    ...
    for d, date in enumerate(issued_dates):
        ...
        for f, fhr in enumerate(fhrs):
            ...
            if data_type in ('grib1', 'grib2'):
                try:
                    data_f[f] = read_grib(file, data_type, grib_var, grib_level, geogrid, yrev=yrev, debug=debug)
                except ReadingError:
                    data_f[f] = np.full(geogrid.num_y * geogrid.num_x, np.nan)
                    dataset.dates_with_files_not_loaded.add(date)
                    dataset.files_not_loaded.add(file)
...
def load_obs(valid_dates, file_template, data_type, geogrid, record_num=None, yrev=False, grib_var=None, grib_level=None, unit_conversion=None, log=False, transform=None, debug=False):
    """
    ...
    for d, date in enumerate(valid_dates):
        ...
        if data_type in ('grib1', 'grib2'):
            try:
                dataset.obs[d] = read_grib(file, data_type, grib_var, grib_level, geogrid, yrev=yrev,
                  debug=debug)
            except ReadingError:
            ...
            try:
                ...
                if record_num is not None:
                    dataset.obs[d] = tempdata.reshape(num_records, geogrid.num_y * geogrid.num_x)[record_num]
                else:
                    dataset.obs[d] = tempdata.reshape(num_records, geogrid.num_y * geogrid.num_x)
            except:
                dataset.obs[d] = np.full(geogrid.num_y * geogrid.num_x, np.nan)
                dataset.dates_with_files_not_loaded.add(date)
                dataset.files_not_loaded.add(file)
...
def load_climos(valid_days, file_template, geogrid, num_ptiles=None, debug=False):
    ...
    for d, date in enumerate(valid_days):
        ...
        try:
            if num_ptiles is not None:
                dataset.climo[d] = np.fromfile(file, 'float32').reshape(num_ptiles, geogrid.num_y * geogrid.num_x)
            else:
                dataset.climo[d] = np.fromfile(file, 'float32').reshape(geogrid.num_y * geogrid.num_x)
        except:
            if num_ptiles:
