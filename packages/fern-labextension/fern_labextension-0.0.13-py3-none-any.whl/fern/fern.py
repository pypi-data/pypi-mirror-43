...
class Leaf(object):
    ...
    def publish(self, garden, tree, leaves, user_ns):
        ...
        # Monkey patch, imports are already stored in outputs but not in a form
        # to be sent to amie, we add them again so import blocks have kvs and
        # dont fail the hash check
        for k in self.imports:
            try:
                amieLeaf.kvs.add(k, user_ns.get(k).__version__)
            except:
                # sys does not have __version__
                try:
                    v_info = user_ns.get(k).version_info
                    v_string = str(v_info[0]) + "." + str(
                        v_info[1]) + "." + str(v_info[2])
                    amieLeaf.kvs.add(k, v_string)
                except:
                    pass
