...
def build_output_email_invoice(document, output):
    ...
    output[
        "new_filename"
    ] = f"{output['header']['supplier_id']}-{slugify(output['header']['invoice_number'])}"
    try:
...
def move_to_no_po(api_key, document_id, document, output, op, no_po):
    ...
    filename_stem = (
        f"{output['header']['supplier_id']}_{output['header']['invoice_number']}"
    )
...
def build_full_output(api_key, doc_list, po_lines):
    ...
    for doc in doc_list:
        ...
        output = build_output_email_invoice(document, output)
        clean_invoice_number = output["header"]["invoice_number"].replace("/", "")
        clean_invoice_number = clean_invoice_number.replace("\\", "")
        filename = (
            op
            / f"{output['header']['supplier_id']}_{slugify(output['header']['invoice_number'])}.json"
        )  # .with_suffix('.json')
        print(filename)
