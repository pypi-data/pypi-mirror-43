    ...
    def __init__(
        ...
        if pretrained_top:
            weights_path = utils.get_file(
                origin='https://storage.googleapis.com/odtk/weights/advancedeast/east_model_weights_3T736.h5',  # noqa; E501
    ...
    def compute_targets(self, collection):
        ...
        for i, scene in enumerate(collection):
            for ann in scene.annotations:
                ...
                isPositive = (dl > 0.1) & (dl < 0.9) & (dt > 0.1) & (dt < 0.9)  # noqa: E501
                isBorder = ((dl < 0.2) | (dr < 0.2)) & isPositive
                isTail = (dr < 0.2) & isPositive
                isHead = (dl < 0.2) & isPositive
                ...
                y[i, ya[isHead], xa[isHead], 3:7] = np.concatenate([
                    v[:, np.newaxis] for v in [(xtl - xa), (ytl - ya), (xbl - xa), (ybl - ya)]  # noqa: E501
                ], axis=1)[isHead]*4
                y[i, ya[isTail], xa[isTail], 3:7] = np.concatenate([
                    v[:, np.newaxis] for v in [(xtr - xa), (ytr - ya), (xbr - xa), (ybr - ya)]  # noqa: E501
                ], axis=1)[isTail]*4
            return y
