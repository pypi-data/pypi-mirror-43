...
def build_model(backbone):
    ...
    # This no op layer is required for compatibility with pretrained weights.
    rbox_geo_map = layers.Lambda(lambda x: x * 1)(rbox_geo_map)
...
def loss(y_true, y_pred):
    epsilon = K.epsilon()
    # Ignored scores are those on the outer edges of the ground truth
    # box. We are okay with the network predicting the boxes or not.
    scores_ignore = K.equal(y_true[:, :, :, 0], -1)
    # Ignored geo losses are those where the box is empty / ignore AND the
    # network does not predict a box there. If the network predicts a box,
    # the geometry must be correct.
    geo_ignore = K.less_equal(y_true[:, :, :, 0], 0) & K.less_equal(y_pred[:, :, :, 0], 0.5)  # noqa: E501

    # Compute score loss
    st, sp = [K.switch(scores_ignore, K.zeros_like(y[:, :, :, 0]), y[:, :, :, 0]) for y in [y_true, y_pred]]  # noqa: E501

    beta = 1 - K.mean(st)
    score_loss = -(beta*st*K.log(sp + epsilon) + (1-beta)*(1-st)*K.log(1. - sp + epsilon))  # noqa: E501
    ...
    intersection = wi*hi + epsilon
    union = ((d2p + d4p)*(d1p + d3p) + (d2t + d4t)*(d1t + d3t))[:, :, :, 0] - intersection + epsilon  # noqa: E501
    laabb = K.switch(geo_ignore, K.zeros_like(intersection), -K.log(intersection / union))  # noqa: E501
    ltheta = K.abs(K.switch(geo_ignore, K.zeros_like(thetap), 1 - K.cos(thetap - thetat)))  # noqa: E501
