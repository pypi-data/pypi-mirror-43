    ...
    def request(self, method, url, headers=None, allow_404=False, **kwargs):
        ...
        if '{}{}'.format(USER_ROOT, 'cache/') not in url and \
            ...
            with CacheTagsStore() as cache_tags:
                if any([
                    all([
                        ...
                        response.headers['POZ-USER-CACHE-TAG'] != cache_tags.user_cache_tag
                    ]),
                    all([
                        'POZ-GLOBAL-CACHE-TAG' in response.headers,
                        response.headers['POZ-GLOBAL-CACHE-TAG'] != cache_tags.global_cache_tag
                    ])
                ]):
            ...
            with CacheTagsStore() as cache_tags:
                ...
                if not cache_tags.user_cache_tag == response.headers['POZ-USER-CACHE-TAG']:
                    cache_response = session.post('/cache/', json={
                        'data': cachestore.get('user'),
                        'timestamp': timegm(datetime.utcnow().utctimetuple())
                    })
                    cache_tags.user_cache_tag = cache_response.headers['POZ-USER-CACHE-TAG']
                    cache_tags.global_cache_tag = cache_response.headers['POZ-GLOBAL-CACHE-TAG']
...
@replace_tags
def deploy(script_id, device_id):
    """
...
@replace_tags
def download_firmware(device_id, device_type):
    """
    This will download the custom firmware for the device with the credentials for the
    Pozetron cloud embedded.
    """
    session = _get_session()
    response = session.get('/devices/{}/firmware/?type={}'.format(device_id, device_type), allow_404=True)
    if response.status_code == 404:
