...
from tmg_etl_library.components.config_classes import *
...
from google.cloud.bigquery.job import CopyJobConfig, ExtractJobConfig, LoadJobConfig
...
class BQTOBQ(Transfer):
    ...
    def run(self):
        if self.config.query:
            ...
            google_bq_table_source = self.source.client._get_table_ref(self.source)
            google_bq_table_target = self.target.client._get_table_ref(self.target)
...
class BQTOGS(Transfer):
    ...
    def run(self):
        ...
        table_ref = self.BQTable.client._get_table_ref(self.BQTable)
...
class CSVTOBQ(Transfer):
    ...
    def run(self):
        ...
        google_bq_table = self.BQTable.client._get_table_ref(self.BQTable)
        ...
        with open(self.CSVFile.full_path, 'rb') as csv_file_obj:
            ...
            job_config.schema = self.BQTable.client._create_gbq_schema(self.BQTable.schema)
...
class GSTOBQ(Transfer):
    def __init__(self, log, GSFile, BQTable, config=GSTOBQConfig()):
        super().__init__(log, GSFile, BQTable, config)
        self.id = uuid.uuid4()
        self.GSFile = GSFile
        self.BQTable = BQTable
