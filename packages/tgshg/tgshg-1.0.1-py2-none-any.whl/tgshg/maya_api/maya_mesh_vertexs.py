...
class MeshVertexs(object):  # class A(object): python2 必须显示地继承object
    ...
    class ImportVertexs:
        ...
        def import_vertexs(self):
            self.current_time = self.time_start+self.time_jump
            ...
            def writing_vertexs():
                if (8==adsk.cmds.getModifiers()):  #8 print ' Alt'
                    self.import_stop()
                if (self.current_time > self.time_stop):
                    ...
                    for v in xrange(0, self.total_vertexs):
                        points.append(self.vertexs[self.current_time-self.time_start-self.time_jump][v])
