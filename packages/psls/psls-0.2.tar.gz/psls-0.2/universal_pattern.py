...
def mixed_modes(nnup,nup,dnu,DPI,q,epsg=0.,verbose=False):
   ...
   ''' 
    ...
    def mixed_mode_period_find(nup,dnu,q,DPI,epsg,nu1,nu2,eps=1e-5):
        P = 1e6/nu2
        P1 = 1e6/nu1
        roots = []
        ...
        for l in range(0,Nbr_l):              # Loop angular degree l
            ...
            if ( (l ==1) & (DPI >0.)  ):
                ...
                for nu in num: # Loop over the mixed-modes
                    ...
                    h = ssilib.gaussenvelop(nu,Ampli,numax,width)
                ...
                if( (rot_f > 0.) & (l>0) ): # &  (rot_f > gamma)):
                    ...
                    for m in range(-l, l+1) : 
                        ...
                        G=np.append(G,gamma)
                        N=np.append(N,n)
                        H=np.append(H,V[l]*h*pr[m+l]*ssilib.gaussenvelop(nu,Ampli,numax,width))
                else:
                '''
                F=np.append(F,nu)
                L=np.append(L,l)
...
def universal_pattern(delta_nu, numax, Ampli, f, nyquist, norders = None, verbose = False, teff=4800., \
                      ...
                      beta = 0. , alpha = -1., h_threshold = 1e-2 , width = -1. , epsg = 0. , V1 = -1. ):
