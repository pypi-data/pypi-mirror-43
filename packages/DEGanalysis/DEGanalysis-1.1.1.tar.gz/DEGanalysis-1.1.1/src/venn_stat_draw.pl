...
if ($Sample_room_with_special) {
	...
	my $cmd1="perl $Bin/veen_statistic.pl -f $all_sample_file -od $od >$od/veen_statistic.out 2>$od/veen_statistic.err";
	`$cmd1`;
	if($sample_num_temp <= 5){
		my $cmd2="perl $Bin/drawVeen.pl -in $od/list.txt -od $od -p venn >$od/veen_draw.out 2>$od/veen_draw.err";
		`$cmd2`;
	}
	if (-s "$od/stat.txt") {
	...
	my $cmd1="perl $Bin/veen_statistic.pl -f $all_sample_file -od $od $real_head >$od/veen_statistic.out 2>$od/veen_statistic.err";
	`$cmd1`;
	if($sample_num_temp<=5){
		my $cmd2="perl $Bin/drawVeen.pl -in $od/list.txt -od $od -p venn >$od/veen_draw.out 2>$od/veen_draw.err";
		`$cmd2`;
