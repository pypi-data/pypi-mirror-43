...
def do_export(args):
    ...
    try:
        ...
        logging.info('Exporting to local directory: {}'.format(args.directory))
    except Exception:
    ...
    for node in tqdm(category_tree, desc='Exporting node tree to {}'.format(args.directory)):
        ...
        file_name = node.val.name.strip() \
            ...
            .replace(":", "_") \
            .replace("?", "")
        write_json_file(args.directory + node.path + '/' + file_name + '.json', data)
