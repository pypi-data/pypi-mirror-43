...
def preorder_traversal_dir(directory, root_node):
    ...
    while len(stack) > 0:
        ...
        if type(top_node.val) is Category:
            dir_list = os.listdir(os.path.join(os.path.abspath(directory), top_node.path))

            for item in dir_list:
                if not item.endswith('.json'):
                    continue

                child_data = read_json_file(os.path.join(os.path.abspath(directory), top_node.path, item))
...
def get_directory_tree(directory):
    ...
    root_node.path = os.path.abspath(directory)
...
def write_json_file(file_name, data):
    with open(os.path.normpath(file_name), 'w') as outfile:
        logger.debug('>Writing file %s', os.path.normpath(file_name))
