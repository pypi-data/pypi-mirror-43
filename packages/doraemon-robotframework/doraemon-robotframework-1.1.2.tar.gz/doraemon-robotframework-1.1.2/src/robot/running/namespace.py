    ...
    def handle_imports(self):
        try:
            ...
            if env is None:
                ...
                env = 'dev'
            exec_dir = exec_dir.split('/tests')[0]
            built_in.set_global_variable("${EXECDIR}", exec_dir)

            is_doraemon_project = os.path.exists(
                exec_dir + '/config/doraemon.yaml')
            if is_doraemon_project is True:
                env_variables = '%s/config/%s.env.yaml' % (exec_dir, env)
                self.import_variables(env_variables, None)
            else:
                is_deprecated_doraemon_project = os.path.exists(
                exec_dir + '/config/doraemon.py')
                if is_deprecated_doraemon_project is False:
                    raise DataError('不是Doraemon结构项目，导入env环境文件失败！')
                deprecated_env_resource = '%s/config/%s.env.robot' % (exec_dir, env)
                self.import_resource(deprecated_env_resource)
