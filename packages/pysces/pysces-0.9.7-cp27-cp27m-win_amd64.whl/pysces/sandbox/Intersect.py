...
class Intersect:
    ...
    def D2_setup_args(self,min1,max1,step1,min2,max2,step2,\
                        input,output,raw_data_file,surfaces,isect_data_file,\
                        isect_tol=1.0e-2,filename=None):
        self._min1 = min1
        self._max1 = max1
        self._step1 = step1
        self._min2 = min2
    ...
    def D2_run(self):
        #try:
        self.D2_setup_scan(self._min1,self._max1,self._step1,self._min2,self._max2,self._step2)
        #self.load_raw_data('p4_raw.dat')
        self.D2_generate_raw_data(self._input,self._output,self._raw_data_file)
        self.D2_calculate_intersection_data(self._surfaces,self._isect_data_file,self._isect_tol)
    ...
    def D2_generate_raw_data(self,input,output,raw_data_file):
        ...
        for P1 in range(len(self.P1range)):
            ...
            for P2 in range(len(self.P2range)):
                ...
                self.mod.doState()
                outArr = [self.P1range[P1],self.P2range[P2]]+[getattr(self.mod, oP) for oP in output]
                self.data_raw[iDx,:] = outArr
                data_dump[P2,:] = outArr
                iDx += 1
            ##  raw_file = scipy.io.write_array(raw_file, data_dump, keep_open=1)
            raw_file = scipy.io.write_array(os.path.join(self.data_dir,raw_data_file), data_dump, keep_open=1)
            del data_dump
            if flush >= 5:
                raw_file.flush()
                flush = 0
    ...
    def D2_calculate_intersection_data(self,surfaces,isect_data_file,isect_tol=1.0e-2):
        ...
        for row in range(self.data_raw.shape[0]):
            if abs(self.data_raw[row,surfidx[0]] - self.data_raw[row,surfidx[1]]) < isect_tol:
...
class IntersectD3(Intersect):
    ...
    def D3_setup_args(self,min1,max1,step1,min2,max2,step2,min3,max3,step3,\
                        input,output,raw_data_file,surfaces,isect_data_file,\
                        isect_tol=1.0e-2,filename=None):
        self._min1 = min1
        self._max1 = max1
    ...
    def D3_run(self):
        #try:
        self.D3_setup_scan(self._min1,self._max1,self._step1,self._min2,self._max2,self._step2,\
                            self._min3,self._max3,self._step3)
        #self.load_raw_data('p4_raw.dat')
        self.D3_generate_raw_data(self._input,self._output,self._raw_data_file)
        self.D3_calculate_intersection_data(self._surfaces,self._isect_data_file,self._isect_tol)
        self.output_intersection_metrics(self._filename)
        #except Exception, EXC:
        #    print EXC
        #    sleep(5)
    ...
    def D3_generate_raw_data(self,input,output,raw_data_file):
        ...
        self.inputP = input
        self.outputP = output
        self.data_raw = scipy.zeros((self.P1steps*self.P2steps*self.P3steps,len(output)+3), self.dtype)
        ...
        for P1 in range(len(self.P1range)):
            ...
            for P2 in range(len(self.P2range)):
                setattr(self.mod, input[1], self.P2range[P2])
                data_dump = scipy.zeros((len(self.P3range),len(output)+3), self.dtype)
                flush += 1
                for P3 in range(len(self.P3range)):
                    setattr(self.mod, input[2], self.P3range[P3])
                    self.mod.doState()
                    outArr = [self.P1range[P1],self.P2range[P2],self.P3range[P3]]+[getattr(self.mod, oP) for oP in output]
                    self.data_raw[iDx,:] = outArr
                    data_dump[P3,:] = outArr
                    iDx += 1
                    SflushCount += 1
                    if SflushCount > self.data_raw.shape[0]/50:
                        self.STDOUT.write('\n\n**************\n') 
                        self.STDOUT.write('%3i' % (float(iDx)/float(self.data_raw.shape[0])*100)+'% complete.\n')
                        self.STDOUT.write('**************\n\n') 
                        self.STDOUT.flush()
                        SflushCount = 0
    ...
    def D3_calculate_intersection_data(self,surfaces,isect_data_file,isect_tol=1.0e-2):
        self.isect_tol = isect_tol
        assert len(surfaces) == 2
        self.surfP = surfaces
