    ...
    def doRateChar(self, fixed, summary=0, solver=0):
        ...
        if summary:
            ...
            for reaction in  getattr(mymap, fixed).isReagentOf():
                for step in getattr(mymap, fixed).isReagentOf()+getattr(mymap, fixed).isModifierOf():
                    of.writelines('\\prc{'+step+'}{'+reaction+'}{'+fixed+'} = '+str(basemod.ec.get(step,fixed)*mod.cc.get(reaction,step))+'='+ str(basemod.ec.get(step,fixed)) +'*'+ str(mod.cc.get(reaction,step))+'\n')
            # steady-state values
...
class RateCharData:
    ...
    def _wrn(self):
        warnings.warn('This is a legacy function, use figure and it\'s  functions instead',stacklevel = 2)
...
class RcFigure:
    ...
    def __init__(self,parent,size = (12.80,10.24),outformat = 'png',legendloc = 3, lims = 0):
        ioff()
    ...
    def set_type_visible(self,line_set_name, visibility, affzero=0):
        ...
        if 'partial' in line_set_name:
            ...
            if values[line_set_name]: values[line_set_name][0].set_visible(visibility)
    ...
    def plotRateCharIndiv(self, file=0):
        ...
        for line in self.huedict.keys():
            ...
            filename = os.path.join(self.parent.parent._psc_out_dir, 
                                    self.parent.fix,
                                    self.parent.modelfilename[:-4]+'_'+self.parent.fix+'_flux_'+ line +'_')             
    ...
    def plotElasRCIndiv(self, file=0):
        ...
        for line in self.huedict.keys():
            ...
            filename = os.path.join(self.parent.parent._psc_out_dir, 
                                    self.parent.fix,
                                    self.parent.modelfilename[:-4]+'_'+self.parent.fix+'_ElasRC_'+ line +'_')             
    ...
    def plotPartialRCIndiv(self, file=0):
        ...
        for line in self.huedict.keys():
            ...
            filename = os.path.join(self.parent.parent._psc_out_dir, 
                                    self.parent.fix,
                                    self.parent.modelfilename[:-4]+'_'+self.parent.fix+'_PartialRC_'+ line +'_')             
