...
class Request:
    ...
    def __init__(self, apikey=None, model_id=None):
        ...
        ----------
        apikey : str
            The API key for this specific request
        model_id : str
            The model identifier that needs to be target for this specific request.

        Raises
        ------
        AttributeError
            If no apikey or model_id is set.
        """
        self._baseapi = "https://models.overture.ai"
        self._apikey = apikey
    ...
    def _health_url(self):
        return self._baseapi + '/health/{}'.format(self._model_id)

    def predict(self, image_filename):
        pass

    def predict_batch(self, images_filenames):
        pass
...
class ImageClassification(Request):
    """An ImageClassification object can be used to analyse and classify given images using your custom-built neural
    network """

    def _url(self):
        return self._baseapi + "/predict/image/classification"

    def predict_batch(self, images_filenames):

        self._validate()

        if not isinstance(images_filenames, (list,)):
        ...
        r = requests.post(self._url(), headers=self._get_header(), json={
            ...
            'images': self._filenames_to_payload(images_filenames),
        })
    ...
    def predict(self, image_filename):
        ...
        r = requests.post(self._url(), headers=self._get_header(), json={
            'model_id': str(self._model_id),
            'images': self._filenames_to_payload([image_filename])
...
class FaceRecognition(Request):
    """A FaceRecognition object can be used to analyse and classify given images to detect persons using your
    custom-built neural network """

    def _url(self):
        return self._baseapi + "/predict/image/facerecognition"
...
class ObjectDetection(Request):
    """An ObjectDetection object can be used to analyse and detect objects on images using your
        custom-built neural network """

    def _url(self):
        return self._baseapi + "/predict/image/objectdetection"
