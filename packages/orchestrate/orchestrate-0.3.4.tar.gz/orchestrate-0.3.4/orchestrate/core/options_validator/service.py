...
class OptionsValidatorService(Service):
  def validate_orchestrate_options(
    ...
    assert repository and is_string(repository), safe_format(
      'repository is not non-empty a string: {}',
      repository
    )

    if affinity:
      assert is_mapping(affinity), safe_format(
        "affinity is not a mapping: {}",
        affinity
      )

    if tolerations:
      assert is_sequence(tolerations), safe_format(
        "tolerations is not a sequence: {}",
        tolerations
      )
