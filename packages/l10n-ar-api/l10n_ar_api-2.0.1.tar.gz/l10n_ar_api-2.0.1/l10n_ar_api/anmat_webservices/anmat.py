...
class Anmat(object):
    ...
    def conectar(self, wsdl=None, prueba=False):
        ...
        try:
            self.InstallDir = self.get_install_dir()
            ...
            url = urlparse(wsdl)
            location = LOCATION.get('location_homologation').replace("localhost", url.hostname) if prueba else LOCATION.get('location_production').replace("localhost", url.hostname)
    ...
    def informar_producto(self, datos_de_envio):
        ...
        """
        res = self.client.service.informarProducto(transacciones=datos_de_envio, usuario=self.usuario, password=self.password)
    ...
    def cancelar_transaccion(self, codigo_transaccion):
        ...
        """
        res = self.client.service.sendCancelacTransacc(transaccion=codigo_transaccion, usuario=self.usuario, password=self.password)
        self.CodigoTransaccion = res.codigoTransaccion
        self._analizar_errores(res)
        return True
    ...
    def cancelar_transaccion_parcial(self, codigo_transaccion, producto_data):
        ...
        """
        res = self.client.service.sendCancelacTransaccParcial(transaccion=codigo_transaccion, usuario=self.usuario, password=self.password, gtin=producto_data.get('gtin'), serie=producto_data.get('nro_serie'))
        self.CodigoTransaccion = res.codigoTransaccion
        self._analizar_errores(res)
        return True
