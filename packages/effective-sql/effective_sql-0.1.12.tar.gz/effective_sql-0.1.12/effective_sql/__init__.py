...
class MySQL:
    ...
    def _conn(self, connect_timeout=60):
        logging.info('CREATE DB(%s) CONNECTION .' % self.db)
        ...
        logging.debug('SET AUTOCOMMIT FALSE;')
        self.conn.autocommit(False)
        self.cursor = self.conn.cursor(self.cursor_type)
        for setting in self.settings:
            self.cursor.execute(setting)
            logging.warn(setting.upper())
    ...
    def _reConn(self):
        ...
        while retry_cnt < self.conn_retries:
            try:
                ...
                retry_cnt += 1
                logging.error('%s PING FAILED FOR [ %s ]\n SLEEP FOR %s s .' % (retry_cnt, e.message, self.conn_retry_sleep))
    ...
    def _close(self):
        ...
        if self.conn is not None:
            self.conn.close()
            self.conn = None
        logging.info('CURSOR & CONNECTION IS CLOSED .')
    ...
    def _atomic_action(self, sql, params, detail):
        ...
        while retry_cnt < self.execute_retries:
            try:
                ...
                logging.debug('[%s/%s:(%s->%s)/%s]' % detail + ' %s time success .' % (retry_cnt + 1))
                self.conn.commit()
                error = None
                break
            except Exception, e:
                try:
                    ...
                    logging.error('[%s/%s:(%s->%s)/%s]' % detail + 'rollback fail for %s.' % traceback.format_exc(0))
                result = 0
                retry_cnt += 1
                error = e
                logging.error('[%s/%s:(%s->%s)/%s]' % detail + ' %s time fail for %s.' % ((retry_cnt + 1), traceback.format_exc(0)))
    ...
    def _batch_action(self, sql, params, batch, pojo, optimized=True):
        ...
        logging.debug("EXECUTE ON %s MODE ." % 'YIELD' if optimized else 'MAIN')
        ...
        if self.is_select:
            ...
            logging.debug("FETCH %s ROWS ." % len(result))
            if pojo is not None:
                if isinstance(result[0], dict):
                    result = [pojo(**res) for res in result]
                else:
                    result = [pojo(*res) for res in result]
                logging.debug("WRAPPER %s ROWS ." % len(result))
