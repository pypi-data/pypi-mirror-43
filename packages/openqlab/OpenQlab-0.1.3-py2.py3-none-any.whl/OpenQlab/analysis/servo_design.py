...
def _human_readable_frequency(frequency):
    """
    Return an approximate string representation of a frequency, with
    appropriate units (kHz, MHz, ...) added. Frequencies should be between
    1µHz and 1THz. Accuracy is ~0.1%.

    Parameters
    ----------
    frequency: :obj:`float`
        the frequency to convert into human-readable representation
    ...
    """
    prefixes = ['µ', 'm', '', 'k', 'M', 'G']
    exp = int(np.log10(frequency) // 3)
    if exp < -2:
        exp = -2
    elif exp > 3:
        exp = 3
    return '{value:.4g}{prefix}Hz'.format(
        value=frequency / 10 ** (exp * 3), prefix=prefixes[exp + 2])
...
class Integrator(Filter):
    ...
    def calculate(self):
        ...
        self._description = 'Int {0}'.format(_human_readable_frequency(self.cF))
...
class Differentiator(Filter):
    ...
    def calculate(self):
        ...
        self._description = 'Diff {0}'.format(_human_readable_frequency(self.cF))
...
class Lowpass(Filter):
    ...
    def calculate(self):
        ...
        self._description = 'LP2 {0}, Q={1:.4g}'.format(_human_readable_frequency(self.cF), self.Q)
...
class Notch(Filter):
    ...
    def calculate(self):
        ...
        self._description = 'Notch {0}, Q={1:.4g}'.format(_human_readable_frequency(cF), Q)
