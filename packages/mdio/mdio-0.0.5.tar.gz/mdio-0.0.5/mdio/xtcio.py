...
def read_frame(up, selection=None):
    ...
    if maxatoms <= 9:
        ...
        if selection is not None:
            frame = Frame(crds[selection], box=box, time=time, timestep=timestep)
        else: 
    ...
    if selection is None:
        frame = Frame(crds[:natoms] * inv_precision, box=box, time=time, timestep=timestep, precision=precision)
    else:
        frame = Frame(crds[selection] * inv_precision, box=box, time=time, timestep=timestep, precision=precision)
    return frame
