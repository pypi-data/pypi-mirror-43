...
def imread_resize(img, size=(1024, 1024)):
    ...
    return img
def plot_respose():
    response_mat = np.load('/home/data/Projects/pix2pix/data/raw/response_CANNON.npy')
    x = np.array(range(400,710,10))
    pl.plot(x,response_mat,lw=2,alpha=1)
    pl.title('Canon 600D response function')
    pl.xlabel('Wavelength (nm)')
    pl.savefig('respose.png',ppi=200)
    pl.show()
