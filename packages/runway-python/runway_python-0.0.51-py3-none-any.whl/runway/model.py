...
class RunwayModel(object):
    ...
    def define_routes(self):
        ...
        def manifest():
            return json.dumps(dict(
                ...
                commands=[serialize_command(cmd)
        ...
        def command_route(command_name):
            try:
                ...
                for inp in inputs:
                    ...
                    deserialized_inputs.append(inp.deserialize(
                        input_dict[name]) or getattr(inp, 'default', None))
                try:
    ...
    def command(self, name, inputs=[], outputs=[]):
        if len(inputs) == 0 or len(outputs) == 0:
            raise Exception(
                'You need to provide at least one input and output for the command')
    ...
    def run(self):
        ...
        parser.add_argument('--debug', action='store_true', default=os.getenv(
            'RW_DEBUG', '0') == '1', help='Activate debug mode (live reload)')
        parser.add_argument('--meta', default=os.getenv(
            'RW_META', '0') == '1', action='store_true',
