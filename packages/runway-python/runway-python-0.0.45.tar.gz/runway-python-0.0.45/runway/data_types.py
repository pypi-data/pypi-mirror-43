...
class category(object):
    def __init__(self, name=None, choices=None, default=None):
        if name is None: raise MissingArgumentError('name')
        if choices is None: raise MissingArgumentError('choices')
        if default is None: raise MissingArgumentError('default')
        self.name = name
...
class number(object):
    def __init__(self, name=None, default=None, min=None, max=None, step=None):
        if name is None: raise MissingArgumentError('name')
        if default is None: raise MissingArgumentError('default')
        if min is None: raise MissingArgumentError('min')
        if max is None: raise MissingArgumentError('max')
        if step is None: raise MissingArgumentError('step')
        self.name = name
        self.default = default
        self.min = min
        self.max = max
