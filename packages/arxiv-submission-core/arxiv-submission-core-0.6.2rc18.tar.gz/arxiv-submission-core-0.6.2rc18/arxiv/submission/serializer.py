...
# def fromisoformat(raw: str) -> datetime:
#     if '+' in raw:
#         sep = '+'
#     elif '-' in raw:
#         sep = '-'
#     else:
#         raise ValueError('Not localized: %s' % raw)
#     date_part, tz_part = raw.split(sep, 1)
#     tz_hours, tz_minutes = tz_part.split(':', 1)
#     tzinfo = tz.tzoffset('TZ', int(tz_hours) * 3600 + int(tz_minutes) * 60)
#     return datetime.strptime(date_part, '%Y-%m-%dT%H:%M:%S.%f').replace(tzinfo=tzinfo)
