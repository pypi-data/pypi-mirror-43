...
class Updater:
    ...
    def update(self):
        ...
        if ((self.targetModelNumber.value >> 8) & 0xFF) == 0x10:
            header.to_ = DeviceType.Drone
            print("* DeviceType.Drone\n")
        else:
            header.to_ = DeviceType.Controller
            print("* DeviceType.Controller\n")
        ...
        for fw in firmware:
            ...
            if self.targetModelNumber == fw.header.modelNumber:
                ...
                print("* {0}\n".format(self.targetModelNumber))

                if self.modeUpdate == ModeUpdate.Ready or self.modeUpdate == ModeUpdate.Update:

                    print("* {0}\n".format(self.modeUpdate))

                    while flagRun:

                        now = time.time() * 1000

                        if timeTransferNext < now:

                            if self.indexBlockNext == 0:
                                timeTransferNext = now + 1200
                            else:
                                timeTransferNext = now + 20
