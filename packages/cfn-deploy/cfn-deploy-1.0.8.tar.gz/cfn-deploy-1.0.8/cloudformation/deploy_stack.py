...
def deploy(stack_name, template, parameters=None, capabilities=None, config=None):  # nopep8
    ...
    try:
        ...
        if _stack_exists(cf_client, stack_name):
            ...
            timestamp = stack['LastUpdatedTime']
        else:
        ...
        _wait(cf_client, r53_client, stack_name, timestamp)
...
def _wait(cf_client, r53_client, stack_name, timestamp):
...
def _register_certificates(cf_client, r53_client, stack_name, cnames, timestamp):  # nopep8
    ...
    for stack in stacks:
        ...
        event_filter = 'StackEvents[?ResourceType == `AWS::CertificateManager::Certificate` && Timestamp >= `{}`]'.format(timestamp)  # nopep8
