...
def approximate_PageRank_weighted_seed(G,
                                  ref_nodes,
                                  ref_vals,
    ...
    (length,xids,values) = aclpagerank_weighted_seed_cpp(n,G.ai,G.aj,G.adjacency_matrix.data,alpha,rho,
                ref_nodes,len(ref_nodes),ref_vals,iterations)
    p = np.zeros(n)
    p[xids] = values
