...
# Load the functions
def _setup_aclpagerank_weighted_seed_args(vtypestr, itypestr, fun):
    ...
    fun.argtypes=[ctypes_vtype,ndpointer(ctypes_itype, flags="C_CONTIGUOUS"),
                  ...
                  ndpointer(ctypes_vtype, flags="C_CONTIGUOUS"),
                  ctypes_vtype,ndpointer(float_type, flags="C_CONTIGUOUS"),ctypes_vtype,
                  ndpointer(ctypes_vtype, flags="C_CONTIGUOUS"),
                  ctypes_vtype,ndpointer(ctypes.c_double, flags="C_CONTIGUOUS")]

    return fun

_graphlib_funs_aclpagerank_weighted_seed64 = _setup_aclpagerank_weighted_seed_args(
    'int64','int64', _graphlib.aclpagerank_weighted_seed64)
_graphlib_funs_aclpagerank_weighted_seed32 = _setup_aclpagerank_weighted_seed_args(
    'uint32','uint32', _graphlib.aclpagerank_weighted_seed32)
_graphlib_funs_aclpagerank_weighted_seed32_64 = _setup_aclpagerank_weighted_seed_args(
    'uint32','int64', _graphlib.aclpagerank_weighted_seed32_64)

def _get_aclpagerank_weighted_seed_cpp_types_fun(ai,aj):
    float_type,vtype,itype,ctypes_vtype,ctypes_itype = determine_types(ai,aj)
    if (vtype, itype) == (np.int64, np.int64):
        fun = _graphlib_funs_aclpagerank_weighted_seed64
    elif (vtype, itype) == (np.uint32, np.int64):
        fun = _graphlib_funs_aclpagerank_weighted_seed32_64
    else:
        fun = _graphlib_funs_aclpagerank_weighted_seed32
...
def aclpagerank_weighted_seed_cpp(n,ai,aj,a,alpha,eps,seedids,nseedids,seedvals,maxsteps,xlength=10**7,flag=0):
    float_type,vtype,itype,ctypes_vtype,ctypes_itype,fun = _get_aclpagerank_weighted_seed_cpp_types_fun(ai,aj)
    seedids=np.array(seedids,dtype=vtype)
    seedvals=np.array(seedvals,dtype=float_type)
    xids=np.zeros(xlength,dtype=vtype)
    values=np.zeros(xlength,dtype=float_type)
    a=np.array(a,dtype=float_type)
    actual_length=fun(n,ai,aj,a,flag,alpha,eps,seedids,nseedids,seedvals,maxsteps,xids,xlength,values)
