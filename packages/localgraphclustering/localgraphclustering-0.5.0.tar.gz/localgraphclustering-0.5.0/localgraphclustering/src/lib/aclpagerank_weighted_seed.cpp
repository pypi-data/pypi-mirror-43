...
#include "include/aclpagerank_weighted_seed_c_interface.h"
...
uint32_t aclpagerank_weighted_seed32(
        ...
        uint32_t* xids, uint32_t xlength, double* values)
{
    ...
    uint32_t actual_length = g.aclpagerank_weighted_seed(alpha, eps, seedids, nseedids, seedvals, maxsteps,
                                                    xids, xlength, values);
    return actual_length;
}

int64_t aclpagerank_weighted_seed64(
        ...
        int64_t* xids, int64_t xlength, double* values)
{
    ...
    int64_t actual_length = g.aclpagerank_weighted_seed(alpha, eps, seedids, nseedids, seedvals, maxsteps,
                                                   xids, xlength, values);
    return actual_length;
}

uint32_t aclpagerank_weighted_seed32_64(
...
vtype graph<vtype,itype>::aclpagerank_weighted_seed(double alpha, double eps, vtype* seedids,
                                               vtype nseedids, double* seedvals, vtype maxsteps, 
                                               vtype* xids, vtype xlength, double* values)
{
    vtype actual_length;
    actual_length=pprgrow_weighted_seed(alpha, eps, seedids, nseedids, seedvals, maxsteps, xids, xlength, values);
...
template<typename vtype, typename itype>
vtype graph<vtype,itype>::pprgrow_weighted_seed(double alpha, double eps, vtype* seedids,
                                           vtype nseedids, double* seedvals, vtype maxsteps, vtype* xids,
                                           vtype xlength, double* values)
{
    ...
    for(size_t i = 0; i < (size_t)nseedids; i ++){
        r_map[seedids[i] - offset] = seedvals[i];
