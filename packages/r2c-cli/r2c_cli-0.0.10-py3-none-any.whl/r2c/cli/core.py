...
def login(org=None):
    ...
    if error:
        click.echo("❌ Error validating token.")
        sys.exit(1)
...
def logout(org=None):
    ...
    try:
        ...
        if org:
            ...
            click.echo("\n✅ logged out of all orgs")
    except Exception as e:
        click.echo(f"❌ Unexpected error. Please contact us")
        logger.exception(e)
        sys.exit(1)
...
def unittest(ctx, analyzer_directory, env_args_string):
    ...
    try:
        ...
        click.echo(
            "❌ Couldn't find an analyzer.json to run tests for. Check that you're in an analyzer directory, or specify a path to your analyzer.json with --analyzer-directory."
        )
        sys.exit(1)
...
def push(ctx, analyzer_directory, env_args_string):
    ...
    try:
        ...
        click.echo(
            "❌ Couldn't find an analyzer.json to push. Check that you're in an analyzer directory, or specify a path to your analyzer.json with --analyzer-directory."
        )
        sys.exit(1)
    ...
    if data.get("status") == "uploaded":
        ...
        click.echo(f"✅ Successfully uploaded analyzer! Visit: {web_url}")
    else:
        click.echo(
...
def run(ctx, analyzer_directory, code, no_login, wait, env_args_string):
    ...
    try:
        ...
        click.echo(
            "❌ Couldn't find an analyzer.json to run. Check that you're in an analyzer directory, or specify a path to your analyzer.json with --analyzer-directory."
        )
        sys.exit(1)
    ...
    if len(dependencies) == 0:
        click.echo(
            "❌ Error parsing analyzer dependencies. Check the dependencies in your analyzer.json and try again."
        )
        sys.exit(1)
    ...
    if not no_login:
        ...
        logger.info(f"Getting credentials from {artifact_link}")
        ...
        if creds is None:
            ...
            sys.exit(1)
        # docker login
        successful_login = docker_login(creds, debug)
        if not successful_login:
            click.echo(
    ...
    try:
        ...
        click.echo(f"❌ Error setting up local analysis. {sym}. Try again as an admin.")
        sys.exit(1)
