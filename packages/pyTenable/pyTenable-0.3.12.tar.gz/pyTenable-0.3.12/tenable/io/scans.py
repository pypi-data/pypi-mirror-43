...
class ScansAPI(TIOEndpoint):
    ...
    def wait(self, scan_id, interval=None):
        '''
        Waits for the specified scan to complete or error.

        Args:
            scan_id (int): The identifier for the scan.
            interval (int, optional): 
                How long to sleep between status checks in seconds.  The default 
                if left unspecified is 60 seconds.
        ...
        '''
        self._check('scan_id', scan_id, int)
        interval = self._check('interval', interval, int, default=60)
        while self.status(scan_id) not in ['completed', 'aborted', 'errored']:
            time.sleep(interval)