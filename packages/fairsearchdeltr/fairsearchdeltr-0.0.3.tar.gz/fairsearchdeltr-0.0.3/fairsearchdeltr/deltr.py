...
class Deltr(object):
    ...
    def __init__(self, protected_feature: int, gamma: float, number_of_iterations=3000, learning_rate=0.001,
        ...
        # keep training max/mins per feature
        self._mins = []
        self._maxs = []
    ...
    def train(self, training_set: pd.DataFrame):
        ...
        for col in range(shape[1]):
            self._mins.append(min(feature_matrix[:,col]))
            self._maxs.append(max(feature_matrix[:, col]))
            feature_matrix[:, col] = (feature_matrix[:, col] - self._mins[col]) / (self._maxs[col] - self._mins[col])
    ...
    def rank(self, prediction_set: pd.DataFrame, has_judgment=False):
        ...
        #TODO: do we needto apply normalization here?
        # shape = feature_matrix.shape
        # for col in range(shape[1]):
        #     feature_matrix[:, col] = (feature_matrix[:, col] - self._mins[col]) / (self._maxs[col] - self._mins[col])
