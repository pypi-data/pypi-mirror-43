...
class ModelWTimestamp( Model ):
    ...
    def __init__(self, env_interface, build_initial_model=False): # Interface (can be sim or env)
        ...
        # add dictionary to track previous state-action pair
        self.previous_state_actionD = {} # index=(s_hash,a_desc), value=set of previous (s_hash,a_desc)
    ...
    def make_pickle_filename(self, fname):
        """Make a file name ending with .bbp_pickle """
        if fname is None:
            fname = self.name.replace(' ','_') + '.bbp_pickle'
            
        else:
            fname = fname.replace(' ','_').replace('.','_') + '.bbp_pickle'
