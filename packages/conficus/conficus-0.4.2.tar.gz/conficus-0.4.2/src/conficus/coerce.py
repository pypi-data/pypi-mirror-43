...
def split_iterable(value):
    # XXX: I don't really like any of this, but so
    # XXX: far it works to alleviate the problem with
    # XXX: strings inside a sequence that contain commas
    def consume_quoted_string(iterator, equals='"'):
        c1 = []
        i, c = next(iterator)
        while c not in equals:
            c1.append(c)
            i, c = next(iterator)
        return ''.join(c1)
    ...
    for i, v in e:
        if v == ",":
            # natural split point
            this = ''.join(collected).strip()
            if this:
                yield this
                collected = []
        elif v == '"':
            this = consume_quoted_string(e)
            print(this)
            yield this
            collected = []
        else:
            collected.append(v)
    last = ''.join(collected).strip()
    if last:
        yield last
