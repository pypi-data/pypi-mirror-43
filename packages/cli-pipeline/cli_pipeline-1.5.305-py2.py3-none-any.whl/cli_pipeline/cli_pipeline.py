...
#kubectl delete -f %s/.generated-virtualservice-mlflow.yaml
...
def cluster_kube_install(tag,
    ...
    rendered = _jinja2.Environment(loader=_jinja2.FileSystemLoader(path)).get_template(filename).render(context)
    rendered_path = _os.path.join(generated_path, '.generated-virtualservice-mlflow.yaml')
    with open(rendered_path, 'wt') as fh:
        fh.write(rendered)
        print("'%s' => '%s'." % (filename, rendered_path))
...
#kubectl create -f %s/.generated-virtualservice-mlflow.yaml
