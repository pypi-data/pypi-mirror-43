...
class GUI():
    ...
    class DownloadWindow(QWidget):
        def __init__(self, beepdrive, folder, courses_dict):
            ...
            self.sboard.hide()
            self.play_button = QPushButton("Play Snake", self)
            self.play_button.setFixedWidth(200)
            self.play_button.move((windowWidth - 200) / 2, 400)
            self.play_button.clicked.connect(self.play_snake)
            self.n_courses = 0
        ...
        def change_label(self, downloading, courses):
            if courses == 0:
                ...
                self.progress.setFormat("Download completed - 100%")
                time.sleep(3)
                self.stop = True
            else:
