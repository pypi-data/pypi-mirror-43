...
class IPSolverParameters:
    ...
    def __init__(self,
        ...
        burst_core_colloc_usage_thresh = 2.0,
...
def optimize_ip(requested_cus, requested_cus_burst_pool, total_available_cus, num_sockets, previous_allocation=None,
        ...
        verbose=False, max_runtime_secs=5, tol_obj=1e-1):
    """
    ...
    CC = [Z[l] <= cp.sum(M[2 * l : 2 * l + 1, :]) for l in range(c)] + [cp.sum(Z) >= min(np.sum(r), c)]
    ...
    if use_per_workload is not None:
        ...
        # Even out CPU usage per core:
        CY1 = [Y[l] >=
                -2 * np.max(weights.T[:uk]) +
                cp.sum(cp.multiply(M[2 * l, :uk], weights.T[:uk])) +
                cp.sum(cp.multiply(M[2 * l + 1, :uk], weights.T[:uk]))
              for l in range(c)]
        CY2 = []

        if requested_cus_burst_pool > 0:
            ## Don't collocate extra burst threads with static one if average CPU usage on core is above relative treshold
            ZZ = cp.Variable(2 * c, boolean=True)
            CB += [ZZ[l] >= M[2 * l + 1, k-1] * solver_params.weight_cpu_use_burst - solver_params.burst_core_colloc_usage_thresh * cp.sum(cp.multiply(M[2 * l, :k-1], weights.T[:k-1])) for l in range(c)]
            CB += [ZZ[l + c] >= M[2 * l, k-1] * solver_params.weight_cpu_use_burst - solver_params.burst_core_colloc_usage_thresh * cp.sum(cp.multiply(M[2 * l + 1, :k-1], weights.T[:k-1])) for l in range(c)]
            CB += [cp.sum(ZZ) <= requested_cus_burst_pool]
            pass
    ...
    try:
        ...
        prob.solve(solver='GLPK_MI', verbose=verbose, **extra_args)
