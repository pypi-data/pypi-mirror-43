...
PREREQUISITE
User and password
To be able to download data you need user and password provided by NASA. Please register at https://urs.earthdata.nasa.gov/users/new;
...
def get_modisfiles(username, password, platform, product, year, tile, proxy,
    ...
    for date in dates:
        ...
        select = np.where(np.char.find(lines,tile) >= 0)
        lines = lines[select]
        select = np.where(np.char.find(lines,".hdf") >= 0)
        lines = lines[select]
...
if __name__ == "__main__":
    username = cf['MODIS']['username']
    password = cf['MODIS']['password']
    platform = "MOLT"
    product_name = "MOD13Q1"
    product = "MOD13Q1.006"
    year = cf['download_year']
    tile = "h17v05"
    out_dir = cf['path'][product_name]

    if username == "":
        print("To be able to download data you need user and password provided by NASA. Please register at https://urs.earthdata.nasa.gov/users/new; ")
        print("now login and move to your profile page.")
        print("If you are an already registered user, login and enable the applications at https://urs.earthdata.nasa.gov/home")
        print("Go to “My application” tab and approve the following applications “LP DAAC Data Pool” and “Earthdata Search”.")
    else:
        get_modisfiles(username, password, platform, product, year, tile, out_dir=out_dir, proxy=None, verbose=True)
