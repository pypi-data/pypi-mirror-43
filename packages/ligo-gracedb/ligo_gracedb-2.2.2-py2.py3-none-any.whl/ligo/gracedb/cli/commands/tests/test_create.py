...
@pytest.mark.parametrize("ra_width,dec_width,duration,comment", EMO_TEST_DATA)
def test_create_emobservation_subcommand(CLI, ra_width, dec_width, duration,
    ...
    # Compile command
    join_s = lambda x: ",".join([str(i) for i in x]) if isinstance(x, list) \
        else x
    ...
    cmd = ('create emobservation {s_id} {group} {ra_list} {ra_width_list} '
        ...
        dec_width_list=join_s(dec_width), duration_list=join_s(duration),
...
@pytest.mark.parametrize("search,labels,offline", EVENT_TEST_DATA)
def test_create_event_subcommand(CLI, search, labels, offline):
    ...
    with mock.patch('ligo.gracedb.rest.GraceDb.searches',
        new_callable=mock.PropertyMock) as mock_searches, \
        mock.patch(func) as mock_cli_func:
...
def test_create_event_legacy_usage(CLI):
    ...
    with mock.patch('ligo.gracedb.rest.GraceDb.searches',
        new_callable=mock.PropertyMock) as mock_searches, \
        mock.patch('ligo.gracedb.cli.commands.create.os.path.isfile') \
        as mock_isfile, mock.patch(func) as mock_cli_func:
...
@pytest.mark.parametrize("filename,tag_name,tag_disp_name", LOG_TEST_DATA)
...
@pytest.mark.parametrize("instrument", SIGNOFF_TEST_DATA)
...
@pytest.mark.parametrize("category,labels,events", SUPEREVENT_TEST_DATA)
...
@pytest.mark.parametrize(",".join(list(VOEVENT_TEST_DICT)), VOEVENT_TEST_DATA)
def test_create_voevent_subcommand(CLI, skymap_type, skymap_filename,
    external, open_alert, hardware_inj, coinc_comment, prob_has_ns,
    prob_has_remnant, bns, nsbh, bbh, terrestrial, mass_gap):
