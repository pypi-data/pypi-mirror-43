...
from ligo.gracedb.cli.commands.subcommands import ShowCommand


###############################################################################
# Test data ###################################################################
###############################################################################
SUBCOMMANDS_TEST_DATA = ["confirm_as_gw", "credentials", "expose", "hide",
    "ping", "show"]
...
def test_credentials_subcommand(CLI):
    """Test credentials subcommand"""
    func = 'ligo.gracedb.rest.GraceDb.show_credentials'
    with mock.patch(func) as mock_cli_func:
        CLI(['credentials'])
    ...
    # Check args used in function call
    assert cli_args == ()
    assert cli_kwargs == {'print_output': False}
...
@pytest.mark.parametrize("cmd,cli_func",
    list(six.iteritems(ShowCommand.options)))
def test_show_subcommand(CLI, cmd, cli_func):
