...
object_id_parser.add_argument("object_id", type=str,
    help="Event graceid or superevent id")

# Graceid parser
graceid_parser = argparse.ArgumentParser(add_help=False)
graceid_parser.add_argument("graceid", type=str,
    help="Event graceid")

# Superevent id parser
superevent_id_parser = argparse.ArgumentParser(add_help=False)
superevent_id_parser.add_argument("superevent_id", type=str,
    help="Superevent id")

# Tag parser
tag_parser = argparse.ArgumentParser(add_help=False)
tag_parser.add_argument("-t", "--tag-name", dest="tag_name",
    type=parse_delimited_string_or_single, default=None,
    help="Tag name or comma-separated list of tag names")
tag_parser.add_argument("-d", "--tag-display-name", dest="tag_display_name",
    type=parse_delimited_string_or_single, default=None,
...
label_parser.add_argument("-l", "--labels", dest="labels",
...
comment_parser.add_argument("comment", type=str, default="",
...
class CustomHelpArgumentParser(argparse.ArgumentParser):
    ...
    def format_subcommand_list(self):
        ...
        header += textwrap.fill("{{{cmds}}}".format(cmds="--".join(
            [sc[0] for sc in self.subcommands])), width=self._width,
        ...
        for cmd_name, cmd in self.subcommands:
            cmd_entry = ''
