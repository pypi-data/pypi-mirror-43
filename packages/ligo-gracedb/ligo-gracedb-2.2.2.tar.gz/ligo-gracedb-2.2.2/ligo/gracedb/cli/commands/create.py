...
delimited_float = lambda x: parse_delimited_string(x, cast=float)
delimited_or_single_float = \
    lambda x: parse_delimited_string_or_single(x, cast=float)
...
class CreateEmobservationCommand(CreateChildBase):
    ...
    def add_custom_arguments(self, parser):
        parser.add_argument("group", type=str,
            ...
            prog=self.base_prog)))
        parser.add_argument("right_ascension", type=delimited_float,
            help=("Comma-separated list of right ascension coordinates "
            "(degrees)"))
        parser.add_argument("right_ascension_width",
            ...
            "(degrees)"))
        parser.add_argument("declination", type=delimited_float,
            help=("Comma-separated list of declination coordinates (degrees)"))
        parser.add_argument("declination_width",
            type=delimited_or_single_float,
            help=("Comma-separated list of declination measurement widths "
            " OR a single number if all measurements have the same width "
            "(degrees)"))
        parser.add_argument("start_time", type=parse_delimited_string,
            help=("Comma-separated list of measurement start times in ISO "
            "8601 format"))
        parser.add_argument("duration", type=delimited_or_single_float,
    ...
    def run(self, client, args):
        return client.writeEMObservation(args.object_id, args.group,
            args.right_ascension, args.right_ascension_width,
            args.declination, args.declination_width, args.start_time,
            args.duration, comment=args.comment)
...
class CreateEventCommand(CreateChildBase):
    ...
    def add_custom_arguments(self, parser):
        ...
        parser.add_argument("--labels", type=parse_delimited_string_or_single,
    ...
    def run(self, client, args):
        ...
        return client.createEvent(args.group, args.pipeline, event_file,
            search=search, offline=args.offline, labels=args.labels)
...
class CreateLogCommand(CreateChildBase):
    ...
    def run(self, client, args):
        return client.writeLog(args.object_id, args.comment,
            filename=args.filename, tag_name=args.tag_name,
...
class CreateSupereventCommand(CreateChildBase):
    ...
    def add_custom_arguments(self, parser):
        ...
        parser.add_argument("--events", type=parse_delimited_string_or_single,
    ...
    def run(self, client, args):
        return client.createSuperevent(args.t_start, args.t_0, args.t_end,
            args.preferred_event, category=args.category, events=args.events,
            labels=args.labels)
