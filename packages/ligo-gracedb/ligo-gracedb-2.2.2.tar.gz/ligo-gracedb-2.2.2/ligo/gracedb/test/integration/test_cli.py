...
def test_voevents():
    ...
    filename = os.path.join(TEST_DATA_DIR, 'test_file.txt')
    cmd = 'create log {gid} "fake skymap" {filename}'.format(gid=gid,
        filename=filename)
    response = run_CLI_test(cmd)
    assert response.status == 201
...
CMDS = ['emgroups', 'groups', 'labels', 'pipelines', 'searches',
    'signoff_statuses', 'signoff_types', 'superevent_categories',
    'voevent_types']
@pytest.mark.parametrize("cmd", CMDS)
def test_show(cmd):
    """Test 'show' command with CLI"""
    response = run_CLI_test('show ' + cmd)

    # Can't really check content of response but we can make sure it splits
    # on commas into a multi-element list
