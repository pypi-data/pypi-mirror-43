...
Updated on: 19th March 09:35 PM
''' % ( __version__)
...
def pinput(prompt='Password: '):
    ...
    if not is_present:
        raise ModuleNotFoundError(
            """getpass module not found.
    Install getpass module using 'pip install getpass'

    * getpass module is used is used to get password input field.""")
...
def validate_ip(ip):
    '''Validates if an IP address is valid or not. It does NOT check if an
    IP address exists in a network or not. It just checks if the given IP address
    is in the correct format or not. 
    
    Takes an argument ip which is the IP address of a string
    Returns a bool'''
    L = ip.split('.')
    if len(L) != 4:
        return False
