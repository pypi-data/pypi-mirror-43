class StandardOutputPrinter:
    def print_results(self, results, main_metric):
        for result in sorted(results, key=lambda x: x["metrics"][main_metric], reverse=True):
    ...
    def __additional_metrics_to_str(result, main_metric):
        ...
        for metric, value in result["metrics"].items():
            if metric != main_metric:
                additional_metrics_str += "{}:{:.4f} |".format(metric, value)
