...
def model(is_training, reuse, input_size=image_size[0], num_classes=2):
    ...
    conv_args = make_args(batch_norm=True, activation=lrelu, w_init=initz.he_normal(scale=1), untie_biases=True, **common_args)
    conv_args_finetune = make_args(batch_norm=True, activation=lrelu, trainable=False, w_init=initz.he_normal(scale=1), untie_biases=True, **common_args)
    fc_args = make_args(activation=lrelu, w_init=initz.he_normal(scale=1), **common_args)
    logit_args = make_args(activation=None, w_init=initz.he_normal(scale=1), **common_args)
    pred_args = make_args(activation=lrelu, w_init=initz.he_normal(scale=1), **common_args)
    ...
    x = conv2d(x, 64, name="conv1_1", **conv_args_finetune)
    x = max_pool(x, name='pool1', **pool_args)
    x = conv2d(x, 64, name="conv1_2", **conv_args_finetune)
    x = conv2d(x, 64, name="conv2_1", **conv_args_finetune)
