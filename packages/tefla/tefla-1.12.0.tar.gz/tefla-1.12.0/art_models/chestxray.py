...
from tefla.core.layers import input, conv2d, fully_connected, max_pool, sigmoid
...
def block_inception_a(inputs, name='block_a', **kwargs):
    """Builds ModelX-A block for ModelX v4 network."""
    # By default use stride=1 and SAME padding
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockModelXA', [inputs], reuse=reuse):
...
def block_reduction_a(inputs, name='reduction_block_a', **kwargs):
    """Builds Reduction-A block for ModelX v4 network."""
    # By default use stride=1 and SAME padding
    reuse = kwargs.get('reuse')
...
def block_inception_b(inputs, name='block_b', **kwargs):
    """Builds ModelX-B block for ModelX v4 network."""
    # By default use stride=1 and SAME padding
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockModelXB', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0 = conv2d(
...
def block_reduction_b(inputs, name='reduction_block_b', **kwargs):
    """Builds Reduction-B block for ModelX v4 network."""
    # By default use stride=1 and SAME padding
    reuse = kwargs.get('reuse')
...
def block_inception_c(inputs, name='block_c', **kwargs):
    """Builds ModelX-C block for ModelX v4 network."""
    # By default use stride=1 and SAME padding
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockModelXC', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0 = conv2d(
                inputs, 256, filter_size=(1, 1), name='Conv2d_0a_1x1', **kwargs)
        with tf.variable_scope('Branch_1'):
...
def inception_v4_base(inputs, name='inception_4', input_size=512, num_block_a=4, num_block_b=7, num_block_c=3, **kwargs):
    """Creates the ModelX V4 network up to the given final endpoint.
    ...
    with tf.variable_scope(name, 'ModelXV4', [inputs]):
        # 448 x 448 x 3
...
def model(inputs, is_training, reuse, input_size=image_size[0], num_block_a=4, num_block_b=5, num_block_c=3, num_classes=14,
    ...
    with tf.variable_scope(name, 'ModelXV4'):
