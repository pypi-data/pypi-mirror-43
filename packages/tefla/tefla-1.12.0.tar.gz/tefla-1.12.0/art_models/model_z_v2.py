...
def block_reduction_0(inputs, name='reduction_block_a', **kwargs):
    ...
    with tf.variable_scope(name, 'BlockReductionA', [inputs], reuse=reuse):
        ...
        with tf.variable_scope('Branch_1'):
            branch_1 = conv2d(
                inputs, 128, filter_size=(1, 1), name='Conv2d_0a_1x1', **kwargs)
            branch_1 = conv2d(branch_1, 128, stride=(
        ...
        outputs = conv2d(outputs, 196, filter_size=(1, 1), name='Conv2d_0b_end_1x1', **kwargs)
        return outputs
...
def modelz_v4_base(inputs, name='modelz', input_size=512, num_block_a=4, num_block_b=7, num_block_c=3, **kwargs):
    ...
    with tf.variable_scope(name, 'modelzV4', [inputs]):
        ...
        # 224 x 224 x 32
        net = conv2d(net, 64, name='Conv2d_2a_1_3x3', **finetune_kwargs)
        net = conv2d(net, 64, name='Conv2d_2a_2_3x3', **finetune_kwargs)
        # 224 x 224 x 32
...
def model(inputs, is_training, reuse, input_size=image_size[0], num_block_a=4, num_block_b=5, num_block_c=3, num_classes=2,
