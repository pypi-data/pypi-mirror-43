...
from tefla.core.layers import input, conv2d, fully_connected, max_pool, softmax, sigmoid
...
def generate_weights(batch_size, num_branches):
    rand_forward = [
        tf.random_uniform(
    ...
    return rand_forward, rand_backward


def block_reduction_0(inputs, name='reduction_block_a', **kwargs):
    """Builds Reduction-A block for modelz v4 network."""
    # By default use stride=1 and SAME padding
    rand_forward, rand_backward = generate_weights(tf.shape(inputs)[0], 2)
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockReductionA', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0_1 = conv2d(inputs, 64, stride=(
                2, 2), name='Conv2d_1a1_3x3', **kwargs)
            branch_0_2 = conv2d(inputs, 64, stride=(
                2, 2), name='Conv2d_1a2_3x3', **kwargs)
            branch_0  = tf.concat([branch_0_1, branch_0_2], axis=-1)
        with tf.variable_scope('Branch_1'):
            branch_1 = conv2d(
                inputs, 64, filter_size=(1, 1), name='Conv2d_0a_1x1', **kwargs)
            branch_1 = conv2d(branch_1, 64, stride=(
                2, 2), name='Conv2d_1a_3x3', **kwargs)
            if kwargs.get('is_training', False):
                branch_1 = branch_1 * rand_backward[0] + tf.stop_gradient(branch_1 * rand_forward[0] -
                                             branch_1 * rand_backward[0])
            else:
                branch_1 *= 1.0 / 2
        with tf.variable_scope('Branch_2'):
            ...
            if kwargs.get('is_training', False):
                branch_2 = branch_2 * rand_backward[1] + tf.stop_gradient(branch_2 * rand_forward[1] -
                                             branch_2 * rand_backward[1])
            else:
                branch_2 *= 1.0 / 2
        outputs = tf.concat([branch_0, branch_1, branch_2], 3)
        outputs = conv2d(outputs, 128, filter_size=(1, 1), name='Conv2d_0b_end_1x1', **kwargs)
        return outputs


def block_modelz_a(inputs, name='block_a', **kwargs):
    """Builds modelz-A block for modelz v4 network."""
    # By default use stride=1 and SAME padding
    rand_forward, rand_backward = generate_weights(tf.shape(inputs)[0], 3)
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockmodelzA', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0_1 = conv2d(inputs, 144, filter_size=(
                1, 1), name='Conv2d_0a1_1x1', **kwargs)
            branch_0_2 = conv2d(inputs, 144, filter_size=(
                1, 1), name='Conv2d_0a2_1x1', **kwargs)
            branch_0  = tf.concat([branch_0_1, branch_0_2], axis=-1)
        ...
        with tf.variable_scope('Branch_3'):
            ...
            if kwargs.get('is_training', False):
                branch_3 = branch_3 * rand_backward[2] + tf.stop_gradient(branch_3 * rand_forward[2] -
                                             branch_3 * rand_backward[2])
            else:
                branch_3 *= 1.0 / 3
        outputs = tf.concat([branch_0, branch_1, branch_2, branch_3], 3)
        outputs = conv2d(outputs, 288, filter_size=(1, 1), name='Conv2d_0b_end_1x1', **kwargs)
        return outputs
...
def block_reduction_a(inputs, name='reduction_block_a', **kwargs):
    ...
    with tf.variable_scope(name, 'BlockReductionA', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0_1 = conv2d(inputs, 256, stride=(
                2, 2), name='Conv2d_1a1_3x3', **kwargs)
            branch_0_2 = conv2d(inputs, 384, stride=(
                2, 2), name='Conv2d_1a2_3x3', **kwargs)
            branch_0  = tf.concat([branch_0_1, branch_0_2], axis=-1)
        with tf.variable_scope('Branch_1'):
            branch_1 = conv2d(
        ...
        with tf.variable_scope('Branch_2'):
            ...
            if kwargs.get('is_training', False):
                ...
                branch_2 *= 1.0 / 2
        outputs = tf.concat([branch_0, branch_1, branch_2], 3)
        outputs = conv2d(outputs, 640, filter_size=(1, 1), name='Conv2d_0b_end_1x1', **kwargs)
        return outputs


def block_modelz_b(inputs, name='block_b', **kwargs):
    """Builds modelz-B block for modelz v4 network."""
    # By default use stride=1 and SAME padding
    rand_forward, rand_backward = generate_weights(tf.shape(inputs)[0], 3)
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockmodelzB', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0_1 = conv2d(
                inputs, 320, filter_size=(1, 1), name='Conv2d_0a1_1x1', **kwargs)
            branch_0_2 = conv2d(
                inputs, 320, filter_size=(1, 1), name='Conv2d_0a2_1x1', **kwargs)
            branch_0  = tf.concat([branch_0_1, branch_0_2], axis=-1)
        with tf.variable_scope('Branch_1'):
            branch_1 = conv2d(
                inputs, 192, filter_size=(1, 1), name='Conv2d_0a_1x1', **kwargs)
        ...
        with tf.variable_scope('Branch_2'):
            ...
            if kwargs.get('is_training', False):
                branch_2 = branch_2 * rand_backward[2] + tf.stop_gradient(branch_2 * rand_forward[2] -
                                             branch_2 * rand_backward[2])
            else:
                branch_2 *= 1.0 / 3
        with tf.variable_scope('Branch_3'):
            branch_3 = avg_pool_2d(
...
def block_reduction_b(inputs, name='reduction_block_b', **kwargs):
    """Builds Reduction-B block for modelz v4 network."""
    # By default use stride=1 and SAME padding
    rand_forward, rand_backward = generate_weights(tf.shape(inputs)[0], 2)
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockReductionB', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0_1 = conv2d(
                inputs, 192, filter_size=(1, 1), name='Conv2d_0a1_1x1', **kwargs)
            branch_0_2 = conv2d(inputs, 192, filter_size=(
                1, 1), stride=(2, 2), name='Conv2d_1a2_3x3', **kwargs)
            branch_0  = tf.concat([branch_0_1, branch_0_2], axis=-1)
        with tf.variable_scope('Branch_1'):
            branch_1 = conv2d(
...
def block_modelz_c(inputs, name='block_c', **kwargs):
    """Builds modelz-C block for modelz v4 network."""
    # By default use stride=1 and SAME padding
    rand_forward, rand_backward = generate_weights(tf.shape(inputs)[0], 3)
    reuse = kwargs.get('reuse')
    with tf.variable_scope(name, 'BlockmodelzC', [inputs], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            branch_0_1 = conv2d(
                inputs, 256, filter_size=(1, 1), name='Conv2d_0a1_1x1', **kwargs)
            branch_0_2 = conv2d(
                inputs, 256, filter_size=(1, 1), name='Conv2d_0a2_1x1', **kwargs)
            branch_0  = tf.concat([branch_0_1, branch_0_2], axis=-1)
        with tf.variable_scope('Branch_1'):
            branch_1 = conv2d(
        ...
        with tf.variable_scope('Branch_3'):
            ...
            if kwargs.get('is_training', False):
                ...
                branch_3 *= 1.0 / 3
        outputs = tf.concat([branch_0, branch_1, branch_2, branch_3], 3)
        outputs = conv2d(outputs, 778, filter_size=(1, 1), name='Conv2d_0b_end_1x1', **kwargs)
        return outputs


def modelz_v4_base(inputs, name='modelz', input_size=512, num_block_a=4, num_block_b=7, num_block_c=3, **kwargs):
    """Creates the modelz V4 network up to the given final endpoint.
    ...
    Returns:
      end_points: the set of end_points from the modelz model.
    ...
    with tf.variable_scope(name, 'modelzV4', [inputs]):
        # 448 x 448 x 3
        ...
        with tf.variable_scope('Mixed_5a'):
            net = block_reduction_0(net, 'Mixed_5a', **kwargs)

        if input_size in (256, 512):
            # 32 x 32 x 384
            # num_block_a x modelz-A blocks
            for idx in range(num_block_a):
                block_name = 'Mixed_5' + chr(ord('b') + idx)
                net = block_modelz_a(net, block_name, **kwargs)
            # 32 x 32 x 384
            # Reduction-A block
            net = block_reduction_a(net, 'Mixed_6a', **kwargs)
        if input_size == 512:
            # 16 x 16 x 1024
            # num_block_b x modelz-B blocks
            for idx in range(num_block_b):
                block_name = 'Mixed_6' + chr(ord('b') + idx)
                net = block_modelz_b(net, block_name, **kwargs)
            # 16 x 16 x 1024
            # Reduction-B block
            net = block_reduction_b(net, 'Mixed_7a', **kwargs)
        # 8 x 8 x 1536
        # num_block_c x modelz-C blocks
        for idx in range(num_block_c):
            block_name = 'Mixed_7' + chr(ord('b') + idx)
            net = block_modelz_c(net, block_name, **kwargs)
        return net


def model(inputs, is_training, reuse, input_size=image_size[0], num_block_a=4, num_block_b=5, num_block_c=3, num_classes=1,
    ...
    with tf.variable_scope(name, 'modelz'):
        net = modelz_v4_base(inputs, name=name, input_size=input_size, num_block_a=num_block_a,
        ...
        with tf.variable_scope('Logits'):
            ...
            predictions = sigmoid(logits, name='predictions', **common_args)
