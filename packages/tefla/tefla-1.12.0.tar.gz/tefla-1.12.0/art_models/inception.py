...
def inception_v4_base(inputs, name='inception_4', input_size=512, num_block_a=4, num_block_b=7, num_block_c=3, **kwargs):
    ...
    with tf.variable_scope(name, 'InceptionV4', [inputs]):
        # 448 x 448 x 3
        net_3 = conv2d(inputs, 32, filter_size=(3, 3), stride=(
            2, 2), name='Conv2d_1a_3x3', **kwargs)
        net_7 = conv2d(inputs, 32, filter_size=(7, 7), stride=(
            2, 2), name='Conv2d_1a_7x7', **kwargs)
        net_11 = conv2d(inputs, 32, filter_size=(11, 11), stride=(
            2, 2), name='Conv2d_1a_11x11', **kwargs)
        net = tf.concat([net_3, net_7, net_11], 3)
        # 224 x 224 x 32
        net = conv2d(net, 96, name='Conv2d_2a_3x3', **kwargs)
        # 224 x 224 x 32
        net = max_pool(net, name='MaxPool_2a_3x3')
        # 112 x 112 x 32
        net = conv2d(net, 96, name='Conv2d_2b_3x3', **kwargs)
        # 112 x 112 x 64
