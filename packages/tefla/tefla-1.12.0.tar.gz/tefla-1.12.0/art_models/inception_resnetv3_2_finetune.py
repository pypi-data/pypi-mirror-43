...
def model(inputs, is_training, reuse, num_classes=5, drop_prob=0.2, input_size=image_size[0], name='InceptionResnetV2'):
    common_args = common_layer_args(is_training, reuse)
    rest_conv_params = make_args(
        use_bias=False, batch_norm=batch_norm, activation=relu, **common_args)
    conv_params_notrain = make_args(
        use_bias=False, batch_norm=batch_norm, trainable=False, activation=relu, **common_args)
    ...
    common_args_finetune = common_layer_args(is_training, reuse, trainable=False)
    rest_conv_params_finetune = make_args(
        use_bias=False, batch_norm=batch_norm, activation=relu, **common_args_finetune)
    conv_params_no_bias_finetune = make_args(
        use_bias=False, batch_norm=batch_norm, activation=relu, **common_args_finetune)

    # inputs = input((None, crop_size[1], crop_size[0], 3), **common_args)

    with tf.variable_scope(name, 'InceptionResnetV2'):
        net = conv2d(inputs, 32, filter_size=(7, 7), stride=(2, 2), name='Conv2d_1a_3x3', **conv_params_no_bias_finetune)
        net = conv2d(net, 32, name='Conv2d_2a_3x3', **conv_params_no_bias_finetune)
        # 112 x 112
        net = conv2d(net, 64, stride=(2, 2),name='Conv2d_2b_3x3', **rest_conv_params)
        # 112 x 112
        net = conv2d(net, 64, stride=(2, 2), name='Conv2d_2b_3x3_pool', **rest_conv_params)
        # net = max_pool(net, name='MaxPool_3a_3x3', **rest_pool_params)
        # 64 x 64
        #  net = conv2d(net, 80,  name='Conv2d_3b_1x1', **rest_conv_params)
        # 64 x 64
        net = conv2d(net, 192, name='Conv2d_4a_3x3', **rest_conv_params)
        # 64 x 64
        net = conv2d(net, 192, name='Conv2d_4a_3x3_pool', **rest_conv_params)
