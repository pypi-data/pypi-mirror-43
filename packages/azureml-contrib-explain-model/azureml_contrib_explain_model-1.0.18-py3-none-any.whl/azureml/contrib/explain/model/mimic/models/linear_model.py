...
def _compute_local_shap_values(model, evaluation_examples, multiclass, mean, covariance, seed):
    ...
    if multiclass:
        ...
        shap_values = []
        for class_coef, intercept in zip(coefs, intercepts):
...
class LinearExplainableModel(BaseExplainableModel):
    ...
    def __init__(self, multiclass=False, random_state=DEFAULT_RANDOM_STATE, **kwargs):
        ...
        if self.multiclass:
            self._linear = LogisticRegression(random_state=random_state)
        else:
    ...
    def fit(self, dataset, labels, **kwargs):
        ...
        """
        self._linear.fit(dataset, labels)
...
class SGDExplainableModel(BaseExplainableModel):
    ...
    def __init__(self, multiclass=False, random_state=DEFAULT_RANDOM_STATE, **kwargs):
        ...
        self.random_state = random_state
        if self.multiclass:
            self._sgd = SGDClassifier(random_state=random_state)
        else:
            self._sgd = SGDRegressor(random_state=random_state)
