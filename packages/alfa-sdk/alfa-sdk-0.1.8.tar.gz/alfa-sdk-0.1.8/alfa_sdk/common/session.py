...
class Session:
    def __init__(self, credentials={}, *, context={}, **kwargs):
        self.context = fetch_context(context)
        alfa_env = fetch_alfa_env(kwargs, context=self.context)
        credentials = extract_credentials(credentials, self.context)
...
def fetch_context(context):
    ...
    if not isinstance(env_context, collections.Mapping):
        env_context = {}
...
def extract_credentials(credentials, context):
    if not isinstance(context, collections.Mapping):
        return credentials

    if "access_token" not in credentials and "accessToken" in context:
        credentials["access_token"] = context.get("accessToken")

    if "auth0_token" not in credentials and "auth0Token" in context:
