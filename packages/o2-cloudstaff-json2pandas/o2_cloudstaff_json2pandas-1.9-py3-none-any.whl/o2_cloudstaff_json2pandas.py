...
def get_pay_increase_from_lines(line_df, online_data):
    try:
        ...
        pay_increase_df = pd.DataFrame()
    if pay_increase_df.shape[0] > 0:
        pay_increase_df["full_name"] = pay_increase_df.apply(
            get_resource, axis=1
        ).fillna(method="ffill")
        ...
        pay_increase_df = pay_increase_df[
            ["full_name", "qty", "unit_price", "line_total"]
        ]
