...
class Config(object):
    ...
    # The token file name contains a hash of the username,
    ...
    def get_token_file_path(self):
        ...
        # This always set to a default if not configured, so no need to check
        env = self.get("ENV")
        # Join username, env, and region and hash the result
        hash_input = ",".join([username, env])
        hash = hashlib.md5(hash_input.encode("utf-8")).hexdigest()
        filename = "token-" + hash + ".txt"
        return self.get_demyst_dir_path() / filename

    def has_jwt_token(self):
        # Can't have token file if username not set.  Exit early, so
        # that self.get_token_file_path() logic doesn't blow up.
        if not self.get("USERNAME"):
            return False
        return self.get_token_file_path().is_file()

    def get_jwt_token(self):
        return self.get_token_file_path().read_text()

    def put_jwt_token(self, token):
        demyst_dir = self.get_demyst_dir_path()
        demyst_dir.mkdir(parents=True, exist_ok=True)
        self.get_token_file_path().write_text(token)
