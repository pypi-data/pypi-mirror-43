...
class DjangoModelProvider(BaseRecordProvider):
    ...
    def get_records(self, query_object):
        ...
        if query_object.end:
            ...
            # Если используется пагинация, при этом БД отдала меньшее
            # количество записей, чем умещается на страницу, значит это
            # конец данных. В этом случае, для подсчета общего количества
            # дополнительный count-запрос не потребуется.
            rows_on_page_expected = end - begin
            rows_on_page_got = len(res_data)
            if 0 < rows_on_page_got < rows_on_page_expected:
