...
from keyup import about, configuration, logger, keyconfig, __version__
from keyup.map import map_identity, map_iam_username
from keyup.list_ops import list_keys
from keyup.cauth import prepare_report
from keyup.ptk_progress import ptk_main
...
async def async_caller():
    ...
    result1 = loop.create_task(prepare_report())
...
def init_cli():
    ...
    if len(sys.argv) == 1:
        ...
        #prepare_report(args.debug)
        global loop
        loop = asyncio.get_event_loop()
        loop.run_until_complete(async_caller())
        loop.close()
        sys.exit(exit_codes['EX_OK']['Code'])
