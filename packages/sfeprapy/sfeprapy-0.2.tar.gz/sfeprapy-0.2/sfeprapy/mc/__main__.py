...
def plot_figures(list_path_out_csv, plot_teq_xlim, reliability_target, path_work, figure_name='t_eq.png',
                 apply_probability=False, figure_show_legend=True, **surplus_args):
    ...
    for i, path_out_csv in enumerate(list_path_out_csv):
        ...
        id_ = os.path.basename(path_out_csv).replace('_out.csv', '').replace('_allout.csv', '')
...
def spawn_json_input_files_from_csv():
...
def run():
    ...
    # load csv input file
    try:
        path_work, list_path_input_json = spawn_json_input_files_from_csv()
    except TypeError:
        return -1
    ...
    for i, dict_mc_params in enumerate(list_dict_input_files):
        ...
        if 'path_mc_params_csv' in dict_mc_params:  # Read MC parameters. If USER provided MC parameters in a *.csv file
            # load MC parameters
            try:
    ...
    beam_c = Thermal().c()
    iso834_time = np.arange(0, 6 * 60 * 60, 1)
    ...
    for i, df_mc_params in enumerate(list_df_mc_params):
        dict_mc_params_static = {
            ...
            'fire_type_enforced': list_dict_input_files[i]['fire_type_enforced'],
            'fire_t_alpha': list_dict_input_files[i]['fire_t_alpha'],
            'fire_gamma_fi_q': list_dict_input_files[i]['fire_gamma_fi_q'],
    ...
    for i, list_dict_mc_params in enumerate(list_list_dict_mc_params):
        ...
        if list_dict_input_files[i]['is_live'] == 0:
            continue
...
def post_processing_plot_figures(path_work, dict_config_params):
    ...
    if len(list_path_out_csv_all) >= 0:
        ...
        path_out_csv_merged = os.path.join(path_work, 'temp', '{}_allout.csv'.format(os.path.basename(path_work)))
