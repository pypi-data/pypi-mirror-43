...
class gameWrapperFactory:
    ...
    def make(name, actrep=4, memlen=4, w=84, h=84, random_start=30):
        ...
        if hasattr(env, 'ale'):  # Arcade Learning Environment
            return gameALE(env, actrep=actrep, memlen=memlen, w=w, h=h, random_start=random_start)
        else:  # Basic game environment wrapper
            return gameWrapper(env, actrep=actrep, memlen=memlen,
                              w=w, h=h, random_start=random_start)
        # gameWrapper(env, actrep=actrep, memlen=memlen,w=w, h=h, random_start=random_start)


class gameWrapper:
    """A small wrapper around OpenAI game ALE"""
    def __init__(self, env, actrep=4, memlen=4, w=84, h=84, random_start=30):
        print('Creating wrapper around game Environment')
        self.env = env
        self.memlen = memlen
        self.W = w
        self.H = h
...
class gameALE(gameWrapper):
    def __init__(self, env, actrep=4, memlen=4, w=84, h=84, random_start=30):
        super(gameALE, self).__init__(env=env, actrep=actrep, memlen=memlen, w=w, h=h,
