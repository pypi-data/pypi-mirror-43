...
@click.group()
@click.option(
    "--test", is_flag=True, help="Only test commands but dont run them.")
@click.option(
    "--verbose",
    "-v",
    is_flag=True,
...
def mount(config, name):
    """
    Run the mount helper
    """

    if config.test:
        print("Running in test mode.")

    # Intent becomes mount name in mount mode
    for entry, val in config.userconfig.mounts.items():
        #print(entry, val)
        if entry == name:
