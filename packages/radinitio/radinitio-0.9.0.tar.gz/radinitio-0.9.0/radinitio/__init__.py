import random, re, gzip, time, sys, argparse, os, multiprocessing
import numpy as np
...
class LibraryOptions:
    def __init__(self, library_type='baird', renz_1='sbfI', insert_mu=350, insert_sigma=30,
            min_distance=500, coverage=20, read_len=150, ierr=0.001, ferr=0.01):
...
def target_reads(n_alleles, coverage, ploidity=2):
    n_reads = (n_alleles // ploidity) * coverage
    return int(n_reads)
...
# Sample from the merged sequencing clone size/error distribution and
# determine the properties of the sequenced clone
def determine_seq_clone_size_error(seq_clone_errors_freq = [0.0, 1.0],
...
class PCRDups:
    def __init__(pcr_c=0,
        ...
        if args.pcr_c == 0:
            ...
            self.ampl_clone_size_distrib = get_library_clone_size_distribution(pcr_model)
            ...
            single_adj_error_distrib = adjust_dist_of_ampl_errors(log_ampl_clone_size_distrib, args.rlen, self.base, self.max_it)
...
def sequence_sample(sample_n, out_dir, alleles_list, library_opts=LibraryOptions(), mutation_opts=MutationModel(), pcr_opts=None):
    # Open output files
    ...
    while remaining_reads > 0:
        ...
        clone_size  = None
        n_mut_reads = None
        # TODO: This needs to connect with the rest of the PCR functions. Right now hardcoded at 0 dups
        if pcr_opts is None:
            clone_size  = 1
            n_mut_reads = 0
        else:
        ...
        if n_mut_reads > 0:
            mut_allele = (allele[0], introduce_pcr_mutation(allele[1], insert_len, library_opts, mutation_model))
...
# Sequence library
def sequence_library(out_dir, popmap_file, alleles_dir, library_opts=LibraryOptions(), mutation_opts=MutationModel(), pcr_opts=None):
    # Check output directory
    out_dir = out_dir.rstrip('/')
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
