...
class Initialise(object):
    def __init__(self, configfile=None, inifile=None, logfile=None
        ...
        # when the assetic-esri package is initiated a logger is created to
        # catch any issues that occur before this config instance is initialised
        # Now we have a log file defined in the config we can remove that
        # handler and attach the sdk handler
        ...
        try:
            ...
            if assetic_version and len(assetic_version) == 4 \
                    and int(assetic_version[0]) >= 2019 \
                    and int(assetic_version[1]) >= 3 \
                    and int(assetic_version[2]) >= 2 \
                    and int(assetic_version[0]) >= 0:
                pass
            else:
                # version may be too old.  Issue warning
                config.asseticsdk.logger.warning(
                    "Current version of assetic python package is too old."
                    " Version is: {0}, expecting minimum of 2019.2.3.0".format(
