...
class SendToCivis(MetalNode):
    ...
    def setup(self):
        ...
        if self.via_staging_table:
            sql_query = (
                '''DROP TABLE IF EXISTS "{schema}"."{staging_table}";''').format(
            ...
            sql_query = (
                ...
                '''"{schema}"."{production_table}");''').format(
                    schema=self.schema,
    ...
    def cleanup(self):
        ...
        if self.via_staging_table:
            ...
            _ = fut.result()
            sql_query = (
                '''DROP TABLE "{schema}"."{staging_table}";'''.format(
                    schema=self.schema, staging_table=self.staging_table))
            logging.debug('Dropping staging table {staging_table}.'.format(
                staging_table=self.staging_table))
        else:
            pass
    ...
    def monitor_futures(self):
        ...
        while run:
            ...
            for table_id, future_dict in table_items:
                ...
                logging.debug(
                    'poller result:' + str(future_obj._state) +
