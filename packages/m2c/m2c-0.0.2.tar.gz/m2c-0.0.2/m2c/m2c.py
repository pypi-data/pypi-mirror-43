...
def openapi():
    ...
    if os.path.exists('config.yaml'):
        try:
            ...
            app = load(open('config.yaml'), Loader)
    ...
    _genSwaggerYaml('swagger.yaml', url, config['title'], config['description'], config['version'])
    print Color.green('Gen openapi to swargger.yaml done.')
...
def init():
    ...
    open(os.path.join(project_name, '.m2c'), 'wb').write(json.dumps(cfg, indent=2))
