...
from cubicweb.server import hook, Service
...
def set_local_cache(vcsrepo):
    ...
    if vcsrepo.source_url:
        ...
        reponame = 'repository'
    vcsrepo.cw_edited['local_cache'] = osp.join(text_type(vcsrepo.eid), reponame)
...
def clone_to_local_cache(vcsrepo):
    ...
    try:
        ...
        msg = _('can not clone the repo from %(source)s, '
                'please check the source url')
        raise ValidationError(vcsrepo.eid, {'source_url': msg}, {'source': url})
...
class VCSFileStartHook(hook.Hook):
    ...
    def __call__(self):
        ...
        def refresh_vcsrepos():
            from cubicweb_vcsfile.hooks import _refresh_repos as refresh
            return refresh(repo)
...
def _refresh_repos(repo, eids=None, bridge=bridge, raise_on_error=False):
    '''
    ...
    with repo.internal_cnx() as cnx:
        ...
        rql = ('Any R, RT, RE, RLC, RSU WHERE R is Repository, '
        ...
        for vcsrepo in reposrset.entities():
            ...
            try:
            ...
            except bridge.VCSException as ex:
                if raise_on_error:
                    raise
                cnx.error(str(ex))
                continue
                ...
                if raise_on_error:
                    raise
                cnx.exception('error while retrieving handler for %s', vcsrepo.eid)
                continue
            ...
            if (config['repository-import']
                and vcsrepo.cw_source[0].name == 'system'):
                bridge.import_vcsrepo_content(
                    cnx, repohdlr, raise_on_error=raise_on_error,
...
class RefreshRepoService(Service):
    ...
    def call(self, eids=None, raise_on_error=False):
        _refresh_repos(self._cw.repo, eids, raise_on_error=raise_on_error)
