...
:copyright: 2007-2016 LOGILAB S.A. (Paris, FRANCE), all rights reserved.
...
from six.moves import zip_longest, reduce
...
class HGRepo(EntityAdapter):
    ...
    configs = [
        # we use the strip command which is an extension
        'extensions.strip=',
        # we want obsolescence markers and rely on the allprecursors revset.
        'extensions.evolve=',
    ]
    path = None
    ...
    def entity_for_cset(self, ctx_hex):
        rset = self._cw.execute('Any X WHERE X from_repository R, R eid %(repo)s, X changeset %(cs)s',
                                {'repo': self.entity.eid, 'cs': ctx_hex.decode('utf-8')})
        if rset:
            return rset.one()
        return None

    def full_path(self, path):
        return osp.join(self.path, path).encode('utf-8')
    ...
    def file_deleted(self, path, rev=None):
        with self.hgrepo() as repo:
            ...
            for file in repo.manifest(rev=rev):
                if file[4].decode(self.entity.encoding) == path:
                    return False
            return True
    ...
    def head(self, path=None, **args):
        ...
        with self.hgrepo() as repo:
            try:
                ...
                if b'unknown revision' in exc.err:
                    return None
                raise
        if logs:
            return logs[0]
    ...
    def log_rset(self, path=None, **args):
        ...
        args = [iter(log)] * 50
        for partiallog in zip_longest(fillvalue=None, *args):
    ...
    def pull(self):
        ...
        if self.entity.source_url:
            with self.hgrepo() as hgrepo:
                try:
                    hgrepo.pull(source=self.entity.source_url.encode('utf-8'))
                except CommandError as exc:
    ...
    def import_content(self, commitevery):
        ...
        with self.hgrepo() as hgrepo:
            ...
            knownheads = [row[0].encode('ascii') for row in rset]
    ...
    def update_phases(self, cnx, hgrepo):
        ...
        for phase in (u'public', u'draft', u'secret'):
            revs = set(ctx.node[:12].decode('ascii') for ctx in hgrepo.log(revrange='%s()' % phase))
            if revs:
                cnx.execute('SET R phase %%(phase)s WHERE R changeset IN (%s),'
                            'NOT R phase %%(phase)s, R from_repository REPO,'
                            'REPO eid %%(repo)s' %
                            ','.join('"%s"' % rev for rev in revs),
    ...
    def update_hidden(self, cnx, hgrepo):
        ...
        oldhidden = set(num for num, in cnx.execute('Any CS WHERE R hidden True, '
                                                    ...
                                                    {'hg': repoeid}).rows)
        newhidden = set(rev.node[:12].decode('ascii') for rev in hgrepo.log(revrange='hidden()'))
        ...
        for new_hide in hide:
            ...
            data = {'r': self.entity.eid,
                    'p': str(ctx)}
            successors_changesets = [cs.node[:12].decode('ascii')
                                     for cs in hgrepo.log(revrange='successors(%s)' % ctx.node().decode('ascii'))]
    ...
    def import_revision(self, hgrepo, i):
        ...
        if self.entity_for_cset(ctx_hex):
            self.debug('skip revision %s, seems already imported', ctx_hex)
            return
        date = ctx.date()
        if date.tzinfo is None:
            date = get_localzone().localize(date)
        revdata = {'date': date,
                   'author': ctx.author().decode(self.entity.encoding),
                   'description': ctx.description().decode(self.entity.encoding),
                   'changeset': ctx_hex.decode('ascii'),
                   'phase': ctx.phase().decode('ascii'),
                   'branch': ctx.branch().decode(self.entity.encoding),
                   'hidden': ctx.hidden(),
        ...
        parent_changesets = [n.node[:12].decode('ascii')
                             for n in parent_changesets] if parent_changesets else []
        precursors_changesets = [cs.node[:12].decode('ascii')
                                 for cs in hgrepo.log(revrange='allprecursors(%s)' % i)]
