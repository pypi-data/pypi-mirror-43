...
from logilab.mtconverter import xml_escape, TransformData

from cubicweb import Unauthorized, tags
from cubicweb.mttransforms import ENGINE
from cubicweb.predicates import is_instance, score_entity, match_kwargs
from cubicweb.view import EntityView
from cubicweb.uilib import soup2xhtml, domid
from cubicweb.web.views import (
    ibreadcrumbs, tableview, primary, tabs, navigation, uicfg
)
...
_pvdc.tag_attribute(('Revision', 'description'),
                    {'showlabel': False, 'vid': 'vcsfile.rev.description'})
...
class RepositoryCheckOutInstructionsView(EntityView):
    ...
    def entity_call(self, entity):
        ...
        if entity.has_anonymous_access:
            msg = _('Non-members may check out a read-only working copy anonymously over {}.').format(
            xml_escape(url_scheme.upper()))
        else:
            msg = _('Members only may check out working copy over {}.').format(
                xml_escape(url_scheme.upper()))
        w(u'<p>%s</p>' % msg)
...
class RevisionPrimaryView(primary.PrimaryView):
    ...
    def render_entity_title(self, entity):
        self.w(u'<h1>%s</h1>' % xml_escape(entity.shortdesc))
    ...
    def render_entity_relations(self, rev):
        ...
        if versioned:
            tpl = u'<a href="#%s">%s</a>\n'
            headers = (self._cw._('files modified by this revision'),)
            subst = lambda obj: (domid(obj.decode('utf-8', 'replace')),
                                 xml_escape(obj.decode('utf-8', 'replace')))
            pyvalue = [[tpl % subst(obj)] for obj in versioned]
            self.wview('pyvaltable', pyvalue=pyvalue, headers=headers)
        else:
...
class RevisionDescriptionView(EntityView):
    __regid__ = 'vcsfile.rev.description'
    __select__ = EntityView.__select__ & is_instance('Revision') & match_kwargs('rtype')

    def entity_call(self, entity, rtype, **kwargs):
        if '\n' not in entity.description:
            return
        longdesc = entity.description.split('\n', 1)[1].lstrip('\n')
        trdata = TransformData(longdesc, 'text/rest', 'utf8', appobject=self)
        data = ENGINE.convert(trdata, 'text/html').decode()
        data = soup2xhtml(data, self._cw.encoding)
        if '<div class="system-message">' not in data:
            # this is what cubicweb/ext/html4zope.py injects when there are
            # parsing errors. Couldn't find a way to get an exception instead...
            self.w(data)
        else:
            self.w(u'<p>\n')
            self.w(xml_escape(longdesc).replace('\n', '<br>'))
            self.w(u'</p>\n')
...
class RevisionBreadCrumbTextView(ibreadcrumbs.BreadCrumbTextView):
    __select__ = is_instance('Revision')

    def cell_call(self, row, col, **kwargs):
        entity = self.cw_rset.get_entity(row, col)
        self.w(entity.changeset)
