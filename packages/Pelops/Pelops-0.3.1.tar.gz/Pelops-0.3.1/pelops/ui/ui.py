...
class UI(Cmd):
    ...
    def __init__(self, intro, prompt, stop_service_cmd, service_is_stopping):
        ...
        self._main_thread = threading.Thread(target=self._wrapper)
    ...
    def _wrapper(self):
        ...
        if not self._service_is_stopping.is_set():
            stop_daemon = threading.Thread(target=self._stop_service_cmd)
    ...
    def add_command(cls, name, function):
        ...
        if not callable(function):
            raise ValueError("parameter function must be callable")
