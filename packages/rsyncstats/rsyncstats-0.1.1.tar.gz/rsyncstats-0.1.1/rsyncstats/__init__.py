    ...
    with open(filename, 'r', encoding='ISO-8859-1') as file:
        ...
        for log in file:
            ...
            if event_data['logtype'] == 'rsync to' or event_data['logtype'] == 'rsync on':
                ...
                logger.debug("Previous Data")
                logger.debug(previous_data)
                logger.debug("Sent Data")
                logger.debug(event_data)
                events.append({ **event_data, **previous_data })
                logger.debug("Merged data")
                logger.debug(events[-1])

    for v in events_buffer:
        logger.info("Still events in the buffer : %s"%events_buffer[v])
    return(events)
