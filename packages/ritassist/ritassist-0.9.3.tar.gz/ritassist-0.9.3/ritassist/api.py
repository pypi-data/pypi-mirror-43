...
class API:
    ...
    def stream_devices(self, callback):
        ...
        data_url = "https://api.ritassist.nl/api/equipment/subscribe/?groupId=0&vehicleCategoryId=0&driverCategoryId=0"

        header = self.authentication_info.create_header()
        response = requests.get(data_url, headers=header, stream=True)
