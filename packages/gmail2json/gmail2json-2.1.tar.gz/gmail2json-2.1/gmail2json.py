...
def make_parent_folder(message, p, archive_name, folder_name):
    email_date = date.fromtimestamp(int(message["internalDate"][:10])).isoformat()
    folder_path = p / archive_name / email_date / folder_name
...
def convert_message_to_json(service, message, body, p, archive_name):
    folder_path = make_parent_folder(message, p, archive_name, "messages")
...
def write_file(
    ...
    folder_path = make_parent_folder(message, "attachments", p, archive_name)
...
def save_attachments(service, message, p, archive_name):
    msg_id = message["id"]
    pdf_attachment_list = []
