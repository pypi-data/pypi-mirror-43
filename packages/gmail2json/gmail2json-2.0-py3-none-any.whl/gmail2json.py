...
def make_parent_folder(message, p, archive_name, parent_name):
    folder = date.fromtimestamp(int(message["internalDate"][:10])).isoformat()
    folder_path = p / archive_name / folder / parent_name
...
def convert_message_to_json(service, message, body, p, archive_name):
    folder_path = make_parent_folder(message, "messages", p, archive_name)
