...
class Instruction:
    ...
    def __init__(self, desc: 'InstDescription', address: int,
                 mnemonic_token: Lexing.Token, arg_tokens: Dict[str, Lexing.Token] = {}):
        """
...
class InstDescription:
    ...
    def construct(self, addr: int, mnemonic_token: Lexing.Token,
                  arg_tokens: Dict[str, Lexing.Token],
...
def LineParser(descriptions: List[InstDescription],
    ...
    #   Grammer: <label> <mnemonic> <argument>* <comment>? (<newline>|<EOF>)
    with_label = MultiMatch(label_types, name='_label') + without_label

    # The final parser is the combination of the three parsers
    return empty_line | without_label | with_label
...
class Context:
    ...
    def resolve_integer(self, token: Lexing.Token, min: int, max: int,
        ...
        if token.value < min or token.value > max:
            raise AssemblerTypeError(("{0} {1} (0x{1:x}) is not in the valid range "
                                      "[{2},{3}]!").format(name, token.value, min, max),
                                     self._lexer_context, token)
        return token.value

    def resolve_label(self, token: Lexing.Token, min: int, max: int) -> int:
    ...
    def assemble(self):
        ...
        while it:
            ...
            if '_mnemonic' in res.annotations:
                ...
                # add entry into the symbol if a label was parsed
                if '_label' in res.annotations:
                    label_token = res.annotations['_label']
                    label_str = label_token.source_string
                    if inst is None:
                        raise AssemblerTypeError(
                            "Labels can only be applied to real instructions!",
                            self._lexer_context, label_token)
                    if label_str in self._symbol_table:
                        # TODO improve error message with previous declaration (requires to add the
                        #      label token into the symbol table)
                        raise AssemblerTypeError(
                            "Redefinition of label {}!".format(repr(label_str)),
                            self._lexer_context, label_token)
                    self._symbol_table[label_str] = inst

        # second pass (indirect) pass over the input by traversing the instruction object list
    ...
    @property
    def symbol_table(self) -> Dict[str, Instruction]:
        """Read only property that returns the symbol table.

        :returns: Dictionary which maps symbol names to instruction objects.
        """
