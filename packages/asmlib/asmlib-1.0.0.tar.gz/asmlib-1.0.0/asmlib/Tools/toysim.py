...
def load_toy_file(filename: str, max_addr: int = 0xFF):
    ...
    while it:
        ...
        data[addr_token.value] = value_token.value
    return data
...
class JsonResultExtension:
    ...
    @staticmethod
    def end_of_simulation(sim: asmlib.Simulation.Simulator,
                          args: argparse.Namespace):
        if args.json:
...
def lib_main(extensions: list, args: argparse.Namespace) -> int:
    ...
    if args.relaxed:
        reg_value = 0
    init_state = asmlib.Simulation.ZeroR0ExecutionState(16, 16, 0xFF, initial_value=reg_value)
...
def main(extensions: list = [asmlib.Toy.IsaSupportExtension,
    ...
    if args is None:
        extensions += asmlib.Toy.cli_extension_parsing()
