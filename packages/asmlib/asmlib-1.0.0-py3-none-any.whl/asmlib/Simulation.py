...
def format_value_list(list: List[Optional[int]], value_bit: int,
    ...
    :param lables: Dictionary that associates indices with specific label strings.
    ...
    if add_element_labels:
        ...
        if len(lables) > 0:
            label_bits = max(label_bits, *[len(v)*4 for v in lables.values()])
    formatted = []
    for idx, value in enumerate(list):
        if add_element_labels:
            label = lables.get(idx, format_value(idx, label_bits))
...
class InstDescription:
    ...
    def format(self, inst: Instruction) -> str:
        """Format the instruction in assembler syntax.
...
class ExecutionState():
    def __init__(self, registers: List[Optional[int]], reg_bits: int, max_pc: int):
        self.simulator = None
        ...
        self._reg_mask = 2**reg_bits
        self._max_pos_reg_value = 2**(reg_bits - 1) - 1
    ...
    def __setitem__(self, nr: int, value: int):
        value = value % self._reg_mask
        if value > self._max_pos_reg_value:
            value = -self._reg_mask + value
        self._registers[nr] = value

    def format_registers(self, per_line: Optional[int] = None,
                         col_alignment: Optional[int] = None) -> str:
        lines = format_value_list(self._registers, self.reg_bits, True,
                                  per_line=per_line,
    ...
    def format(self, per_line: Optional[int] = None,
        ...
        lines += [self.format_registers(per_line=per_line, col_alignment=col_alignment)]
        return lines
...
class ZeroR0ExecutionState(ExecutionState):
    def __init__(self, reg_nr: int, reg_bit: int, max_pc: int, initial_value: Optional[int] = None):
        super().__init__([0] + [initial_value] * (reg_nr - 1), reg_bit, max_pc)
...
class PeripheralDevice:
    ...
    def format_state(self) -> List[str]:
        return []
...
class SimpleRAM(MemorySubsystem, PeripheralDevice):
    def __init__(self, simulator: Optional['Simulator'],
        ...
        super().__init__(simulator)
        self._name = name
        self._content = [0] * len(content)

        # copy the content, truncate the elements and make them signed
        reg_mask = 2**element_bits
        max_pos_reg_value = 2**(element_bits - 1) - 1
        for idx, element in enumerate(content):
            if element is None:
                self._content[idx] = None
                continue
            self._content[idx] = element % reg_mask
            if self._content[idx] > max_pos_reg_value:
                self._content[idx] = -reg_mask + self._content[idx]
    ...
    def format_state(self) -> List[str]:
        return format_value_list(self._content, self._element_bits, False, per_line=16)
...
class Simulator:
    ...
    def run(self, instructions: Optional[int] = None,
        ...
        while not self.state.end_of_execution:
            ...
            pc = self.state.pc
            encoding = self.inst_mem.read(pc, 1)[0]
            if encoding is None:
    ...
    def format_state(self, include_instructions: bool = True) -> List[str]:
        lines = []
        lines.append('Processor State (e.g., Registers)')
        lines.append('---------------------------------')
        lines += self.state.format()
        for device in self.devices:
            device_state = device.format_state()
