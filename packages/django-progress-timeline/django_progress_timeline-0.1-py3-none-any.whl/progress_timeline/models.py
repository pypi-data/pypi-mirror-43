...
class ProgressTimeLine(models.Model):
    ...
    def get_progress_bar(self):
        ammount = sum(self.currentprogress_set.values_list('ammount', flat=True))
        return {
            'currently': self._get_progress((datetime.now() - self.start_date).total_seconds(), (self.end_date - self.start_date).total_seconds()),
            'ratio': self._get_progress(ammount, self.goal),
        }

    def preview(self):
        if self.pk:
            return '''<div style="height:135px;width:500px;top:-1em;left:10em;position:relative;">
