...
class AbstractNtlmAuthHandler:
    ...
    def __init__(self, password_mgr=None, debuglevel=0):
        self.logger = logging.getLogger('ntlm3')
    ...
    def http_error_authentication_required(self, auth_header_field, req, fp, headers):
        ...
        self.logger.debug(
            'init handler auth header %s - %s', auth_header_field, auth_header_value,
        )
        if auth_header_field:
            if auth_header_value is not None and 'ntlm' in auth_header_value.lower():
                fp.close()
                self.logger.debug('init handler - handling')
    ...
    def retry_using_http_NTLM_auth(self, req, auth_header_field, realm, headers):
        ...
        if pw is None:
            self.logger.debug('handler - exit no pwd')
            return None
        ...
        if auth_header_val == auth:
            self.logger.debug(
                'handler - exit no match %s %s',
                auth_header_val, auth,
            )
            return None
        ...
        headers = dict((name.title(), val) for name, val in headers.items())
        try:
            self.logger.debug('handler - kick of re-request %r', headers)
            h.request(req.get_method(), selector, req.data, headers)
            ...
            self.logger.debug('handler - kick of re-request done')
            return infourl
...
class HTTPNtlmAuthHandler(AbstractNtlmAuthHandler, urllib.request.BaseHandler):
    ...
    def http_error_401(self, req, fp, code, msg, headers): # pylint: disable=unused-argument
        ...
        """
        self.logger.debug('http_error_401')
