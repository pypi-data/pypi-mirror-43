...
from pygrisb.mbody.coulomb_matrix import U_J_to_radial_integrals, \
...
def h5save_usr_qa_setup(material, log=sys.stdout):
    ...
    if '-eqidx' in sys.argv:
        ...
        if yn == 'n':
            while True:
                string_idx_equivalent_atoms = input(
    ...
    unique_corr_symbol_list = [s.encode() for s in unique_corr_symbol_list]
    f['/usrqa/unique_corr_symbol_list'] = unique_corr_symbol_list
    f['/usrqa/unique_df_list'] = [df.encode() for df in unique_df_list]
