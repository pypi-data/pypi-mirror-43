...
class FileValidation(object):
    ...
    def run_validator(self, path, glob_pattern="*", consider_correctly_formed_files=True, raise_exceptions=False):
        ...
        if self.prefix or self.suffix:
            logger.info(f"Carrying out filename validation with following prefix \"{self.prefix}\""
                        f" and suffix \"{self.suffix}\"")
            ...
            logger.info(f"Files excluded for filename: {excl_for_filename}")
        if self._file_size:
