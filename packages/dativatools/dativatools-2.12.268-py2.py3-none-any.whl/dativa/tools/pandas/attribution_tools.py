...
class Shapley:
    ...
    def __init__(self, df_impression, df_conversions, id_column, sets_column, default_score=0, infer_missing=False, metric='conversions'):
    ...
    def _generate_subsets(self, exclude=None):
        ...
        while i < pop_size:  # < so stops at len - 1, so doesn't IndexError
            for c in combinations([p for p in self.population if p != exclude], i):
                yield c
            i += 1
            logger.debug('completed for combination of length {0} / {1} at {2:%H:%M:%S}'.format(i, pop_size, datetime.now()))
