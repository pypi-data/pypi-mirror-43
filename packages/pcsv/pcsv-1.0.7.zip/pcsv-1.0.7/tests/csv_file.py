...
class TestCsvFile(object):
    """Test for CsvFile class."""

    # pylint: disable=R0205,R0904
    ...
    def test_init_exceptions(self):
        ...
        func_pointers = [
            ...
            (RE, "Column headers are not unique in file {0}", write_cols_not_unique),
    ...
    def test_add_dfilter(self):
        ...
        with pmisc.TmpFile(write_no_header_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter={0: 3}, has_header=False)
            obj.add_dfilter({0: 1})
    ...
    def test_data(self):
        ...
        assert obj.data(filtered=True) == [[1, 4, 20]]
        with pmisc.TmpFile(write_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter=({"Result": 20}, "Ref"))
        ...
        with pmisc.TmpFile(write_no_header_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter=[0, 2], has_header=False)
    ...
    def test_data_exceptions(self):
        ...
        assert obj.data() == [["Col1", "Col2", "Col3"], [1, None, 10], [1, 4, None]]
        assert obj.data(no_empty=True) == [["Col1", "Col2", "Col3"]]
        obj.dfilter = [0, 2]
        assert obj.data(filtered=True, no_empty=True) == [["Col1", "Col3"], [1, 10]]
    ...
    def test_replace(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            obj = pcsv.CsvFile(fname=fname)
        obj.cfilter = [0, 2]
        obj.replace([[1.0, "a"], [2.0, "b"], [3.0, "c"], [4.5, "d"], [5.0, "e"]], True)
        assert obj.data() == [
            [1.0, 3, "a"],
            [2.0, 4, "b"],
            [3.0, 4, "c"],
        ...
        assert obj.data() == [[1, "a", 7], [2, "b", 8], [3, "c", 9], [1, "d", 6]]
        with pmisc.TmpFile(write_no_header_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter={0: 1}, has_header=False)
        obj.cfilter = [0, 2]
        obj.replace([[1.0, "a"], [2.0, "b"]], filtered=True)
        assert obj.data() == [[1.0, 4, "a"], [2, 5, 8], [3, 6, 9], [2.0, 6, "b"]]
    ...
    def test_replace_exceptions(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            ...
            exmsg = "Number of columns mismatch between input and replacement data"
    ...
    def test_reset_dfilter(self):
        ...
        assert obj.dfilter == (None, None)
        with pmisc.TmpFile(write_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter=({"Ctrl": 5}, ["Ref"]))
        # No reset
        assert obj.dfilter == ({"Ctrl": 5}, ["Ref"])
        obj.reset_dfilter("N")
        assert obj.dfilter == ({"Ctrl": 5}, ["Ref"])
    ...
    def test_write_exceptions(self):
        ...
        with mock.patch("pmisc.make_dir", side_effect=mock_make_dir):
            ...
            exmsg = "File {0} could not be created: Permission denied".format(root_file)
