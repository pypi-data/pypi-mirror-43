...
def analyzer_name_from_manifest(manifest_path: str) -> str:
    """
    ...
    if os.path.exists(manifest_path):
        with open(manifest_path) as f:
            try:
                spec = json.load(f)
                return spec["analyzer_name"]
            except:
                raise ValueError("analyzer_name attribute not defined in analyzer.json")
    raise ValueError(f"No analyzer.json found at {manifest_path}")
...
def recursive_chmod(dir: str, mode: int) -> None:
    os.chmod(dir, mode)
    for (cur_path, dirnames, filenames) in os.walk(dir):
        dirpaths = [os.path.join(cur_path, dirname) for dirname in dirnames]
        filepaths = [os.path.join(cur_path, filename) for filename in filenames]
