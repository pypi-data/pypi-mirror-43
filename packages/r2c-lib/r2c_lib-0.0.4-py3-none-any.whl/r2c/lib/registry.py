...
ManifestWithStateJson = TypedDict(
    "ManifestWithStateJson", {"manifest": AnalyzerManifestJson, "pending": bool}
)
...
class RegistryData:
    ...
    def add_pending_manifest(self, manifest: AnalyzerManifest) -> "RegistryData":
        ...
        for dep in manifest.dependencies.items():
            resolved = new_reg._resolve(AnalyzerName(dep[0]), dep[1])
            if not resolved:
                raise InvalidManifestException(
                    f"A dependency in this manifest cannot be resolved: {dep}"
                )
            # and that it doesn't depend on itself
            if resolved == va:
                raise InvalidManifestException(
    ...
    def get_direct_dependencies(self, va: VersionedAnalyzer) -> List[VersionedAnalyzer]:
        """
        ...
        resolved_values = [
            self._resolve(AnalyzerName(dep[0]), dep[1])
            for dep in manifest.dependencies.items()
        ]
        return [r for r in resolved_values if r is not None]

    def sorted_deps(
        self, versioned_analyzer: VersionedAnalyzer
    ) -> List[VersionedAnalyzer]:
        """
            DAG of all recursive dependencies for this analyzer version.
            Resolution of ranges to pinned versions also happens here.
        """
        deps_graph = self._dependency_graph(subgraph_from_node=versioned_analyzer)
    ...
    def _resolve(
        ...
        resolved = Spec(spec_string).select(all_versions)
        if resolved:
            return VersionedAnalyzer(analyzer_name, resolved)
        return None

    def _dependency_graph(
        self, subgraph_from_node: VersionedAnalyzer = None
    ) -> Graph[VersionedAnalyzer]:
        edges = set()
        nodes = set()
        ...
        if subgraph_from_node:
            ...
            to_explore = set(self.versioned_analyzers)
        ...
        while to_explore:
            va = to_explore.pop()
            next_manifest = self.manifest_for(va)
            if not next_manifest:
                raise ManifestNotFoundException(
                    f"manifest not found for analyzer {va.name} at version {va.version}. Registry data: {self.to_json()}"
                )

            deps = next_manifest.dependencies.items()

            nodes.add(va)
            for dep in deps:
                resolved_dep = self._resolve(AnalyzerName(dep[0]), dep[1])
                if not resolved_dep:
                    raise Exception(f"Can't resolve dependency {dep} of {va}")
                edges.add((va, resolved_dep))
                if not resolved_dep in nodes:
                    to_explore.add(resolved_dep)
