    ...
    for test_filename in test_vectors:
        ...
        with open(test_path) as test_content:
            ...
            with tempfile.TemporaryDirectory(
                ...
                # cleanup mount_folder with proper permissions
                if os.name == "nt":
                    recursive_chmod(tempdir, stat.S_IWRITE)
                elif os.name == "posix":
                    recursive_chmod(tempdir, 0o777)
...
def build_docker(docker_image, path_to_dockerfile, version, env_args_dict, verbose):
    extra_build_args = [f"--build-arg {k}={v}" for (k, v) in env_args_dict.items()]
    build_cmd = f"docker build -t {docker_image} {path_to_dockerfile} " + " ".join(
        extra_build_args
    )
    if verbose:
        build_cmd += " 1>&2"
...
def run_analyzer_on_local_code(
    ...
    for va in versions:
        with tempfile.TemporaryDirectory(prefix=LOCAL_RUN_TMP_FOLDER) as mount_folder:
            ...
            # cleanup mount_folder with proper permissions
            if os.name == "nt":
                recursive_chmod(mount_folder, stat.S_IWRITE)
            elif os.name == "posix":
    ...
    if validator:
        ...
        temp_fd, temp_filename = tempfile.mkstemp(dir=get_tmp_dir())
        infra.get_file(
            S3_ANALYSIS_BUCKET_NAME, key=results["s3_key"], name=temp_filename
        )
        if validator:
            return validator(temp_filename)
