...
class ID4meClient(object):
    def __init__(self, validate_url,
        ...
        :param preferred_client_id: (optional) preferred client ID
    ...
    def get_consent_url(self, context, state='', claimsrequest=None, prompt=None, usenonce=True):
        """
    ...
    def get_user_info(self, context):
        ...
        self._decode_user_info(context, registration, response, user_claims, context.iss)
        return user_claims

    def _get_distributed_claims(self, context, registration, endpoint, access_code, user_claims, leeway):
        """
        :param context: identifier context
        :type context: ID4meContext
        ...
        try:
            ...
            if status == 200:
                ...
                self._decode_user_info(context, registration, response, user_claims, iss, leeway)
            else:
    ...
    def _decode_token(self, token, context, registration, iss, tokentype, leeway=datetime.timedelta(minutes=5), verify_aud=True, auth_jwkset=None):
        """
        Decodes OIDC tokens
        ...
        :param auth_jwkset: Authority JWKSet override (for unit tests)
        :type auth_jwkset: jwk.JWKSet
    ...
    def _decode_user_info(self, context, registration, jwtresponse, user_claims, iss, leeway=datetime.timedelta(minutes=5)):
        """
            ...
            for claimref in response['_claim_names']:
                if response['_claim_names'][claimref] in response['_claim_sources'] \
                    ...
                    if (endpoint, access_token) not in queried_endpoints:
                        self._get_distributed_claims(context, registration, endpoint, access_token, user_claims, leeway)
