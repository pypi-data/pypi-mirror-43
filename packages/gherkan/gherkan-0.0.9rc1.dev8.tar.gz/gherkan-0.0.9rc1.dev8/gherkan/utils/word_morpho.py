...
class Finetuner():
    def __init__(self):
        self.morph_lwt = majka.Majka(os.path.join(
            imp.find_module("gherkan")[1], "utils/majka.l-wt"))
        self.morph_wlt = majka.Majka(os.path.join(
            imp.find_module("gherkan")[1], "utils/majka.w-lt"))
    ...
    def find_verb(self, lang, sentence, head_only=True):
        ...
        if verb:
            if verb[0] in verb_dic[lang]["not verbs"]:
                verb = []
        if not verb:
            for word in sentence_l.split():
                if word in verb_dic[lang]["verbs"]:
                    if word == "-být":
                        ...
                        verb = [word]
        if not verb:
            print(colored("Warning: did not detect any verb in sentence: {}".format(
                sentence), "yellow"))
        return verb
