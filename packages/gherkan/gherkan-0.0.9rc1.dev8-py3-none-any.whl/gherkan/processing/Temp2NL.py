...
CZ_PROGRAM_DICT_DIR = os.path.join(
    c.PROJECT_ROOT_DIR, "gherkan/utils", "RobotPrograms_cz.json")
EN_PROGRAM_DICT_DIR = os.path.join(
    c.PROJECT_ROOT_DIR, "gherkan/utils", "RobotPrograms_en.json")
GENERAL_YAML_DIR = os.path.join(
    c.PROJECT_ROOT_DIR, "gherkan/utils", "en-cz_phrases.yaml")
    ...
    def ParseMainAction(self, data):
        ...
        for key in varz:  # separate actor names (shuttleXY > shuttle XY)
            if not varz[key].isupper():
                # we leave out the first letter in case it is upper
                string = str(varz[key][1:])
                actor = (re.findall('([A-Z0-9].*)', string))
                if actor:
                    varz[key] = varz[key].replace(
                        actor[0], " {}".format(actor[0]))
                else:
                    pass
        ...
        if verb:
            verb_conj = self.finetune.conjugate(
                self.lang, verb[0], 3)  # conjugate verb to third person
        ...
        if phrase:
            ...
            print(colored("Failed to create NL description for following actions: {}".format(
                self.data["niceStr"]), "yellow"))
            ...
            with open(EN_PROGRAM_DICT_DIR, 'r', encoding="utf-8") as stream:
                ...
                try:
                    ...
                    if actor.lower() in action_list:
                        ...
                        print(colored("Warning: Did not find program {} for robot {} in English".format(
                            prog, actor), "yellow"))
                    else:
                        return False
            ...
            with open(CZ_PROGRAM_DICT_DIR, 'r', encoding="utf-8") as stream:
                ...
                try:
                    ...
                    if actor.lower() in action_list:
                        ...
                        print(colored("Warning: Did not find program {} for robot {} in Czech".format(
                            prog, actor), "yellow"))
                    else:
                        return False
        return action
