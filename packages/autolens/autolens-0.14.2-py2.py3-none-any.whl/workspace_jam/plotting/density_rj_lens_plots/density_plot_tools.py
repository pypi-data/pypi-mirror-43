...
class SLACS(object):
    ...
    def density_vs_radii_sie(self, values, radius_kpc, number_bins=60):
        ...
        for r in range(number_bins):
            ...
            sie_density = self.critical_density_arcsec * (self.lens_galaxy.sie.angular_mass_within_circle(radii[r + 1]) -
                                                          self.lens_galaxy.sie.angular_mass_within_circle(radii[r])) / annuli_area
    ...
    def density_vs_radii_ltm2(self, values, radius_kpc, center_skip, ltm_skip, gradient=None, number_bins=60):
        ...
        for r in range(number_bins):
            ...
            bulge_density = self.critical_density_arcsec * (self.lens_galaxy.bulge.angular_mass_within_circle(radii[r + 1]) -
                                                            self.lens_galaxy.bulge.angular_mass_within_circle(radii[r])) / annuli_area

            envelope_density = self.critical_density_arcsec * (self.lens_galaxy.envelope.angular_mass_within_circle(radii[r + 1]) -
                                                               self.lens_galaxy.envelope.angular_mass_within_circle(radii[r])) / annuli_area

            halo_density = self.critical_density_arcsec * (self.lens_galaxy.halo.angular_mass_within_circle(radii[r + 1]) -
                                                           self.lens_galaxy.halo.angular_mass_within_circle(radii[r])) / annuli_area
    ...
    def masses_of_all_samples_sie(self, radius_kpc, values):
        ...
        sie_mass =  self.critical_density_arcsec * self.lens_galaxy.sie.angular_mass_within_circle(radius=radius_arc)
    ...
    def masses_of_all_samples_ltm2(self, radius_kpc, values, center_skip, ltm_skip):
        ...
        radius_arc = radius_kpc * self.arcsec_per_kpc
        bulge_mass =  self.critical_density_arcsec * self.lens_galaxy.bulge.angular_mass_within_circle(radius=radius_arc)
        envelope_mass = self.critical_density_arcsec * self.lens_galaxy.envelope.angular_mass_within_circle(radius=radius_arc)
        halo_mass = self.critical_density_arcsec * self.lens_galaxy.halo.angular_mass_within_circle(radius=radius_arc)
