...
def make_pipeline(phase_folders=None):
    ...
    pipeline_name = 'pipeline_subhalo_light_profile_no_lens_light_power_law'
    ...
    # In phase 2, we will fit the lens galaxy's mass and one source galaxy, where we:

    # 1) Fit a power-law mass profile, using priors fromo the SIE of the previosu phase.
    ...
    class LensSourcePhase(ph.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.mass.axis_ratio = results.from_phase('phase_1_source').variable.lens.mass.axis_ratio
            self.lens_galaxies.lens.mass.phi = results.from_phase('phase_1_source').variable.lens.mass.phi
    ...
    phase2 = LensSourcePhase(phase_name='phase_2_power_law', phase_folders=phase_folders,
    ...
    class GridPhase(autofit_ph.as_grid_search(ph.LensSourcePlanePhase)):
        ...
        def pass_priors(self, results):
            ...
            self.lens_galaxies.lens.mass = results.from_phase('phase_2_power_law').constant.lens.mass
            self.lens_galaxies.lens.shear = results.from_phase('phase_2_power_law').constant.lens.shear
            self.source_galaxies.source = results.from_phase('phase_2_power_law').variable.source
    ...
    return pipeline.PipelineImaging(pipeline_name, phase1, phase2, phase4)