...
from tkinter import Tk,Entry,StringVar,Listbox
...
class AutoEntry(Entry):
    ...
    def _on_keyrelease(self, event):
        ''' méthode prévue pour remplacer _on change, à vérifier !'''
                ...
                if choices:
                    ...
                    if not self._listboxUp:
                        self.listbox = Listbox(width=self["width"],
                                                height=self._max,
                                                **self._highlight)
                        self.listbox.place(x=self.master.winfo_x(),
