...
class Item(ItemBase, metaclass=ItemMetaclass, no_setup=True):
    ...
    **To configure an item class use next class variables:**
    ...
    :cvar norewrite_fields: Dictionary with fields that should not be rewritten.
        ...
        .. note::
            You can use `True` key for all not listed fields.
    ...
    :cvar deleter_selector_fields: Set of model field values that is going to
        be collected from the processed (created and updated) ORM models.
        See `deleter_keeper_fields` for more details.
    
    :cvar deleter_keeper_fields: Set of model field values that is going to
        be collected from the processed (created and updated) ORM models.
        Default value: `None` if `deleter_selector_fields` were not set,
        ...
        `deleter_selector_fields`, but not the same as collected using
        `deleter_keeper_fields`.
    
    :cvar deleter_execute_on_persist:
        If `True` then deleter going to be executed upon item persistence.
        Default: `False`.
    """
