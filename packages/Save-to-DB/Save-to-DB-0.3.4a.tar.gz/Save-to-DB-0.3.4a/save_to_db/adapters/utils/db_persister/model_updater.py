...
class ModelUpdater(object):
    ...
    def can_set_relation(self, item, model, field_name, value):
        ...
        if field_name not in item.norewrite_fields:
            return True
        ...
        for group in item.creators:
            ...
            for field_name in group:
                if field_name in item.relations:
                    ...
                    if field_name not in fkeys or \
                            not self.can_set_relation(item, None, field_name,
                                                        fkeys[field_name]):
                        can_create = False
                        break
    ...
    def set_related_model(self, item, model, fkey, fmodel):
        ...
        """
        if not self.can_set_relation(item, model, fkey, fmodel):
            return
    ...
    def add_related_models(self, item, model, fkey, fmodels):
        ...
        filtered_models = [
            ...
            if self.can_set_relation(item, model, fkey, fmodel)
        ]
    ...
    def update_model_fields(self, item_track, model):
        ...
        for field_name in item_track.item.data:
            if field_name in item_track.item.fields:
                if self.can_set_relation(item_track.item, model, field_name,
