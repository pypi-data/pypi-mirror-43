        ...
        if len(text_data) == 0:
            ...
            if culture_ranges:
                for word in text_data.split(' '):
                    if len(word) >= 2:
                        symbol_range = self.get_symbol_range(word[0])

                        for symbol in word[1:]:
                            if symbol_range != self.get_symbol_range(symbol):
                                self.logger.warning(
                                    TRANSLATOR, 
                                    '\033[33mWord "\033[93m{0}\033[33m" specified at \033[92m{1}\033[33m contains mixed scripts.\033[0m'.format(
                                        word,
                                        text_cell))
                                break
...
class IosDataWriter(object):
    ...
    def write(self, translation_data):
        ...
        for file_name, file_data in translation_data.items():
            ...
            with open(file_path, 'w', encoding='utf-8') as strings_file:
                for code, item in file_data.items():
                    if item.is_first_in_group:
                        strings_file.write('\r\n')
...
class TranslationBuilder(object):
    def __init__(self, logger, platform, sheet_id, base_path, culture_name):
        ...
        if platform == 'android':
            ...
            self.writer = AndroidDataWriter(logger, self.base_path, self.culture_name) if self.base_path else None
        elif platform == 'ios':
            self.reader = GoogleSheetDataReader(logger, sheet_id, culture_name, 2, 3)
            self.writer = IosDataWriter(logger, self.base_path, self.culture_name) if self.base_path else None
