...
class Storage(BaseStorage):
    ...
    def reconnect_redis(self):
        ...
        storage = StrictRedisCluster(
            startup_nodes=map(lambda x: { "host": x.split(':')[0], "port": x.split(':')[1] if len(x.split(':')) > 1 else "6379" }, [ x.strip() for x in self.context.config.REDIS_RESULT_STORAGE_SERVER_STARTUP_NODES.split(',') ]),
