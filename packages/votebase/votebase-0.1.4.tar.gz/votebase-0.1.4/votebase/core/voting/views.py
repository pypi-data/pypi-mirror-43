...
class VoteView(TemplateResponseMixin, View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if get_password_ident(self.round) not in request.session:
            if self.round.is_secured():
                return redirect(reverse('voting_password',
                    args=(self.round.pk, )))
        ...
        if self.round.ancestor is None:
            # it is not child
            request.session['current_round'] = self.round
        else:
            ...
            if request.user.is_authenticated():
                ...
                if not voters.count():
                    ...
                    request.session['ancestor_child'] = self.round
                    request.session['current_round'] = self.round.ancestor
                    return redirect(reverse('voting_vote', args=(self.round.ancestor.pk, )))
            else:
                # Anonymous user
                request.session['ancestor_child'] = self.round
                if request.session.get('current_round', None) != self.round:
                    request.session['current_round'] = self.round.ancestor
                    return redirect(reverse('voting_vote', args=(self.round.ancestor.pk, )))

        if 'current_round' in request.session:
            if request.session['current_round'] != self.round:
                return redirect(reverse('voting_vote', args=(request.session['current_round'].pk, )))
        ...
        if ancestor_childs.count():
            request.session['ancestor_child'] = ancestor_childs[0]
        ...
        if get_voting_started_ident(self.round) not in request.session:
            request.session[get_voting_started_ident(self.round)] = now()
    ...
    def post(self, request, pk):
        ...
        if is_valid:
            messages.success(
                request, _(u'Your votes has been successfully saved.'))
            ...
            if get_voting_started_ident(self.round) in request.session:
                voting_started = request.session[get_voting_started_ident(self.round)]
                voting_ended = now()
        ...
        if self.round.count_questions is not None and\
           self.round.count_questions != self.survey.question_set.count():
            ...
            if get_questions_ident(self.round) in request.session:
                questions = request.session[get_questions_ident(self.round)]
            else:
                questions = self.round.get_random_questions()
                request.session[get_questions_ident(self.round)] = questions
        else:
...
class PasswordView(FormView):
    ...
    def get_form(self, form_class):
        return form_class(self.round, self.request.POST or None)
...
class FinishView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.round = get_object_or_404(Round, pk=kwargs.get('pk'))
        self.voter = request.session['voter'] if 'voter' in request.session else None
        ...
        # Chaining
        if 'ancestor_child' in request.session:
            ancestor_child = request.session['ancestor_child']
            request.session['current_round'] = ancestor_child
#            return redirect(reverse('voting_vote', args=(ancestor_child.pk, )))
