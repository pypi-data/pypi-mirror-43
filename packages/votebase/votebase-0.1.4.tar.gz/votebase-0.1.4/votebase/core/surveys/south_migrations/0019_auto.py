...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'Survey', fields ['hash_key']
        db.create_index('votebase_surveys', ['hash_key'])

        # Adding index on 'Round', fields ['is_repeatable']
        db.create_index('votebase_rounds', ['is_repeatable'])

        # Adding index on 'Round', fields ['is_quiz_result_visible']
        db.create_index('votebase_rounds', ['is_quiz_result_visible'])

        # Adding index on 'Round', fields ['date_from']
        db.create_index('votebase_rounds', ['date_from'])

        # Adding index on 'Round', fields ['is_active']
        db.create_index('votebase_rounds', ['is_active'])

        # Adding index on 'Round', fields ['is_quiz_correct_options_visible']
        db.create_index('votebase_rounds', ['is_quiz_correct_options_visible'])

        # Adding index on 'Round', fields ['date_to']
        db.create_index('votebase_rounds', ['date_to'])

        # Adding index on 'Round', fields ['is_anonymous']
        db.create_index('votebase_rounds', ['is_anonymous'])


    def backwards(self, orm):
        # Removing index on 'Round', fields ['is_anonymous']
        db.delete_index('votebase_rounds', ['is_anonymous'])

        # Removing index on 'Round', fields ['date_to']
        db.delete_index('votebase_rounds', ['date_to'])

        # Removing index on 'Round', fields ['is_quiz_correct_options_visible']
        db.delete_index('votebase_rounds', ['is_quiz_correct_options_visible'])

        # Removing index on 'Round', fields ['is_active']
        db.delete_index('votebase_rounds', ['is_active'])

        # Removing index on 'Round', fields ['date_from']
        db.delete_index('votebase_rounds', ['date_from'])

        # Removing index on 'Round', fields ['is_quiz_result_visible']
        db.delete_index('votebase_rounds', ['is_quiz_result_visible'])

        # Removing index on 'Round', fields ['is_repeatable']
        db.delete_index('votebase_rounds', ['is_repeatable'])

        # Removing index on 'Survey', fields ['hash_key']
        db.delete_index('votebase_surveys', ['hash_key'])
