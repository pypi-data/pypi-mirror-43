...
class UserProfileMixin(object):
    def get_profile_data(self, profile, use_display_values=False):
        ...
        result = {}
        facebook_fields = FacebookProfileModel._meta.get_all_field_names()
        facebook_fields.remove('gender')

        for key in profile._meta.get_all_field_names():
            if key.startswith('_') or key in ('api_key', 'user', 'package_valid_to') or key in facebook_fields:
                continue
