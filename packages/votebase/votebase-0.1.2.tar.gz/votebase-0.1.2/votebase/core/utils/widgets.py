...
from django.forms.widgets import ClearableFileInput, RadioInput, MultiWidget, SubWidget
...
class MatrixRadioInput(RadioInput):
    def render(self, name=None, value=None, attrs=None, choices=()):
        name = name or self.name
        value = value or self.value
        attrs = attrs or self.attrs
...
class BootstrapRadioTableFieldRenderer(forms.widgets.RadioFieldRenderer):
    ...
    def render(self):
        ...
        for w in self:
            w.choice_label = ''
            output.append(
                '<td><div class="radio">%s</div></td>' % force_unicode(w))
...
class Matrix(MultiWidget):
    def __init__(self, rows=None, columns=None):
        ...
        for row in rows:
            widgets.append(forms.RadioSelect(
                choices=columns, renderer=BootstrapRadioTableFieldRenderer))
    ...
    def render(self, name, value, attrs=None):
        ...
        for i, widget in enumerate(self.widgets):
            ...
            output.append('<tr><th class="option %s">%s</th>%s</td>' % (
                klass, rendered_headings, rendered_output))
        return mark_safe(
