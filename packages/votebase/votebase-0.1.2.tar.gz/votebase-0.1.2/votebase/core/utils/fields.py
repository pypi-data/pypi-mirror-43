...
class MatrixField(MultiValueField):
    ...
    def __init__(self, rows=None, columns=None, required=True, widget=Matrix,
        ...
        for row in rows:
            self.fields.append(self.choice_field(choices=columns, initial=40))
        super(MatrixField, self).__init__(
            self.fields, initial=initial, help_text=help_text,
            required=required, label=label, *args, **kwargs)
