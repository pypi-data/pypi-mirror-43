...
def new_payment(payment):
    subject = _(u'New payment on VoteHub')
    text = _(u'Dear admin, \n\n new payment was received by %(username)s \
        in value of %(cash)s%(currency)s') % {
            'username': payment.user.username,
            'cash': payment.cash,
            'currency': settings.VOTEHUB_CURRENCY_SYMBOL,
        }
    return send_mail(
        subject, text, 'info@votehub.net', settings.VOTEHUB_PAYMENT_NOTIFY)
