...
def voter_form_result(context, form, voter=None):
    try:
        ...
        return u'<div class="alert alert-%(class)s">%(message)s</div>' % {
            'class': klass,
            'message': message
        }
...
def voter_quiz_result(context, voter, only_percent=False):
    ...
    why_so_message = _(u'This results are different, because survey creator '
                       u'changed correct options in quiz questions. Contact him.')

    why_so = u'<div class="alert alert-warning">%(message)s</div>' % {
        'message': why_so_message
    }

    return '%(stored)s<br>%(calculated)s%(why_so)s<br>' % {
