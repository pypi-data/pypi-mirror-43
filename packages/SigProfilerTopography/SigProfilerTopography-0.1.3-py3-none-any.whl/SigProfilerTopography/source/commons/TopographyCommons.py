...
IndelsSignatures2NumberofMutationsDictFilename = 'IndelsSignatures2NumberofMutationsDict.txt'
Sample2IndelsSignatures2NumberofMutationsDictFilename = 'Sample2IndelsSignatures2NumberofMutationsDict.txt'
...
########################################################################################
def getSignaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict(outputDir,jobname):
    ...
    return signaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict
########################################################################################


########################################################################################
def getSample2SignaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict(outputDir,jobname):
    ...
    return sample2SignaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict
########################################################################################


########################################################################################
def getSamplesWithAtLeast10KMutations2NumberofMutationsDict(outputDir,jobname):
    ...
    return samplesWithAtLeast10KMutations2NumberofMutationsDict
########################################################################################


########################################################################################
def getIndelsSignatures2NumberofMutationsDict(outputDir,jobname):
    indelsSignatures2NumberofMutationsDict = {}
    indelsSignatures2NumberofMutationsDictFilePath = os.path.join(outputDir,jobname,DATA,IndelsSignatures2NumberofMutationsDictFilename)

    if (os.path.exists(indelsSignatures2NumberofMutationsDictFilePath)):
        indelsSignatures2NumberofMutationsDict = readDictionary(indelsSignatures2NumberofMutationsDictFilePath)

    return indelsSignatures2NumberofMutationsDict
########################################################################################


########################################################################################
def getSample2IndelsSignatures2NumberofMutationsDict(outputDir,jobname):
    sample2IndelsSignatures2NumberofMutationsDict = {}

    sample2IndelsSignatures2NumberofMutationsDictFilePath = os.path.join(outputDir,jobname,DATA,Sample2IndelsSignatures2NumberofMutationsDictFilename)

    if (os.path.exists(sample2IndelsSignatures2NumberofMutationsDictFilePath)):
        sample2IndelsSignatures2NumberofMutationsDict = readDictionary(sample2IndelsSignatures2NumberofMutationsDictFilePath)

    return sample2IndelsSignatures2NumberofMutationsDict
########################################################################################
...
def fillNecessaryDataStructures(genome,outputDir,jobname):
    ...
    chrNamesInNucleosomeList = getChrNamesInNucleosomeList()
    signaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict = getSignaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict(outputDir, jobname)
    indelsSignatures2NumberofMutationsDict = getIndelsSignatures2NumberofMutationsDict(outputDir, jobname)
    sample2SignaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict = getSample2SignaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict(outputDir,jobname)
    samplesWithAtLeast10KMutations2NumberofMutationsDict = getSamplesWithAtLeast10KMutations2NumberofMutationsDict(outputDir,jobname)
    sample2IndelsSignatures2NumberofMutationsDict = getSample2IndelsSignatures2NumberofMutationsDict(outputDir,jobname)
...
def readMutations(mutationsWithSignatureBasedProbabilitiesFileName):
    ...
    print('Signature\tNumberofMutationsWithProbability>=%f\tNumberofSamples' %SUBSTITUTION_MUTATION_SIGNATURE_PROBABILITY_THRESHOLD)
    for signature in signatures:
        signaturebased_df = mutation_df[mutation_df[signature] >= SUBSTITUTION_MUTATION_SIGNATURE_PROBABILITY_THRESHOLD]
    ...
    for sample in samplesWithAtLeast10KMutations2NumberofMutationsDict:
        for signature in signaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict:
            ...
            numberofMutations = len(mutation_df[ ((mutation_df[SAMPLE]==sample) & (mutation_df[signature]>= SUBSTITUTION_MUTATION_SIGNATURE_PROBABILITY_THRESHOLD)) ])
            if (numberofMutations>= TEN_THOUSAND):
...
def fillSignalArrayAndCountArrayForIndels(
    ...
    for signature in indelsSignatures2NumberofMutationsDict:
        if (indel_row[signature]>INDEL_MUTATION_SIGNATURE_PROBABILITY_THRESHOLD):
            ...
            ####################################################
            if (sample in sample2IndelsSignatures2NumberofMutationsDict) and (signature in sample2IndelsSignatures2NumberofMutationsDict[sample]):
...
def readIndelsAndWriteChrBasedParallel(outputDir,jobname,indelsFileName):
    ...
    writeDictionaryUnderDataDirectory(indelsSignatures2NumberofMutationsDict,outputDir,jobname,IndelsSignatures2NumberofMutationsDictFilename)
    writeDictionaryUnderDataDirectory(sample2IndelsSignatures2NumberofMutationsDict,outputDir,jobname,Sample2IndelsSignatures2NumberofMutationsDictFilename)
    #############################################################################################################################
    ####################################################### Write ends ##########################################################
    #############################################################################################################################
...
def readIndels(allIndelsFileName):
    ...
    # We assume that after the column named 'Context' there are the signature columns in tab separated way.
    signatures = columnNamesList[(contextIndex + 1):]
    print('For Debug MArch 15, 2019 starts')
    print('Indels Signatures')
    print(signatures)
    print('For Debug MArch 15, 2019 ends')
    #################################################
    ...
    for signature in signatures:
        ...
        numberofMutations = len(signaturebased_df)
        indelsSignatures2NumberofMutationsDict[signature] = numberofMutations
    ...
    for sample in listofSamples:
        for signature in indelsSignatures2NumberofMutationsDict:
            ...
            if sample in sample2IndelsSignatures2NumberofMutationsDict:
                sample2IndelsSignatures2NumberofMutationsDict[sample][signature] = numberofMutations
            else:
                sample2IndelsSignatures2NumberofMutationsDict[sample]={}
                sample2IndelsSignatures2NumberofMutationsDict[sample][signature] = numberofMutations
    #################################################################################################


    return sample2NumberofIndelsDict,indelsSignatures2NumberofMutationsDict,sample2IndelsSignatures2NumberofMutationsDict,indels_df
##################################################################
