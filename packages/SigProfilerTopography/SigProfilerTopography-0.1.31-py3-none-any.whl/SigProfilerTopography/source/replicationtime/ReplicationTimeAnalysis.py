...
#TODO left here pool write int numpy arrays in parallel
def readReplicationTimeDataAndWriteChrBasedReplicationTimeNPArrays(genome,replicationTimeFilename):
    ...
    for decile_df in decile_df_list:
        chrBased_grouped_decile_df = decile_df.groupby('chr')
...
def generateIntervalVersion(replication_time_wavelet_signal_unprocessed_df):
    ...
    replication_time_wavelet_signal_interval_version_df[CHROM].astype(str)
    replication_time_wavelet_signal_interval_version_df[START].astype(np.int32)
    replication_time_wavelet_signal_interval_version_df[END].astype(np.int32)
    replication_time_wavelet_signal_interval_version_df[SIGNAL].astype(np.float32)
...
def readRepliSeqTimeData(genome,pool,repliseqDataFilename):
    ...
    chrNamesInReplicationTimeDataArray = replication_time_interval_version_df['chr'].unique()
