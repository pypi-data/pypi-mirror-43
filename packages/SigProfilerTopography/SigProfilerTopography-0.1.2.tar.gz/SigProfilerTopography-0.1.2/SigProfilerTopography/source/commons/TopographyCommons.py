...
#New version
#March 15, 2019
def fillSplitBasedSignalArrayAndCountArrayForIndelsWithExtraSampleBasedAndIndelsSignatures(
        indel_row,
        nucleosome_array,
        maximum_chrom_size,
        sample2SignaturesWithAtLeast10KEligibleMutations2NumberofMutationsDict,
    ...
    for signature in indelsSignatures2NumberofMutationsDict:
        if (indel_row[signature]>MUTATION_SIGNATURE_PROBABILITY_THRESHOLD):
...
def readIndels(allIndelsFileName):
    ...
    for signature in signatures:
        signaturebased_df = indels_df[indels_df[signature] >= MUTATION_SIGNATURE_PROBABILITY_THRESHOLD]
        numberofMutations = len(signaturebased_df)
    ...
    for sample in listofSamples:
        for signature in indelsSignatures2NumberofMutationsDict:
            #check if there are at least 10K mutations with probability >= 0.5 for this (sample,signature) pair
            numberofMutations = len(indels_df[ ((indels_df[SAMPLE]==sample) & (indels_df[signature]>= MUTATION_SIGNATURE_PROBABILITY_THRESHOLD)) ])
