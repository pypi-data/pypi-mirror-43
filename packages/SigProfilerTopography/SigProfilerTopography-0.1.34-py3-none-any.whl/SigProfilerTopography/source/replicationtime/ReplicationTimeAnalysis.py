...
def fillReplicationTimeNPArrays(inputList):
    ...
    chrBased_grouped_decile_df_list = inputList[2]
    replicationTimeFilename_wo_extension = inputList[3]

    chrBasedReplicationTimeDataNPArray = fillChrBasedReplicationTimeNPArray(chrLong, chromSize,chrBased_grouped_decile_df_list)
...
##################################################################
def writeNumberofAttributableBases(decile_df_list,replicationTimeFilename_wo_extension,filename):
    ...
    filepath = os.path.join(current_abs_path,ONE_DIRECTORY_UP,ONE_DIRECTORY_UP,LIB,REPLICATION,replicationTimeFilename_wo_extension,filename)
    writeDictionaryWithFilePath(decileIndex2NumberofAttributableBasesDict,filepath)
##################################################################

##################################################################
#March 21, 2019 starts
def readReplicationTimeDataAndWriteChrBasedReplicationTimeNPArrays(genome,chromSizesDict,replicationTimeFilename):
    ...
    decileIndex2NumberofAttributableBasesDictFilename = '%s_%s' %(replicationTimeFilename_wo_extension,DecileIndex2NumfAttributableBasesDictFilename)
    writeNumberofAttributableBases(decile_df_list,replicationTimeFilename_wo_extension,decileIndex2NumberofAttributableBasesDictFilename)
    ...
    poolInputList = []
    for chrLong in chrNamesInReplicationTimeDataArray:
        inputList = []
...
def generateIntervalVersion(replication_time_wavelet_signal_unprocessed_df):
    ...
    print('Debug starts Marhc 22, 2019 starts')
    print('replication_time_wavelet_signal_interval_version_df.dtypes')
    print(replication_time_wavelet_signal_interval_version_df.dtypes)
    print('Debug starts Marhc 22, 2019 ends')
...
def readRepliSeqTimeData(genome,pool,repliseqDataFilename):
    ...
    for decile_df in deciles_df_list:
...
##################################################################
def searchSubsOnNPArray(inputList):
...
def loadChrBasedReplicationTimeNPArray(chrLong,replicationTimeDataFilename):
    ...
    chrBasedReplicationTimeArrayFilenamePath = os.path.join(current_abs_path, ONE_DIRECTORY_UP, ONE_DIRECTORY_UP, LIB, REPLICATION,
                                                            CHRBASED,chrBasedReplicationTimeArrayFilename )
...
##################################################################
def calculateCountsForSubsAndIndelsUsingNPArrays(outputDir,jobname,numofProcesses,pool,
                                    sample2NumberofSubsDict,
                                    sample2NumberofIndelsDict,
                                    subsSignature2NumberofMutationsDict,
                                    indelsSignature2NumberofMutationsDict,
    ...
    for chrLong in chromNamesList:
        ...
        for split_index in range(numofProcesses):
            ...
            listofDictionaries = pool.map(searchSubsOnNPArray,poolInputList)
...
##################################################################
def replicationTimeAnalysis(chromNamesList,outputDir,jobname,singlePointMutationsFileName,indelsFilename,repliseqDataFilename):
    ...
    #Last decile_df in decile_df_list contains the intervals that are replicated the latest.
    #Please note that each decile_df contains intervasl from all chroms (mixed chroms)
    #TODO why are you reading again?
    #We must not read again
    # we must use the written files under lib
    ...
    decileIndex2NumberofAttributableBasesDictFilename = '%s_%s' %(replicationTimeFilename_wo_extension,DecileIndex2NumfAttributableBasesDictFilename)
    decileIndex2NumberofAttributableBasesDict = getDecileIndex2NumberofAttributableBasesDict(decileIndex2NumberofAttributableBasesDictFilename)
    ...
    #old way filling NP array runtime
    ...
    #new way
    #loading offline prepared numpy arrau
    type2DecileBasedAllChrAccumulatedCountDict , sample2Type2DecileBasedAllChrAccumulatedCountDict = calculateCountsForSubsAndIndelsUsingNPArrays(
                                                                                                                            outputDir,jobname,numofProcesses,pool,
                                                                                                                            sample2NumberofSubsDict,
                                                                                                                            sample2NumberofIndelsDict,
                                                                                                                            subsSignature2NumberofMutationsDict,
    ...
    #can we get rid of decile_df_list?
    #old way
