...
async def encode_mapping(mapping, validat):
    loop = asyncio.get_event_loop()
    with ProcessPoolExecutor() as pool:
        mapping = await loop.run_in_executor(pool, _encode_mapping, mapping, validat)
    return mapping
...
def _encode_mapping(mapping, validat):
    ...
    for fromeid, toeidsorlocations in mapping.items():
        if not isinstance(toeidsorlocations, dict):
            pbmapping.mapping[fromeid].eids.extend(toeidsorlocations)
        else:
            _encode_locations(toeidsorlocations, pbmapping.mapping[fromeid].locations)
    mapping.validat.FromSeconds(validat)
    return pbmapping
...
def _decode_mapping(pbmapping):
    ...
    for eid in pbmapping:
        if mapping[eid].HasField('eids'):
            mapping[eid] = pbmapping[eid].eids
        else:
            mapping[eid] = _decode_locations(pbmapping[eid].locations)
    return mapping
