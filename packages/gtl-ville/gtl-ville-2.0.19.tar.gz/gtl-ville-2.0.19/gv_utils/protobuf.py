...
async def decode_locations(response):
    loop = asyncio.get_event_loop()
    with ProcessPoolExecutor() as pool:
        locations = await loop.run_in_executor(pool, _decode_locations, *(response.locations,))
    return locations
...
async def decode_mapping(response):
    loop = asyncio.get_event_loop()
    with ProcessPoolExecutor() as pool:
        mapping = await loop.run_in_executor(pool, _decode_mapping, *(response.mapping,))
    return mapping
