...
# see: https://gitpython.readthedocs.io/en/stable/reference.html?highlight=RemoteProgress#git.remote.RemoteProgress.update
def progress_bar(op_code, cur_count, max_count=None, message=''):

    total_size = 30

    if max_count is not None:
        f = total_size * ( float(cur_count) / float(max_count) )
        size = int( f + 0.5 ) 

    str_ = ''.join( ( '\r[' , '|'*size , ' '*(total_size-size) , ']') )
...
def clone(url,destination_path):
    #git.Repo.clone_from(url,destination_path,progress=Progress())
