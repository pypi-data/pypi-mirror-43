...
def start_interpreter(**variables):
    console = type('haravan ' + haravan.version.VERSION, (code.InteractiveConsole, object), {})
    import readline
...
class Tasks(object):
    _shop_config_dir = os.path.join(os.environ["HOME"], ".haravan", "shops")
    ...
    def add(cls, connection):
        ...
        if os.path.exists(filename):
            ...
            domain = raw_input("Domain? (leave blank for %s.myharavan.com) " % (connection))
            if not domain.strip():
                domain = "%s.myharavan.com" % (connection)
    ...
    def console(cls, connection=None):
        ...
        start_interpreter(haravan=haravan)

    @classmethod
    @usage("version")
    def version(cls, connection=None):
        """output the haravan library version"""
        print(haravan.version.VERSION)
