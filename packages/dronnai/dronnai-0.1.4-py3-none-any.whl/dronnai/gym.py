...
class GYM:
    def __init__(self,train,valid,model,metric,optimizer,loss,loss_ratios=[1],names=['Task'],device=torch.device('cuda:0'),cut_output=[False]):
        ...
        self.train_rs = [t.random_shuffler.random_state for t in self.train]
    ...
    def workout(self,check_interval=0,max_epochs=float('Inf'),stop_after_one=False,dynamic_loss=False,drop_last=False):
        ...
        for train, state in zip(self.train,self.train_rs):
            train.random_shuffler.random_state = state
            train.iterations = 0
        ...
        while True:
            try:
                ...
                if stop:
                    ...
                    break
                if self.train[0].epoch >= max_epochs:
    ...
    def log_metrics(self,t,t_metric,v_metric,t_loss,v_loss,name):
        ...
        to_print = self.log_template.format(name,time.time() - self.time_start,
                                        self.train[t].epoch, 
                                        self.train[t].iterations, 
                                        self.train[t].state_dict()['iterations_this_epoch'],
                                        self.train_lens[t],
                                        100. * self.train[t].state_dict()['iterations_this_epoch'] / self.train_lens[t],
                                        t_loss,
                                        v_loss, 
                                        t_metric, 
                                        v_metric)
...
class GYM_DataLoader(GYM):
    def __init__(self,train,valid,model,metric,optimizer,loss,loss_ratios=[1],names=['Task'],device=torch.device('cuda:0'),cut_output=[False]):
        """
        GYM which work better with DataLoader
        """
    ...
    def log_metrics(self,t,t_metric,v_metric,t_loss,v_loss,name):
        ...
        to_print = self.log_template.format(name,time.time() - self.time_start,
                                       ...
                                       self.train_iters[t], 
                                       self.train_lens[t]%self.train_iters[t],
                                       self.train_lens[t],
