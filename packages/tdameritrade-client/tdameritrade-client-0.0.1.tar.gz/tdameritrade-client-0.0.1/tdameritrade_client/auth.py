...
class TDAuthenticator(object):
    ...
    def _get_url(self, type: str):
        """
        Get the correct request URL for a given task.
        Args:
            type: A string specifying what type of request can be made. Supports:
        ...
        elif type == 'token_request':
            url += urls.TOKEN_REQUEST
        else:
            raise NotImplementedError('The requested url type {} is not supported.'.format(type))
    ...
    def _start_server(self, key_path: str, cert_path: str):
        """
        Builds secure socket listening on the callback address.
        Args:
    ...
    def _create_ssl_cert(cert_path: str, key_path: str) -> None:
        ...
        try:
            ...
            pass
        print('Keep pressing enter for default certificate.')
        ssl_cmd = 'openssl req -newkey rsa:2048 -nodes -keyout {} -x509 ' \
                  '-days 365 -out {}'.format(key_path, cert_path)
        os.system(ssl_cmd)
