...
def get_cloudiness_from_jp2(cloud_prob_data_path,cutline_geojson,gauss_filter_sigma=4):
	logsup.logger.info('cloud_prob_data_path: %s',cloud_prob_data_path)
	nodata_value=255
	cloud_value=100
	b=DataSet(cloud_prob_data_path).do_warp(cutline_json=cutline_geojson,dstNodata=nodata_value,out_epsg=3857)
	# b2=DataSet(cloud_prob_data_path).do_warp(cutline_json=cutline_geojson,dstNodata=nodata_value,out_epsg=3857)
	nodata_mask=b.array==nodata_value
	pre_gaussian_array=np.copy(b.array)
	pre_gaussian_array[nodata_mask]=0
	classified_land=binary_erosion(gaussian_filter(pre_gaussian_array*10,gauss_filter_sigma),np.ones((3,3)))*cloud_value
	classified_land[nodata_mask]=nodata_value
	# from image_processing.support import show_images
	# show_images([b.array,classified_land])
	cloud_pixel_count=np.count_nonzero(classified_land==cloud_value)
	cloud_free_pixel_count=np.count_nonzero(classified_land!=nodata_value)
	logsup.logger.info('cloud_free_pixel_count: %s',cloud_free_pixel_count)
	
	if cloud_free_pixel_count==0:
		message='The provided image does not correspond to cadastre land of interest, i.e. the wrong image is being supplied.'
		logsup.logger.critical(message)
		raise NotImplementedError(message)
	logsup.logger.info('Cloudiness from %s: %.1f%%',gsup.get_name(cloud_prob_data_path),cloud_pixel_count/cloud_free_pixel_count*100)
	return cloud_pixel_count,cloud_free_pixel_count

def get_group_cloudiness(cloud_infos):
	###
	# cloud_infos: each cloud_info is an output of "get_cloudiness_from_jp2"
	return np.divide(*np.concatenate(cloud_infos).reshape(len(cloud_infos),-1).sum(axis=0))*100
