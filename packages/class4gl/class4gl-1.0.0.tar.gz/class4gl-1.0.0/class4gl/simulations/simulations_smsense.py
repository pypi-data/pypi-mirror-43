...
def execute(**kwargs):
    ...
    EXP_DEFS  =\
    {
      'BASE_SM_ORI':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False},
      'BASE_SM_LOW':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False},
      'BASE_SM_HIG':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False},
      'BASE_SM_Q90':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False},
      'BASE_SM_Q10':{'sw_ac' : ['adv',],'sw_ap': True,'sw_lit': False},
      'BASE_SM_Q50':{'sw_ac' : [],'sw_ap': True,'sw_lit': False},
    ...
    for iexpname,expid in enumerate(experiments):
        ...
        for istation,current_station in run_stations.iterrows():
            ...
            for imonth,month in enumerate(months):
                ...
                wg_q90.append(dat[dat.wg >= dat.wg.quantile(0.90)].wg.median())
                wg_q10.append(dat[dat.wg <=  dat.wg.quantile(0.10)].wg.median())
                ...
                # wg_q90.append(records_morning_station[select_season].wg.quantile(0.90))
            ...
            if not skip_chunk:
                ...
                for (STNID,chunk,index),record_morning in records_morning_station_chunk.iterrows():
                        ...
                        if expname == "BASE_SM_LOW": 
                            c4gli_morning.update(source='BASE_SM_LOW',pars={'wg' : wg_low})
                            c4gli_morning.update(source='BASE_SM_LOW',pars={'w2' : wg_low})
                        if expname == "BASE_SM_HIG": 
                            c4gli_morning.update(source='BASE_SM_HIG',pars={'wg' : wg_hig})
                            c4gli_morning.update(source='BASE_SM_HIG',pars={'w2' : wg_hig})

                        month = c4gli_morning.pars.datetime.month
                        imonth = months.index(month)
                        if expname == "BASE_SM_Q10": 
                            c4gli_morning.update(source='BASE_SM_Q10',pars={'wg' : wg_q10[imonth]})
                            c4gli_morning.update(source='BASE_SM_Q10',pars={'w2' : wg_q10[imonth]})

                        if expname == "BASE_SM_Q90": 
                            c4gli_morning.update(source='BASE_SM_Q90',pars={'wg' : wg_q90[imonth]})
                            c4gli_morning.update(source='BASE_SM_Q90',pars={'w2' : wg_q90[imonth]})
    
                        if expname == "BASE_SM_Q50": 
                            c4gli_morning.update(source='BASE_SM_Q50',pars={'wg' : wg_q50[imonth]})
                            c4gli_morning.update(source='BASE_SM_Q50',pars={'w2' : wg_q50[imonth]})
