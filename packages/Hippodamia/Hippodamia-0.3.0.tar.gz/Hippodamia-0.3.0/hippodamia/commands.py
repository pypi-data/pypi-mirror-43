 ...
 - show all service that run on devices with the name 'Thermostat': HIERARCHICAL DEVICE Thermostat"""
    ...
    def _print_stats(self, gid, type):
        ...
        if type == StatType.PROPERTIES or type == StatType.ALL:
            output += "properties: @{}\n".format(stats["properties"]["timestamp"])
            output += pprint.pformat(stats["properties"]["entries"], indent=4)
            output += "\n\n"

        if type == StatType.PING or type == StatType.ALL:
            output += "ping: #{} @{}\n".format(stats["ping"]["counter"], stats["ping"]["timestamp"])
            output += pprint.pformat(stats["ping"]["message"], indent=4)
            output += "\n\n"

        if type == StatType.RUNTIME or type == StatType.ALL:
            output += "runtime: #{} @{}\n".format(stats["runtime"]["counter"], stats["runtime"]["timestamp"])
            output += pprint.pformat(stats["runtime"]["message"], indent=4)
            output += "\n\n"

        if type == StatType.CONFIG or type == StatType.ALL:
            output += "config: #{} @{}\n".format(stats["config"]["counter"], stats["config"]["timestamp"])
            output += pprint.pformat(stats["config"]["message"], indent=4)
            output += "\n\n"

        if type == StatType.REQUEST or type == StatType.ALL:
            output += "request: #{} @{}\n".format(stats["request"]["counter"], stats["request"]["timestamp"])
            output += pprint.pformat(stats["request"]["message"], indent=4)
            output += "\n\n"
...
service: AGENTLOG GID 1 FLAT"""
    ...
    def do_a(self, arg):
        # """shortcut for 'stats gid hippodamia all'"""
        self._print_stats("hippodamia", StatType.ALL)

    def do_g(self, arg):
        # """shortcut for 'stats gid hippodamia general'"""
        self._print_stats("hippodamia", StatType.GENERAL)
