...
COMMAND_MAP = {
    ...
    'vvane': {'uid': 5, 'values': {'auto/stop': 0, 'swing': 10, "manual1": 1, "manual2": 2, "manual3": 3, "manual4": 4,
                                   "manual5": 5}},
    'hvane': {'uid': 6, 'values': {'auto/stop': 0, 'swing': 10, "manual1": 1, "manual2": 2, "manual3": 3, "manual4": 4,
                                   "manual5": 5}},
    'setpoint': {'uid': 9}
}
...
class IntesisHome(asyncio.Protocol):
        ...
        if data != '':
            ...
            for line in lines:
                ...
                if jsonResponse['command'] == "connect_rsp":
                    ...
                    # Value has changed
                    self._update_device_state(jsonResponse['data']['deviceId'], jsonResponse['data']['uid'],
        ...
        if self._connectionStatus == API_DISCONNECTED:
            ...
            try:
                if self._cmdServer and self._cmdServerPort and self._token:
                    ...
                    _LOGGER.error('Failed to retrieve the IntesisHome API host address, port or authentication token')
    ...
    def _set_mode(self, deviceId, mode):
        ...
        if mode in COMMAND_MAP['mode']['values']:
            self._set_value( deviceId, COMMAND_MAP['mode']['uid'], COMMAND_MAP['mode']['values'][mode])
    ...
    def set_fan_speed(self, deviceId, fan):
        ...
        self._set_value(deviceId, COMMAND_MAP['fan_speed']['uid'], COMMAND_MAP['fan_speed']['values'][fan])

    def _set_power(self, deviceId, power):
        """Internal method to set power on or off"""
        self._set_value(deviceId, COMMAND_MAP['power']['uid'], COMMAND_MAP['power']['values'][power])

    def set_vertical_vane(self, deviceId, vane):
        """Public method to set the vertical vane"""
        self._set_value( deviceId, COMMAND_MAP['vvane']['uid'], COMMAND_MAP['vvane']['values'][vane])

    def set_horizontal_vane(self, deviceId, vane):
        """Public method to set the horizontal vane"""
        self._set_value( deviceId, COMMAND_MAP['hvane']['uid'], COMMAND_MAP['hvane']['values'][vane])
