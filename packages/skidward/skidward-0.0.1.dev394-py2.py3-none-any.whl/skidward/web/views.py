    ...
    def on_model_change(self, form, model, is_created):
        if model.enabled:
            try:
                ...
                flash("Cron String not valid : {}".format(e), category="warning")
                abort(redirect(request.url))
            if not model.context:
                flash("Context needed to enable a task", category="warning")
                abort(redirect(request.url))
...
class LiveLogView(SkidwardView):
    ...
    def get_logs(self, job_id):
        ...
        if not logging_key:
            flash("No logging key for Redis was set.")
