...
def start_workers_main(runtime, datadir):
    ...
    # Split the cpu to workers.
    cpu_count = multiprocessing.cpu_count()
    if cpu_count >= worker_count:
        cpu_per_worker = cpu_count // worker_count
    else:
        cpu_per_worker = 0
    ...
    for worker_uuid in registered_workers.keys():
        ...
        available_cpus = None
        for i in range(0, cpu_per_worker):
            if available_cpus is None:
                available_cpus = str(i+cpu_index)
            else:
                available_cpus += ("," + str(i+cpu_index))
            cpu_index += 1

        assert(cpu_index <= cpu_count)
        worker_config["gpu_id"] = gpu_id
        worker_config["available_cpus"] = available_cpus
...
def main(debug_config=None):
    ...
    if not worker_registered:
        success = register(runtime, metadir, workdir, sharing, secrete)
