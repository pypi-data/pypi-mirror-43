...
class Worker(object):
    ...
    # return {success}, {uuid}
    @classmethod
    def register(cls, master_endpoint, worker_type="gpu", secrete=None):
    ...
    def poll(self, is_poll_no_task: bool = False, simulate_job_response: Dict[str, Any] = None):
        ...
        if simulate_job_response is not None:
            simulate_config = simulate_job_response.get("config", None)
            if simulate_config is not None:
                simulate_job_response["config"] = str(simulate_config).replace("'", '"')
        ...
        if status_code == 202:
            self.worker_logger.log_info(
                "poll",
                "no task is ready for the worker")
        ...
        # Server indicates that a task is ready for the worker.
            ...
            try:
                ...
                response_body["config"] = json.loads(response_body["config"])
                task = TrainingTask(**response_body.get("config"))
                    ...
                    - The file layout looks like:
                        ...
                        /runtime  <= CWD
                            /venv <= 'workdir/VENV' folder mounted as '/runtime/venv' [Read-Write]
                                ...
                                /lib
                                    <user_packages>
                            /code <= 'workdir/code' folder mounted as '/runtime/code' [Read-Write]
                                user_code.py
                                main.py
                            worker_run.py
    ...
    def _execute_container(self, venv_python_path, env, container_config):
        ...
        if self.available_cpus is not None:
            container_config["cpuset_cpus"] = self.available_cpus
        docker_client = docker.from_env()
