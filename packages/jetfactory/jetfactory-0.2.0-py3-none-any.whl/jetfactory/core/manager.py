...
from jetfactory.db import Database
...
class JetManager:
    ...
    def _load_packages(self, pkg_modules: list):
        """Takes a list of packages, ensures sanity and registers with manager"""

        for assigned_path, module in pkg_modules:
            if not hasattr(module, 'export'):
                raise Exception(f'Invalid package module {module}: pkg attribute missing')
    ...
    async def _register_controllers(self):
        ...
        for pkg_name, pkg in self.pkgs:
            ...
            for handler, route in ctrl.registry:
    ...
    async def attach(self, app, loop):
        ...
        :param app: Instance of jetfactory.Application
