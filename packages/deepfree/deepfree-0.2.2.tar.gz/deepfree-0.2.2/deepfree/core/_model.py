...
class Model(Train,Evaluate,Result):
    def __init__(self,**kwargs):
        ...
        # 创建唯一变量
        self.dropout = PHVariable(1,unique = True)('dropout')
        self.batch_normalization = PHVariable(1,dtype = tf.bool,unique = True)('batch_normalization')
    ...
    def next_activation(self):
        if type(self.hidden_func) == list:
            activation = self.hidden_func[self.activation_cnt]
            self.activation_cnt = np.mod(self.activation_cnt + 1, len(self.hidden_func))
            return activation
        else:
