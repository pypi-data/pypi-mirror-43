...
class License:
    ...
    def __init__(self, **kwargs):
        # license支持的授权项配置
        # self.profiles = kwargs['profiles'] if 'profiles' in kwargs else dict()
        # 发证机构代码，一般在子类中修改
    ...
    def parse(self, text):
        ...
        for item in domains_text:
            ...
            if domain in self.profiles:
                ...
                self.json[domain] = lic.json

        return self

    def update(self, node, owner, json, my_lic):
        """
        ...
        for domain, lic in sorted(json.items()):
            if domain in self.profiles and domain in my_lic.parser_dic:
                p = self.parser_dic[domain]

                # 将授权的资源正是授权者自己所属的资源，即平级授权，兄弟授权
                is_brother = node == my_lic.node
                p.update(lic, my_lic.parser_dic[domain], is_brother)

                self.json[domain] = p.json
    ...
    def signed(self, resource, owner, nonce):
        ...
        rdpool.rds.set(key, lic_token, self._idle_time)

        sign = tools.gen_md5(self.text + self.node + self.owner + nonce + lic_token)

        expired = time.second() + self._timeout
    ...
    def verify(self, *auth_args):
        ...
        dynamic_sign = tools.gen_md5(text + self.node + self.owner + nonce + lic_token)
    ...
    def operable(self, cur_resource, *actions):
        ...
        :param actions: 操作列表，格式：DOMAIN.ACTION，如org.create, catalog.read...
        """
...
class Parser:
    ...
    def __init__(self, profile):
        ...
        # 开关类
        self.SWITCH = profile['switches'] if 'switches' in profile else []
        # 数值类
        self.NUMBER = profile['numbers'] if 'numbers' in profile else []
        # 数值范围类
        self.RANGE = profile['ranges'] if 'ranges' in profile else []
    ...
    def update(self, json, operator_license, is_brother):
        fields = self.text.split(';')
        ...
        if sw_obj is not None and sw_len > 0:
            ...
            for i, k in enumerate(self.SWITCH):
                if k != 'NA':
                    # 给兄弟主体授权时，不能超过自己的权限范围
                    if is_brother and sw_obj[k]:
                        try:
                            operator_license.operable(k)
                ...
                if k != 'NA':
                    ...
                    if vals is not None:
                        if len(vals) == 2:
                            # 给兄弟主体授权时，不能超过自己的权限范围
                            if is_brother:
                                self_range = operator_license.range(k)
