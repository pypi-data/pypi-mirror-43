import numpy as np
import pandas as pd
from .common import get_simple_binary_image, get_shape_features, get_position_features, get_intensity_features, get_texture_features, get_network_features
...
def get_features(img, extra_features=[]):
    ...
    for pid, prop in enumerate(props):
        ...
        if "skeleton" in extra_features:
            fextra = get_network_features(seg=comp)
            features = {**features, **fextra}
    ...
    if ncomps == 0:
        ...
        if "intensity" in extra_features:
            ...
            features = {**features, **fextra}
        if "skeleton" in extra_features:
            fextra = get_network_features(seg=img)
            features = {**features, **fextra}
        if "texture" in extra_features:
