...
class _AWS(object):
    ...
    def get_obj_key(self, path, md5=False, ignore_prefix=False, user_prefix=None):
        """Get file object key"""
        if ignore_prefix is True or self.ignore_prefix:
            key = path
        else:
            prefix = user_prefix or self._prefix or ""
    ...
    def upload_files(self, path, add_checksum=False, progress=False, encryption_key=None, prefix=None):
        ...
        for item in get_files():
            ...
            key = self.get_obj_key(itemname, user_prefix=prefix)
