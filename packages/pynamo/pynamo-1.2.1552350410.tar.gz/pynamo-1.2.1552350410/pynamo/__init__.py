...
class Pynamo(object):
    ...
    def get_item(self, table_name, key, **kwargs):
        ...
        response = deepcopy(self.__default_response)
        kwargs['TableName'] = '%s%s' % (self.__table_prefix, table_name)
        convert_key_response = self.__convert_key(table_name, key)
        if convert_key_response['status'] != 200:
            return convert_key_response
        kwargs['Key'] = convert_key_response['data']
    ...
    def replace_item(self, table_name, original_key, new_item):
        ...
        # Get table keys
        orig_key_response = self.__convert_key(table_name, original_key)
        if orig_key_response['status'] != 200:
            return self.__return(self.__client_error(orig_key_response['error_msg'], response, table_name))
        orig_key = orig_key_response['data']

        new_key_response = self.__convert_key(table_name, new_item)
        if new_key_response['status'] != 200:
            return self.__return(self.__client_error(new_key_response['error_msg'], response, table_name))
        new_key = new_key_response['data']  
...
####################################################################################################
    ...
    def __convert_key(self, table_name, key):
        ...
        response['data'] = self._dic_dyno(final_key)
        return response
