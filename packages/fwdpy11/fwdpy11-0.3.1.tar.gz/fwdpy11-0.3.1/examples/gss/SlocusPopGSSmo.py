...
class Recorder(object):
    ...
    def __call__(self, pop, recorder):
        ...
        if self.interval > 0 and pop.generation >= 10*pop.N:
            # dmd = np.array(pop.diploid_metadata, copy=False)
            # tables, imap = fwdpy11.ts.simplify(pop, dmd['nodes'].flatten())
            # mc = fwdpy11.ts.count_mutations(tables, pop.mutations,
            #                                 [i for i in range(2*pop.N)])
            if pop.generation % 100 == 0.0:
                print(pop.generation,len(pop.fixations),len(pop.tables.edges),len(pop.tables.preserved_nodes))
            if pop.generation <= 12*pop.N and pop.generation % self.interval == 0.0:
                if self.nsam < pop.N:
...
def runsim(args):
    ...
    sregions = [fwdpy11.GaussianS(i[0]+5,i[0]+6,1,0.25) for i in locus_boundaries]
    recregions = [fwdpy11.PoissonInterval(*i,args.rho/(4*args.popsize)) for i in locus_boundaries]
    recregions.extend([fwdpy11.BinomialPoint(i[1],0.5) for i in locus_boundaries[:-1]])
    print([(i[1],0.5) for i in locus_boundaries[:-1]])
    ...
    #popsizes = [args.popsize]*int(args.time*float(args.popsize))
    print(len(popsizes))
    ...
    p['demography'] = np.array(
        [args.popsize]*(1+int(args.time*float(args.popsize))), dtype=np.uint32)
    params = fp11.model_params.ModelParams(**p)
    print(params.demography.dtype,len(params.demography))
    fwdpy11.wright_fisher_ts.evolve(
        rng, pop, params, 100, r, suppress_table_indexing=False,
        track_mutation_counts=True)
    print(len(pop.fixations),"fixations")
    print(pop.fixation_times)
