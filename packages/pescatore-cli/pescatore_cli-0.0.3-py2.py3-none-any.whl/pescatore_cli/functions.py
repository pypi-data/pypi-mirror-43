...
from .api.esche import (list as list_esche, create as create_esca, read as read_esca)
from .api.uscite import create as create_uscita


def cerca_stringa_in_bacino(stringa, bacino, id_licenza=None, id_esca=None, note=None, intervallo=5):
    if settings.DEBUG:
    ...
    if id_licenza is None:
        ...
        licenza = read_licenza(id_licenza).get('data')[0]
    ...
    if id_esca is None:
        ...
        esca = read_esca(id_esca).get('data')[0]

    if esca is None:
        esca = create_esca(stringa=stringa).get('data')[0]

    if licenza is None:
        raise Exception('Unable to find licenza for bacino {}.'.format(bacino))
    if esca is None:
        raise Exception('Unable to find esca with stringa "{}".'.format(stringa))
