...
class NaturalLanguageUnderstanding(object):
    ...
    def fit(self, data_path):
        ...
        self.ml_intent.fit(sentences, domains, intents)
        ...
        ret = []
        for a, b, c in loop:
            user_action = self.forward(''.join(a))
            ret.append((
                b == user_action.domain,
                c == user_action.intent
            ))
        ...
        print(
            f'domain_accuracy: {domain_accuracy}\n'
            f'intent_accuracy: {intent_accuracy}\n'
            f'slot_accuracy: {slot_accuracy}'
        )
