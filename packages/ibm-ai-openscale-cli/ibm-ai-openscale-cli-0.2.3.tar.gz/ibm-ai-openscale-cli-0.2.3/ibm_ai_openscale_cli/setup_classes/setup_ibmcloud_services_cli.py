...
class SetupIBMCloudServicesCli(SetupIBMCloudServices):
    ...
    def __init__(self):
        ...
        executeCommand('bx login --apikey "{}" -a "{}"'.format(Args.args.apikey, Args.env_dict['api']))
        logger.debug('Target resource-group "%s"', Args.args.resource_group)
        executeCommand('bx target -g "{}"'.format(Args.args.resource_group))
    ...
    def setup_wml(self):
        ...
        if Args.args.wml is not None:
            return self._get_credentials(wml_params, None, Args.args.wml)
