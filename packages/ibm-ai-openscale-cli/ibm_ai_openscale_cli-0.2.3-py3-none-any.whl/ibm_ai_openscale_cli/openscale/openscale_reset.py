...
class OpenScaleReset(OpenScale):
    ...
    def reset_metrics(self):
        ...
        if OpenScale._database is None:
            logger.info('No database specified OR metrics for {} internal database instance cannot be reset'.format(Args.service_name))
        else:
            OpenScale._database.reset_metrics_tables(self._datamart_name)
    ...
    def reset_monitors(self):
        ...
        subscription_uids = OpenScale._client.data_mart.subscriptions.get_uids()
        for subscription_uid in subscription_uids:
            try:
                start = time.time()
                subscription = OpenScale._client.data_mart.subscriptions.get(subscription_uid)
        ...
        if OpenScale._database is None:
            logger.info('No database specified OR monitor-related tables for {} internal database instance cannot be reset'.format(Args.service_name))
        else:
            OpenScale._database.drop_metrics_tables(self._datamart_name)

    @retry(tries=5, delay=4, backoff=2)
    def delete_subscriptions(self):
        logger.info('Deleting datamart subscriptions ...')
        bindings_uids = OpenScale._client.data_mart.bindings.get_uids()
        # TODO get subscriptions from binding id and delete those specifically
        for binding_uid in bindings_uids:
            subscriptions_uids = OpenScale._client.data_mart.subscriptions.get_uids()
            for subscription_uid in subscriptions_uids:
                try:
                    start = time.time()
                    OpenScale._client.data_mart.subscriptions.delete(subscription_uid, binding_uid=binding_uid)
                    elapsed = time.time() - start
                    logger.debug('TIMER: data_mart.subscriptions.delete in {:.3f} seconds'.format(elapsed))
                except:
                    logger.warn('Datamart subscription with the given binding id could not be deleted')

    @retry(tries=5, delay=4, backoff=2)
    def delete_bindings(self):
        logger.info('Deleting datamart bindings ...')
        try:
            bindings_uids = OpenScale._client.data_mart.bindings.get_uids()
            for binding_uid in bindings_uids:
                start = time.time()
                OpenScale._client.data_mart.bindings.delete(binding_uid)
                elapsed = time.time() - start
                logger.debug('TIMER: data_mart.bindings.delete in {:.3f} seconds'.format(elapsed))
        except:
            logger.warn('Datamart bindings could not be deleted')

    @retry(tries=5, delay=4, backoff=2)
    def delete_datamart(self):
        logger.info('Deleting datamart ...')
        try:
            start = time.time()
            OpenScale._client.data_mart.delete()
            elapsed = time.time() - start
    ...
    def clean_database(self):
        ...
        if OpenScale._database is None:
            logger.info('No database specified OR table/schema for {} internal database instance cannot be removed'.format(Args.service_name))
        else:
            OpenScale._database.drop_existing_schema(self._datamart_name, self._keep_schema)
