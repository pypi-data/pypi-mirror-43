...
class WatsonMachineLearningEngine:
    ...
    def __init__(self, credentials):
        start = time.time()
        WatsonMachineLearningEngine._client = WatsonMachineLearningAPIClient(dict(credentials))
        elapsed = time.time() - start
        logger.info('Using Watson Machine Learning Python Client version: {}'.format(self._client.version))
        logger.debug('TIMER: Connect to WatsonMachineLearningAPIClient in {:.3f} seconds'.format(elapsed))
    ...
    def _create_pipeline(self, model_name, pipeline_metadata_file):
        ...
        elapsed = time.time() - start
        logger.debug('TIMER: WML repository.store_definition(pipeline) in {:.3f} seconds'.format(elapsed))
        ...
        for model in models['resources']:
            ...
            if model_name == model['entity']['name']:
                try:
                    ...
                    elapsed = time.time() - start
                    logger.debug('TIMER: WML repository.delete(model) in {:.3f} seconds'.format(elapsed))
    ...
    def _create_model(self, model_name, model_metadata_file):
        ...
        logger.debug('TIMER: WML repository.store_model in {:.3f} seconds'.format(elapsed))
        logger.info('Created new model {} successfully (guid: {})'.format(model_name, model_guid))
    ...
    def _list_all_models(self):
        ...
        elapsed = time.time() - start
        logger.debug('TIMER: WML repository.list_models in {:.3f} seconds'.format(elapsed))
    ...
    def _deploy_model(self, model_guid, deployment_name, deployment_description):
        ...
        logger.debug('TIMER: WML deployments.create in {:.3f} seconds'.format(elapsed))
        logger.info('Created new deployment {} successfully (guid: {})'.format(deployment_name, deployment_guid))
        ...
        for details in deployment_details['resources']:
            ...
            if deployment_name == details['entity']['name']:
                try:
                    ...
                    elapsed = time.time() - start
                    logger.debug('TIMER: WML deployments.delete in {:.3f} seconds'.format(elapsed))
                    logger.info('Deployment deleted successfully'.format(deployment_name))
                except:
    ...
    def _list_all_deployments(self):
        ...
        deployment_details = self._client.deployments.get_details()
        elapsed = time.time() - start
        logger.debug('TIMER: WML deployments.get_details in {:.3f} seconds'.format(elapsed))
        for details in deployment_details['resources']:
