...
class GetSources(unittest.TestCase):
	def test_longest_source(self):
		sess								= ebscopy.Session()
		res									= sess.search("HISTORY-MATCHING AND FORECASTING TIGHT/SHALE GAS CONDENSATE WELLS USING COMBINED ANALYTICAL, SEMI-ANALYTICAL, AND EMPIRICAL METHODS")				# 1284993 is the AN for "HISTORY-MATCHING AND FORECASTING TIGHT/SHALE GAS CONDENSATE WELLS USING COMBINED ANALYTICAL, SEMI-ANALYTICAL, AND EMPIRICAL METHODS" by "CLARKSON, C R; WILLIAMS-KOVACS, J D; QANBARI, F; BEHMANESH, H; HEIDARI SURESHJANI, M", in the pta database, which has two Sources in the record, and a duplicate in the BibEntity

		rec_0								= res.records_simple[0]
		self.assertIsInstance(rec_0["Sources"], list)
		self.assertIsInstance(rec_0["Source"], str)
		self.assertGreater(len(rec_0["Source"]), 0)
