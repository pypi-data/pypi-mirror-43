		...
		if self.stat_total_hits > 0:
			...
			for record in data["SearchResult"].get("Data", {}).get("Records", []):
				...
				for item in record.get("Items", []):
					if item["Group"] == "Src":
						...
						if re.search('&lt;', item["Data"]):
							sources.append(html.fromstring(html.fromstring(item["Data"]).text_content()).text_content())
						else:
							sources.append(item["Data"])
				...
				if len(sources) > 0:
					...
					simple_rec["Sources"]		= sources
					simple_rec["Source"]		= max(sources, key=len)
