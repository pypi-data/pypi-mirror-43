...
class Mask_RCNN(Application):
    def __init__(self, class_list_path = None, weights_path = None):
        ...
        if type(weights_path) == type(None):
            ...
            Application.__init__(self)
            class_list = open(class_list_path).readlines()
            self.load_model(class_list, weights_path)
    ...
    def load_model(self, class_list = None, weights_path = None):
    ...
    def predict(self, data_array = None, data_path = None, predict_classes = True, batch_size = 32, verbose = 0, steps = None, use_coco_config = False):
        if data_path :
            try:
