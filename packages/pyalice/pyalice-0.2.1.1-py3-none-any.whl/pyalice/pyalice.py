...
class _DateTime(_Entity):
    ...
    def __init__(self, j):
        ...
        dt = datetime.utcnow()
        self.year = self.__convert_date(dt.year, self.value.year, self.value.year_is_relative)
        self.month = self.__convert_date(dt.month, self.value.month, self.value.month_is_relative)
        self.day = self.__convert_date(dt.day, self.value.day, self.value.day_is_relative)
        self.hour = self.__convert_date(0, self.value.hour, self.value.hour_is_relative)
        self.minute = self.__convert_date(0, self.value.minute, self.value.minute_is_relative)
        self.dt_utc = datetime(self.year, self.month, self.day, self.hour, self.minute)
    
    def __convert_date(self, cur, val, relative):
        if relative == None:
            return cur
        elif relative == False:
            return val
