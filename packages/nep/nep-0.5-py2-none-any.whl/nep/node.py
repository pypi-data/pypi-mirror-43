...
class node:
    ...
    def __wait_kill(self):
        ...
        exit = False
        time.sleep(.1)
        conf = self.direct("127.0.0.1", "7002", "one2many") 
        exit_sub  = self.new_sub("/nep_node", conf)
        time.sleep(.1)
    ...
    def new_client(self, topic):
        ...
        s, port, ip  = nep.masterRegister(self.node_name, topic, master_ip = '127.0.0.1', master_port = 7000, socket = "client")
    ...
    def new_server(self, topic):
        ...
        s, port, ip  = nep.masterRegister(self.node_name, topic, master_ip = '127.0.0.1', master_port = 7000, socket = "server")
        if s:
