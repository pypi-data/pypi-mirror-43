...
class AWSS3Collector(AWSBaseAccountCollector):
    ...
    def _get_bucket_information(self, bucket):
        ...
        try:
            ...
            # region Check if bucket is public
            try:
                res = s3.get_bucket_policy_status(Bucket=bucket.name)

                public = res['PolicyStatus']['IsPublic']
            except ClientError as ex:
                rex = ex.response['Error']['Code']

                if 'NoSuchBucketPolicy' in rex:
                    public = False
                else:
                    raise
            # endregion

            return BucketInfo(website=website_enabled, tags=tags, public=public)
