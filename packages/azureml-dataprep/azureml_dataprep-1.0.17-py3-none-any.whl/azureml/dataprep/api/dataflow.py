...
class Dataflow:
    ...
    def get_profile(self, include_stype_counts: bool = False) -> DataProfile:
        """
        Requests the data profile which collects summary statistics on the data produced by the Dataflow.

        :param include_stype_counts: Whether to include checking if values look like some well know semantic types of information. For Example, "email address". Turning this on will slow gathering the profile.
        :return: The data profile.
        """
        include_stype_counts = bool(include_stype_counts)
        cacheIndex = int(include_stype_counts)
        if self._profileCache[cacheIndex] is not None:
            return self._profileCache[cacheIndex]
        self._raise_if_missing_secrets()
        # cache the profile in the Dataflow.
        self._profileCache[cacheIndex] = DataProfile._from_execution(self._engine_api, make_activity_reference(self), include_stype_counts)
        return self._profileCache[cacheIndex]
    ...
    def _path_to_get_files_block(path: FilePath, archive_options: ArchiveOptions = None) -> 'Dataflow':
        try:
            ...
            if isinstance(path, DataReference) or isinstance(path, AbstractDatastore) or isinstance(path, DataPath):
