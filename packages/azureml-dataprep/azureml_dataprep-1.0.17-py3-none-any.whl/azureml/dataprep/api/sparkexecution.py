...
from .engineapi.typedefinitions import (AnonymousBlockData, ExportScriptFormat,
                                        ...
                                        ExecutorType)
from .engineapi.api import EngineAPI, get_engine_api, get_host_secret
...
def _ensure_interactive_spark():
    ...
    if _interactive_spark is None:
        _interactive_spark = _InteractiveSparkExecutor(get_engine_api())
...
class _InteractiveSparkExecutor:
    class Handler(socketserver.BaseRequestHandler):
        def handle(self):
            from py4j.protocol import Py4JJavaError

            with self.request.makefile() as reader:
                module_details = json.loads(reader.readline())
                request_secret = module_details['host_secret']
                if request_secret != get_host_secret():
                    raise ValueError('Unauthorized')

                module = _import_loader(module_details['path'])
                operation = module_details['operation']
                with self.request.makefile('w') as writer:
                    try:
                        if operation == 'collect':
                            lariat_dataset = module.get_lariat_dataset(module_details['secrets'])
                            ...
                            module.run_dataflow(module_details['secrets'])
                            writer.write(json.dumps({'result': 'success'}))
                        else:
                            writer.write(json.dumps({'result': 'error', 'error': 'InvalidOperation'}))
                    except Py4JJavaError as e:
                            writer.write(json.dumps({'result': 'error', 'error': e.errmsg}))

    def __init__(self, engine_api: EngineAPI):
        self._server = socketserver.TCPServer(("localhost", 0), _InteractiveSparkExecutor.Handler)
        self._server_thread = threading.Thread(target=self._server.serve_forever)
        self._server_thread.daemon = True
        self._server_thread.start()
        engine_api.set_interactive_spark_server(self._server.server_address[1])
