...
## -- On s assure de stopper correctement Alfresco 
...
if [ $? -ne 0 ]; then
    echo "- Connexion a la base de donnee impossible"
    echo "- Fin du script"
    exit 1
fi
...
echo -e "\n- DEBUT DU TRAITEMENT VISANT A RE RECONSTRUIRE LES INDEXES ALFRESCO\n"
