...
class LocalAnalysisContext:
    ...
    def set_qc_status(
            self, status=QCStatus.PASS, comments='',
            entity=QCEntity.ANALYSIS, input_id=None
    ):
        """
    ...
    def download_session_qa_requirements(self, path):
        ...
        if os.path.exists(path):
            raise RuntimeError(
                'Cannot download session qa because {} already exists!'.format(path)
            )
        ...
        for file_description in self.__settings[input_id]:
            ...
            if file_filter_condition_name:
                if 'file_filter_condition_name' not in file_description:
                    ...
                    desc_ff_name = file_description['file_filter_condition_name'].replace(
                        'c_', '')
            ...
            if _should_include_file(
                    os.path.basename(
                        file_description['path']), fmodality, set(ftags),
    ...
    def upload_file(self, source_file_path, destination_path, modality=None, tags=None, file_info=None,
                    file_format=None, protocol=None, container_id=None, replace=None):
        """
        ...
        if file_info:
            logger.warning(
                'File information ignored (local execution): {}'.format(file_info))
        if file_format:
            logger.warning(
                'File information ignored (local execution): {}'.format(file_format))
        if container_id:
            logger.warning(
                'File information ignored (local execution): {}'.format(container_id))

        out_path = os.path.join(self.__out_folder, destination_path)
        mkdirs(os.path.dirname(out_path))
        logger.info('Uploading {!r} to {!r}'.format(
    ...
    def download_resource(self, resource_path, destination_path):
        ...
        self._download_resource(
            resource_path, destination_path, type='tool_resource')
    ...
    def _download_resource(self, path, destination_file_path, type='template'):
        ...
        if not self.__res_folder:  # Default keyword for external users
            ...
            logger.info('Mocking resources from directory: {}'.format(
                self.__res_folder))
            logger.info(
                'The selected mode ({}) is ignored in local execution.'.format(type))

        logger.info('Downloading resource {!r} to {!r}'.format(
            path, destination_file_path))

        mkdirs(os.path.dirname(destination_file_path))
        try:
            shutil.copyfile(os.path.join(self.__res_folder,
                                         path), destination_file_path)
...
class LocalFile:
    ...
    def __download_file(self, destination_file_path):
        ...
        """
        logger = logging.getLogger(__name__)
        logger.info('Downloading {!r} to {!r}'.format(
            self.name, destination_file_path))
        mkdirs(os.path.dirname(destination_file_path))
        ...
        if source_file.endswith('.zip') and unpack:
            ...
            with TemporaryDirectory() as temp_dir:
                ...
                with zipfile.ZipFile(temp_path, 'r') as zip_ref:
                    ...
                    logger.info('Decompressing {!r} to {!r}'.format(
                        file_list, dest_path))
                    mkdirs(dest_path)
