...
def need_upgrade(config, ec):
    ...
    ec_ebs = next(v for v in list(ec.volumes.all()) if v.volume_type not in ['standard', 'gp2'])
    ...
    if to_upgrade:
        with click.progressbar(to_upgrade, label='Upgrading ec2:'.ljust(25)) as bar:
            for config, ec, instance_type, ebs in bar:
                ...
                if ebs:
                    try:
                        vol = next(v for v in list(ec.volumes.all()) if v.volume_type not in ['standard', 'gp2'])
