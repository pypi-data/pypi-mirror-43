...
class Transaction(SerializableMixin, HashableMixin):
    ...
    def serialize(self):
        ...
        if not (self.version == 1 and len(self.inputs) == 1 and self.inputs[0].is_coinbase):
    ...
    def deserialize(cls, file_):
        ...
        if not (initargs['version'] == 1 and len(initargs['inputs']) == 1 and initargs['inputs'][0].is_coinbase):
    ...
    def __repr__(self):
        if self.version == 1 and len(self.inputs) == 1 and self.inputs[0].is_coinbase:
            lock_height_str = ''
        else:
