...
class InceptionV3(PretrainedModelMixin, chainer.Chain):
	...
	def init_layers(self, n_classes):
		...
		pool_args = dict(pool_type="avg", ksize=3, stride=1, pad=1)
		...
		pool_args = dict(pool_type="max", ksize=3, stride=2, pad=0)
		# input 288 x 35 x 35
	...
	def loss(self, pred, gt):
		pred0, aux_pred = pred
		aux_loss = 0 if aux_pred is None else F.softmax_cross_entropy(aux_pred, gt)
		loss = F.softmax_cross_entropy(pred0, gt)
