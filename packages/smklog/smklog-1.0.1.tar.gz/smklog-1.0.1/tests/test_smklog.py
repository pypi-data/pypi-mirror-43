...
class TestSmkLog(unittest.TestCase):
    ...
    def test_smklog(self):
        ...
        self.assertEqual(len(json.loads(msg1)), 9,
                         "all fields should be included.")
        self.assertEqual(json.loads(msg1)["smklog.data"], data,
                         "Chinese characters should keep original, not escaped")
        self.assertFalse("smklog.store_id" in json.loads(msg1),
                         "store_id is optional.")
        self.assertFalse("smklog.request_id" in json.loads(msg1),
                         "request_id is also optional")

        msg2 = logger.info(source, category, label, message, data, timing, user,
                           store_id, request_id)

        self.assertEqual(len(json.loads(msg2)), 11,
                         "all fields should be included, including store_id "
                         "and request_id.")

        self.assertTrue("smklog.store_id" in json.loads(msg2),
                        "we passed store_id field")
        self.assertTrue("smklog.request_id" in json.loads(msg2),
        ...
        msg3 = logger.info(source, category, label, message, data_json, user,
                           store_id, request_id)
        self.assertEqual(json.loads(msg3)["smklog.data"], '{"key":"value","键":"值"}',
