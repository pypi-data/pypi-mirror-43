...
class BottleMongoQueryBuilder(object):
    ...
    def __init__(self, request: BaseRequest = None, schema: Schema = None):
        ...
        self.page = self._get_page(self._bottle_params.get(PAGE_KEY, 1))

        self.limit = self._get_limit(self._bottle_params.get(LIMIT_KEY, 20))
    ...
    def validate_filter_and_sort(self):
        ...
        for field in self.sort:
            if field not in valid_query_fields:
                raise ValidationError(
                    "Sort field: '{}' is not a valid sort field!".format(field))
...
class MongoDBManager(object):
    ...
    def update(self, condition: dict = None, changed_value: dict = None):
        """
        Update the changed value and also update the udpated_at field with default current time 
        """
        changed_value.update(
            {'updated_at': datetime.now().astimezone(timezone.utc)})
        changed_value = {'$set': changed_value}
        self.collection.update_many(condition, changed_value)
    ...
    def filter(self, query: Union[MongoQuery, Dict] = None, soft_delete: bool = True) -> (Cursor, int):
        ...
        limit = query.pop('limit', 20)  # TODO need to get from setting file
        query.pop('page', 1)
    ...
    def delete(self, condition: dict = None, soft_delete: bool = True):
        """
    ...
    def create(self, data: Union[List[Dict], Dict] = None, soft_delete: bool = True) -> (list, dict):
        ...
        for obj in data:
            ...
            # generate default id field in mongo with UUID value
            if '_id' not in obj:
                obj['_id'] = obj.pop('id', uuid.uuid4())
...
class DefaultMongoDBManager(MongoDBManager):
    DB_CONFIG = DefaultSettings.mongo_db_config()
...
if __name__ == '__main__':
    config = DefaultSettings.mongo_db_config()
    # print(config)
