...
from locust.core import HttpLocust, TaskSet
from . import fc
import sys
import logging
...
class RequesterKeywords(RequestsKeywords):
    ...
    @keyword("Locust Get")
    def locust_get(self, host, api, params=None, config={'time':10, 'user':1, 'increase':1}, callback=None, *args):
        f_handler=open('locust.log', 'w')
        sys.stdout=f_handler
        l = HttpLocust
        l.host = host
        l.config = config
        if 'min_wait' in config:
            l.min_wait = int(config['min_wait'])
        if 'max_wait' in config:
            l.max_wait = int(config['max_wait'])
        # if 'time' in config:
        #     l.stop_timeout = int(config['time'])
        l.task_set = TaskSet
        if callable is not None:
            l.task_set.tasks = [fc.makeMultiLineLambda('''
            lambda l:
                from robot.libraries.BuiltIn import BuiltIn
                args = %s
                with l.client.get('%s', catch_response=True) as response:
                    array = []
                    for item in args:
                        if item == 'locust_response':
                            item = response
                        array.append(item)
                    BuiltIn().run_keyword('%s', *tuple(array))
            ''' % (str(args), api, callback))]
        else:
            l.task_set.tasks = [fc.makeMultiLineLambda('''
            lambda l:
                l.client.get('%s')
