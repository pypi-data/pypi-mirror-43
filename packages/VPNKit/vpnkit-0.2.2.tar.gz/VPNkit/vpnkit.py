...
def start_openvpn():
    ...
    s = subprocess.Popen(
        ['ss-local', '-s', servername, '-p', '443', '-k', password, '-m', 'aes-256-cfb', '-l', '1080', '-t',
...
def get_ss_pass():
    password = ''
    with open("/usr/share/vpnkit/client.conf") as userfile:
        lines = userfile.readlines()

    for line in lines:
        if 'Shadowsocks password' in line:
            p, ps, password = line.split(' ')
