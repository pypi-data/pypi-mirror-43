...
def learner_constructor(experiment, original, *args, **kwargs):
    ...
    try:
        ...
        LOGGER.error("Failed to run Learner.__init__ logger",
                     exc_info=True)
...
def callback_on_epoch_begin(experiment, original, retval, *args, **kwargs):
    if experiment.auto_metric_logging:
        try:
            ...
            LOGGER.error("Failed to run CallbackHander.on_epoch_begin logger",
                         exc_info=True)
    return retval
...
def callback_on_epoch_end(experiment, original, retval, *args, **kwargs):
    if experiment.auto_metric_logging:
        try:
            ...
            LOGGER.error("Failed to run CallbackHander.on_epoch_end logger",
                         exc_info=True)
    return retval
