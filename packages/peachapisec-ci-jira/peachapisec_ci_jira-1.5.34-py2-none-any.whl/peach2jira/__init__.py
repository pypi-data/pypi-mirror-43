...
class Jira2Peach:
    ...
    def _jiraJsonRequestGet(self, opts, url):
        ...
        response = requests.get(opts['url'] + url,
    ...
    def _jiraGetCustomFieldId(self, opts, bucket_name):
        response = requests.get(opts['url'] + "/rest/api/2/field",
            headers={'Content-Type':'application/json'},
            auth=(opts['user'], opts['pass']))
    ...
    def _jiraCreateIssue(self, opts):
        response = requests.post(opts['url']+ "/rest/api/2/issue",
            json=opts['issue'],
            headers={'Content-Type':'application/json'},
            auth=(opts['user'], opts['pass']))
    ...
    def _jiraBucketCheck(self, opts):
        ...
        bucket_hash = hashlib.md5(bucket).hexdigest()[0:8]
