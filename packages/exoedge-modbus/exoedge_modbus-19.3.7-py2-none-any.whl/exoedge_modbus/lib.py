...
class ExositeModbus(object):
    ...
    def evaluate_registers(cls, **kwargs):
        ...
        def eval_signed(decoder, count):
            ...
            if count == 1 and registers[0] <= RANGES['RANGE_INT_8']['high']:
                LOG.debug("using: decode_8bit_int")
                decoder.skip_bytes(1)
                payload = decoder.decode_8bit_int()
            elif count == 1:
        ...
        def eval_unsigned(decoder, count):
            ...
            if count == 1 and registers[0] <= RANGES['RANGE_UINT_8']['high']:
                LOG.debug("using: decode_8bit_uint")
                decoder.skip_bytes(1)
                payload = decoder.decode_8bit_uint()
            elif count == 1:
    ...
    def read_coils(self, eval_kwargs, no_decode=False):
        address = eval_kwargs.get('register_offset')
