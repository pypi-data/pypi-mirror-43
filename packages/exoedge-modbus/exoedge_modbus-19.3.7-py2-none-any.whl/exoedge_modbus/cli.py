...
def main():
    ...
    eval_kwargs = {
        ...
        'bitmask': bitmask,
        'no_decode': docopt_args.get('--no-decode'),
    }
    ...
    if docopt_args.get('read_coils'):
        print(c.read_coils(eval_kwargs))

    elif docopt_args.get('read_discrete_inputs'):
        print(c.read_discrete_inputs(eval_kwargs))

    elif docopt_args.get('read_input_registers'):
        print("read_input_registers kwargs: {}".format(eval_kwargs))
        print(c.read_input_registers(eval_kwargs))

    elif docopt_args.get('read_holding_registers'):
        print(c.read_holding_registers(eval_kwargs))

    # ####################################### #
    # WRITE
    # ####################################### #
    elif docopt_args.get('write_coil'):
        print(c.write_coil(address, value))

    elif docopt_args.get('write_coils'):
        print(c.write_coils(address, value))

    elif docopt_args.get('write_register'):
        print(c.write_register(address, value, eval_mode, byteorder, wordorder))

    elif docopt_args.get('write_registers'):
        print(c.write_registers(address, count, value, eval_mode, byteorder, wordorder))
