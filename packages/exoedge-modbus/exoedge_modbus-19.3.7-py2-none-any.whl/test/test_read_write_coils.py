...
import threading
import json
from test.synchronous_server import TestModbusTCPServer
...
class TestReadWriteCoils(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.server_thread = TestModbusTCPServer()
    ...
    def setUp(self):
        ...
        self.ConfigIO = ConfigIO(
            config_io=json.load(open(config_io_file, 'r')))
    ...
    def test_001_1_holding_coil(self):
        ...
        start = time.time()
        while not self.ConfigIO.has_channels() or time.time()-start < 1.0:
            time.sleep(0.1)
