...
model = str(config["device"]["model"])
display_width = EPaperConstants.model[model]["height"]
display_height = EPaperConstants.model[model]["width"]
topic_sub_image = config["device"]["topics-sub"]["image"]

stats_event = threading.Event()
stats_event.clear()
stats = MessageQueueStats(mqtt_client, config["device"]["topics-pub"]["message_queue_size"], stats_event)

logger.info("--- wiping screen at start-up -------------------------------------------------------")
print("--- wiping screen at start-up ---")
config["device"]["wipe-screen"]["every-nth-day"] = 0
config["device"]["wipe-screen"]["time"] = "00:00"
config["device"]["wipe-screen"]["at-start-up"] = True
...
config["device"]["wipe-screen"]["every-nth-day"] = 0
config["device"]["wipe-screen"]["time"] = "00:00"
config["device"]["wipe-screen"]["at-start-up"] = False
eps = EPaperSimple(config)
eps_thread = threading.Thread(target=eps.run)
eps_thread.start()
eps._is_started.wait()
...
config["device"]["wipe-screen"]["every-nth-day"] = 1
config["device"]["wipe-screen"]["time"] = target_time
config["device"]["wipe-screen"]["at-start-up"] = False
