def _add_property_object(schema, key, entries, description):
    if len(entries) > 0:
        ...
        schema["device"]["properties"][key] = prop
        schema["device"]["required"].append(key)
    return schema


def _add_properties(schema, properties):
    for k, v in properties.items():
        schema["device"]["properties"][k] = v
        schema["device"]["required"].append(k)
    return schema
...
def get_schema(type, driver_specific_properties, topics_pub, topics_sub, mqtt_translations):
    schema = {
                "device": {
                    "description": "Device configuration '{}'.".format(type),
                    "type": "object",
                    "properties": {
                        "name": {
