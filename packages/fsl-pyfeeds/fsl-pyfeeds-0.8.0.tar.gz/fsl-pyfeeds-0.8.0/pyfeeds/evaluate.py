    ...
    for benchmark in benchmarks:
        ...
        try:
            ...
            log.error('Error evaluating {}'.format(relbenchmark),
                      exc_info=True)
            results.appoend((relbenchmark, 'ERROR', rname, None, tolerance))
            finalResult = False
...
def evalHeader(testfile, benchmark, alldims=True):
    ...
    img1 = imageCache[testfile]
    img2 = imageCache[benchmark]
...
def evalHeaderRestrictDims(testfile, benchmark):
    """Evaluation routine which compares the header fields of two NIFTI
    ...
    Returns 0 if they all match, 1 otherwise.
    """
    return evalHeader(testfile, benchmark, alldims=False)


def evalImage(testfile, benchmark):
...
def evalNumericalText(testfile, benchmark):
...
def evalMD5(testfile, benchmark):
...
def evalVectorImage(testfile, benchmark):
...
def evalPolarCoordinateImageGroup(testfiles, benchmarks):
    ...
    testtheta  = imageCache[testtheta[ 0]].get_data()
    testphi    = imageCache[testphi[   0]].get_data()
    benchtheta = imageCache[benchtheta[0]].get_data()
    benchphi   = imageCache[benchphi[  0]].get_data()
    ...
    imageCache[name + '_test']      = testvec
    imageCache[name + '_benchmark'] = benchvec

    return evalVectorImage(name + '_test', name + '_benchmark')


def evalGiftiVertices(testfile, benchmark):
    """Compare the vertices of two GIFTI surfaces. Uses the :func:`cmpArrays`
    function.
    """

    surf1 = imageCache[testfile]
    surf2 = imageCache[benchmark]
