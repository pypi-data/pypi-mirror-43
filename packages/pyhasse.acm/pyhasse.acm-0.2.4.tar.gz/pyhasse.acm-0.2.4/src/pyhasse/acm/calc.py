...
class ACM(object):
    ...
    :param r: number of rows (of vertices in the Hasse diagram)
    :param k: number of attributes (syn.: indicators)
    ...
    def generate_setofpairs(self, objred, user_list):
        """ pairs of objects (xi1,xi2) and of attributes (qj1,qj2) are generated.
        ...
        :param objred: list of representative elements of equivalence classes
        ...
        for i1 in range(0, len(user_list)):
            iob1 = objred.index(user_list[i1])
            for i2 in range(i1 + 1, lu):
                iob2 = objred.index(user_list[i2])
                px.append((iob1, iob2))
    ...
    def calc_acm(self, px=[], pq=[]):
        ...
        # bool1: True if xi1 - xi2 > 0 with respect to qj1
        # bool2: True if xi1 - xi2 < 0 with respect to qj2
        # bool3: True if xi1 - xi2 < 0 with respect to qj1
        # bool4: True if xi1 - xi2 > 0 with respect to qj2
        for i in range(0, self.lpx):
            for j in range(0, self.lpq):
                ...
                iob2 = px[i][1]
                iatt2 = pq[j][1]
                bool1 = (float(self.dm[iob1][iatt1]) - float(self.dm[iob2][iatt1])) > 0
                bool2 = (float(self.dm[iob1][iatt2]) - float(self.dm[iob2][iatt2])) < 0
                bool3 = (float(self.dm[iob1][iatt1]) - float(self.dm[iob2][iatt1])) < 0
                bool4 = (float(self.dm[iob1][iatt2]) - float(self.dm[iob2][iatt2])) > 0
                if bool1 and bool2:
                    self.acm[i].append(1)
                elif bool3 and bool4:
                    self.acm[i].append(1)
                else:
                    self.acm[i].append(0)
        return self.acm
    ...
    def find_optimalpairs(self, px, pq):
        ...
        :userlist: The input by the user. Should be but must not be an antichain
