...
def encode_advanced_purchase(df, dptr_field='departuredate', sales_field='utctimestamp'):
    ...
    '''
    df[dptr_field] = pd.to_datetime(df[dptr_field], format='%Y-%m-%d %H:%M:%S').dt.tz_convert(None)
    df[sales_field] = pd.to_datetime(df[sales_field], format='%Y-%m-%d %H:%M:%S').dt.tz_convert(None)
