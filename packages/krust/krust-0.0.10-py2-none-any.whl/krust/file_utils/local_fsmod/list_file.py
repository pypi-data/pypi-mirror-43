...
def LS(root=None, pattern=None, parse_info=False, recursive=False, with_root=None, ignore_hidden=True, use_glob=None, **kwargs):
    ...
    if pattern:
        # For base_only case, use 2-tuple form for grep.
        fnames_to_match = [(full_fname, os.path.basename(full_fname)) if pattern_is_base_only else full_fname for full_fname in result]
...
def LS_GROUP(root=None, pattern=None, keys=None, exclude_keys=None, individual_info=False,
             recursive=True, with_root=None, ignore_hidden=True, use_glob=None, **kwargs):
    grep_res = LS_GREP(root=None, pattern=pattern, parse_info=True, recursive=recursive, with_root=with_root, ignore_hidden=ignore_hidden, use_glob=use_glob, **kwargs)

    group_paras = {}
    groups = {}
    for fname, info in grep_res:
        d = {k: info[k] for k in keys if k in info} if keys else info.copy()
        ...
        files = groups.setdefault(keystr, [])
        if individual_info:
