...
class GridCacheItem:
    ...
    def __init__(self, grid):
        ...
        self._measures = dict(
            (measure.measure_type_id, measure) for measure in grid.measures
        )
...
""".format(len(grids))
    ...
    def get_cache_key_from_operation(self, operation):
        ...
        Build a cache key based on the given operation object
        """
        if self.filetype == AccountingOperationUpload.SYNCHRONIZED_ACCOUNTING:
            key = (operation.company_id, self.upload.date, self.upload.id)
        else:
            key = (operation.company_id, self.upload.date)
        return key
    ...
    def get_cache_key_from_grid(self, grid):
        ...
        """
        if self.filetype == AccountingOperationUpload.SYNCHRONIZED_ACCOUNTING:
            key = (grid.company_id, grid.date, grid.upload_id)
        else:
    ...
    def _query_grid(self, operation):
        ...
        if self.filetype == AccountingOperationUpload.SYNCHRONIZED_ACCOUNTING:
            query = query.filter_by(upload_id=self.upload.id)
