...
class File(ABC):
    ...
    def update_cache(self, response, index=None):
        ...
        if file_id is None:
            ...
            new_res = _res.get(self.file_type, None)
            if new_res is None:
                new_res = _res.get(self.other_response_type)

            self.cache[self.file] = self._get_file_id(new_res)['file_id']
