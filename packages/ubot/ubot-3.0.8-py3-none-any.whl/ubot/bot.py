...
class Bot(object):
    ...
    def __init__(self, token, json_lib=None, max_handlers=1, loop=None):
        ...
        self.base_url = 'https://api.telegram.org:443/bot{}/'.format(token)

    async def api_request(self, method, endpoint, fields=None, headers=None):
        """
        Wraps the urllib3 request in a more user friendly way, making it async and premitting the base Telegram API url.
        NOTE: if you pass a Content-Length you should take care of converting the object to bytes
        ...
        if not fields:
            ...
            data = self.json.dumps(fields).encode()
    ...
    async def start(self):  # noqa: C901
        ...
        while True:
            ...
            for trigger in checked_triggers:
                ...
                method, url, args, fields, headers = endpoint

                _url = '{}?{}'.format(url, urlencode(args))
                response = await self.api_request(method, _url, fields, headers)
                ...
                try:
                    ...
                    when = ExceptionThrownWhen.executing_callback
                    await self.handle_exception(update, e, when)
                    continue
