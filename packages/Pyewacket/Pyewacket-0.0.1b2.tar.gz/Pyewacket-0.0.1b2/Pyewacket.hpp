...
// Pyewacket Special Functions
Integer randrange(Integer start, Integer stop, int step) {
    auto step_by = std::abs(step);
    if (step_by > 1) {
        const Integer width = std::max(start, stop) - std::min(start, stop);
        if (start < stop)
            return start + step_by * random_below((width + step_by - 1) / step_by);
        else return start - step_by * random_below((width + step_by - 1) / step_by);
    }
    else if (step_by == 1) {
        return start + random_below(stop - start);
    }
    else return start;
}
...
Float vonmisesvariate(Float mu, Float kappa) {
    ...
    if (u3 > 0.5) return std::fmod(mu + std::acos(f), TOU);
    else return std::fmod(mu - std::acos(f), TOU);
}
