...
_OPT_WIDGETS = {bool: (QCheckBox, QCheckBox.isChecked, QCheckBox.setChecked),
                str:  (QLineEdit, QLineEdit.text,      QLineEdit.setText)}


def option_widget(opt_type:type) -> QWidget:
    ...
    w.load = lambda val: setter(w, w_type(val))
...
class OptionPage(QFrame):
    ...
    def __init__(self, opt_dict:dict):
        ...
        for (name, opt) in opt_dict.items():
            w = self._widgets[name] = option_widget(type(opt.default))
    ...
    def load(self, page:dict) -> None:
        """ Send config values from a dict to each widget. """
        for (name, val) in page.items():
            if name in self._widgets:
                self._widgets[name].load(val)
...
class ConfigDialog(QDialog):
    ...
    def __init__(self, parent:QWidget, save_callback:Callable, cfg_info:Dict[str, dict]):
        """ Create UI elements and connect basic signals. """
        ...
        # Create all tabs and widgets using config info from the dict.
        self._pages = {}
        self.w_tabs.clear()
