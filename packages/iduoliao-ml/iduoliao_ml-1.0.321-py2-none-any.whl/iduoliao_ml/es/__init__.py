...
esTimeFormat = u"%Y-%m-%dT%H:%M:%S.%f+0800"
	        ...
	        },
	        {
	          "range": {
	            "create_time": {
	              ...
	              "lte": timeToEsTime(int(time.time()) - 90 if isTest else min(lastPersonasTime + 60 * 60, int(time.time()) - 90))
	            }
	          }
	        }
	      ]
...
def timeToEsTime(pTime, hasMs=False):
	return time.strftime(esTimeFormat if hasMs else esTimeFormatNoMs, time.localtime(pTime))

def esTimeToTime(esTime, hasMs=False):
	return time.mktime(time.strptime(esTime, esTimeFormat if hasMs else esTimeFormatNoMs))
