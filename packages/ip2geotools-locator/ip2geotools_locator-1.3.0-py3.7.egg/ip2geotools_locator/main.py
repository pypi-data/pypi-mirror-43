...
class Locator:
    ...
    def __init__(self, generate_map=True):
        ...
        logger.debug(
            "Settings parsed for commercial and noncommercial databases.")
    ...
    def get_locations(self, ip_address, databases=None):
        ...
        if self.noncommercial["host_ip"]["active"]:
            ...
            host_ip = HostIpDB()
            self.locations["HostIP"] = (host_ip.get_location(ip_address))

            # Add location to the map?
            if self.noncommercial["host_ip"]["generate_marker"]:
                logger.debug("""Folium marker for HostIP database is set as Active in settings.
                             Calling add_to_map method.""")
                host_ip.add_to_map()
        ...
        if self.noncommercial["ip_city"]["active"]:
            ...
            ip_city = IpCityDB()
            self.locations["DbIpCity"] = (ip_city.get_location(ip_address))

            if self.noncommercial["ip_city"]["generate_marker"]:
                logger.debug("""Folium marker for DbIpCity database is set Active in settings.
                             Calling add_to_map method.""")
                ip_city.add_to_map()

        # Json settings parsed for Ip2location database
        if self.noncommercial["ip2location"]["active"]:
            logger.debug("""database Ip2location is set as Active in settings. Gathering
                         location.""")
            ip2location = Ip2LocationDB(
                self.noncommercial["ip2location"]["db_file"])
            self.locations["Ip2location"] = (ip2location.get_location(ip_address))

            if self.noncommercial["ip2location"]["generate_marker"]:
                logger.debug("""Folium marker for Ip2location database is set as Active in
                             settings. Calling add_to_map method.""")
                ip2location.add_to_map()

        # Json settings parsed for Ipstack database
        if self.noncommercial["ipstack"]["active"]:
            logger.debug(
                "database Ipstack is set as Active in settings. Gathering location.")
            ipstack = IpstackDB(self.noncommercial["ipstack"]["api_key"])
            self.locations["Ipstack"] = (ipstack.get_location(ip_address))

            if self.noncommercial["ipstack"]["generate_marker"]:
                logger.debug("""Folium marker for Ipstack database is set as Active in settings.
                             Calling add_to_map method.""")
                ipstack.add_to_map()

        # Json settings parsed for MaxMind GeoLite2City database
        if self.noncommercial["max_mind_lite"]["active"]:
            logger.debug("""database MaxMind GeoLite2City is set as Active in settings.
                         Gathering location.""")
            max_mind_lite = MaxMindLiteDB(
                self.noncommercial["max_mind_lite"]["db_file"])
            self.locations["MaxMind_GeoLite2City"] = (max_mind_lite.get_location(ip_address))

            if self.noncommercial["max_mind_lite"]["generate_marker"]:
                logger.debug("""Folium marker for MaxMind GeoLite2City database is set as Active
                             in settings. Calling add_to_map method.""")
        ...
        if self.commercial["eurek"]["active"]:
            ...
            eurek = EurekDB()
            self.locations["Eurek"] = (eurek.get_location(ip_address))

            if self.commercial["eurek"]["generate_marker"]:
                logger.debug("""Folium marker for Eurek database is set as Active in settings.
                             Calling add_to_map method.""")
                eurek.add_to_map()

        # Json settings parsed for GeobytesCityDetails database
        if self.commercial["geobytes_city"]["active"]:
            logger.debug("""database GeobytesCityDetails is set as Active in settings. Gathering
                         location.""")
            geobytes_city = GeobytesCityDB()
            self.locations["GeobytesCityDetails"] = (geobytes_city.get_location(ip_address))

            if self.commercial["geobytes_city"]["generate_marker"]:
                logger.debug("""Folium marker for GeobytesCityDetails database is set as Active
                             in settings. Calling add_to_map method.""")
        ...
        if self.commercial["ip_info"]["active"]:
            ...
            self.locations["IP_Info"] = (ip_info.get_location(ip_address))

            if self.commercial["ip_info"]["generate_marker"]:
                logger.debug("""Folium marker for IP Info database is set as Active in settings.
                             Calling add_to_map method.""")
                ip_info.add_to_map()
        ...
        if self.commercial["ip_web"]["active"]:
            ...
            ip_web = IpWebDB()
            self.locations["DbIpWeb"] = (ip_web.get_location(ip_address))

            if self.commercial["ip_web"]["generate_marker"]:
                logger.debug("""Folium marker for DbIpWeb database is set as Active in settings.
                             Calling add_to_map method.""")
                ip_web.add_to_map()

        # Json settings parsed for DbIpWeb database
        if self.commercial["ip2location_web"]["active"]:
            logger.debug("database IP2Location Web is set as Active in settings. Gathering location.",
                    )
            ip2location_web = Ip2locationWebDB()
            self.locations["IP2Location Web"] = (ip2location_web.get_location(ip_address))
        ...
        if self.commercial["max_mind"]["active"]:
            logger.debug("""database MaxMindGeoIp2City is set as Active in settings. Gathering
                         location.""")
            max_mind = MaxMindDB()
            self.locations["MaxMindGeoIp2City"] = (max_mind.get_location(ip_address))

            if self.commercial["max_mind"]["generate_marker"]:
                logger.debug("""Folium marker for MaxMindGeoIp2City database is set as Active in
                             settings. Calling add_to_map method.""")
                max_mind.add_to_map()
        ...
        if self.commercial["neustar_web"]["active"]:
            ...
            self.locations["NeustarWeb"] = (neustar_web.get_location(ip_address))

            if self.commercial["neustar_web"]["generate_marker"]:
                logger.debug("""Folium marker for NeustarWeb database is set as Active in
                             settings. Calling add_to_map method.""")
        ...
        if self.commercial["skyhook"]["active"]:
            ...
            skyhook = SkyhookDB()
            self.locations["Skyhook"] = (skyhook.get_location(ip_address))

            if self.commercial["skyhook"]["generate_marker"]:
                logger.debug("""Folium marker for Skyhook database is set as Active in settings.
                             Calling add_to_map method.""")
                skyhook.add_to_map()
