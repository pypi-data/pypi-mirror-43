...
class MaxMindDB:
    ...
    def get_location(self, ip_address):
        ...
        try:
            ...
            logger.info("%s: DB returned location %.3f N, %.3f E", __name__,
                        self.__db_data.latitude, self.__db_data.longitude)
            ...
            logger.error("%s: Service is unavailable. ServiceError: %s ", __name__, str(exception))

        except LimitExceededError as exception:
            # Handling for LimitExceededError exception
            logger.warning("%s: LimitExceededError: %s ", __name__, str(exception))
            ...
            logger.warning("%s: DB returned invalid values. TypeError: %s", __name__,
                           str(exception))
    ...
    def add_to_map(self):
        ...
        """
        try:
            logger.debug("%s: Calling add_marker method for %s DB", __name__,
            ...
            logger.warning("%s: Cannot add empty marker %s ", __name__, str(exception))
