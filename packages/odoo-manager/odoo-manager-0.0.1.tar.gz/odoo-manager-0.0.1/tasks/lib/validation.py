...
def check_packages(ctx):
    ...
    :param ctx {invoke.context.Context}: Invoke context variable
    """
    check_executable(ctx, ['git', 'docker', 'docker-compose', 'docker', 'invoke', 'curl', 'echo', 'sed', 'pip3', 'tail', 'pylint', 'yapf'])
...
def check_project(ctx):
    ...
    must_have_files = ['.env',
                       '.container/config/odoo.conf',
                       '.container/log/odoo.log',
                       '.container/core/LICENSE', ]
    ...
    with open('.env', 'r') as f:
        if 'enterprise=1' in f.read().lower():
            if not os.path.isfile('.container/enterprise/LICENSE'):
                ctx.run('echo "It looks like you have enterprise enabled. Make sure you have the source files setup in '
