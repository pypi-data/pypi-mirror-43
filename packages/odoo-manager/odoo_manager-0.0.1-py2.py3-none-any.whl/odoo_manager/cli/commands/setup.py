...
class Setup(BaseCommand):
    ...
    def run_dependencies(self):
        ...
        :return {NoneType}:
        """
        options = {'--no-update': self.options.get('--no-update', False),
        ...
        if 'depends' in configuration:
            ...
            for dependency_name in dependencies:
                if not options['--no-update'] or (options['--no-update'] and not os.path.isdir(self.paths.base('_lib/{}'.format(dependency_name)))):
            ...
            if effected_modules:
                ...
                if options['--yes'] or str(input('Review any diffs above. Should we continue to overwrite? This will overwrite the _lib modules listed above with files '
