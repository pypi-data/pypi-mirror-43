...
UPDATE_MESSAGE = "Update is available for missinglink.\nTo install, please run:\n$ pip missinglink -U update"
...
def update_ml_version():

    latest_version = get_latest_pip_version(MissinglinkVersion.get_missinglink_package())
    current_version = MissinglinkVersion.get_missinglink_cli_version()
    # If in dev environment
    if current_version is None or current_version.startswith('0'):
        return False
...
def get_latest_pip_version(package=None):
...
@click.option('--skip-version-check', '-nvc', default=None, required=False)
@click.pass_context
def cli(ctx, output_format, config_prefix, config_file, log_level, skip_version_check):
    global __pre_call_hook
    ...
    ctx.obj.skip = skip_version_check

    if __pre_call_hook is not None:
        __pre_call_hook(ctx)


def check_skip_version(ctx):
    if ctx.obj.skip is None or ctx.obj.skip.lower() == 'y' or ctx.obj.skip.lower() == 'yes':
        return True
...
def process_result(ctx, result, **kwargs):
    if check_skip_version(ctx) and update_ml_version():
