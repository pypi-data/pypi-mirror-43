...
from atrader.tframe.udefs import CALLABLETYPE, INTEGERSTYPE
...
from . import convertor as regfunc_cvt
...
@apply_rule(verify_that('target_indices').is_instance_of(*INTEGERSTYPE, list, tuple))
def get_current_bar(target_indices=()):
    ...
    return regfunc_cvt.convert_current_bar_to_df(result)
...
@apply_rule(verify_that('reg_idx').is_instance_of(dict),
            ...
            verify_that('df').is_in((True, False)),
            verify_that('sort_by').is_in(('bpfactor', 'target_idx', 'date')))
def get_reg_factor(reg_idx,
    ...
    if df:
        data_df = regfunc_cvt.convert_reg_factor_to_df(dt, value, target_indices, factor).sort_values(
            sort_by, ascending=True, na_position='first')  # type: pd.DataFrame
        data_df.index = range(data_df.shape[0])
        return data_df
    else:
        data_dict = regfunc_cvt.convert_reg_factor_to_dict(dt, value, target_indices, factor)
        return data_dict
...
def get_reg_userdata(reg_idx,
    ...
    return regfunc_cvt.convert_reg_userdata_to_df(datetime_tl, value)
...
def get_reg_userindi(reg_idx,
    ...
    return regfunc_cvt.convert_user_indi_to_df(t, v)
...
def get_instruments(target_indices=(), length=1, df=False, sort_by_date=True):
    ...
    return regfunc_cvt.convert_instrument_matrix_to_output(target_list, res_matrix, df, sort_by_date)
