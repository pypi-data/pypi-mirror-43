...
from machinelearning.game.test.game import Board, Game
from machinelearning.game.test.mcts import MCTSPurePlayer, MCTSPlayer
from machinelearning.game.test.net.policy_value_net_keras import PolicyValueNet  # Keras
# from machinelearning.game.gomoku.net.policy_value_net import PolicyValueNet  # Theano and Lasagne
# from machinelearning.game.gomoku.net.policy_value_net_pytorch import PolicyValueNet  # Pytorch
# from machinelearning.game.gomoku.net.policy_value_net_tensorflow import PolicyValueNet # Tensorflow
...
class GomokuTrainPipeline():
    def __init__(self, init_model=None):
        ...
        self.game_batch_num = 1  # selfplay对战次数
        self.batch_size = 2  # data_buffer中对战次数超过n次后开始启动模型训练
        self.check_freq = 2  # 每对战n次检查一次当前模型vs旧模型胜率
        ...
        # 纯MCTS的模拟数，用于评估策略模型
        self.pure_mcts_playout_num = 10  # 用户纯MCTS构建初始树时的随机走子步数
    ...
    def policy_update(self):
        ...
        pickle.dump(state_batch, open(CUR_PATH+'/log/state_batch.data', 'wb'), protocol=2)
        pickle.dump(mcts_probs_batch, open(CUR_PATH+'/log/mcts_probs_batch.data', 'wb'), protocol=2)
        pickle.dump(winner_batch, open(CUR_PATH+'/log/winner_batch.data', 'wb'), protocol=2)
