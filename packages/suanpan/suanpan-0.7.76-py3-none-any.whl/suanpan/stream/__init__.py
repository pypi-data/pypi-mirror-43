...
class Handler(Component):
    ...
    def init(self, message):
        ...
        self.current = objects.Context(  # pylint: disable=attribute-defined-outside-init
            context=context
        )
...
class Stream(interfaces.HasBaseServices, interfaces.HasLogger, interfaces.HasDevMode):
    ...
    STREAM_ARGUMENTS = [
        ...
        BoolOrInt("stream-recv-queue-block", default=30000),
    ...
    def setCurrent(self, args):  # pylint: disable=unused-argument
        ...
        self.current = objects.Context(message=message, handler=handler)
        return self.current
    ...
    def send(self, results, queue=None, message=None, extra=None):
        outputs = self.current.handler.save(
            self.current.handler.current.context, results
        )
