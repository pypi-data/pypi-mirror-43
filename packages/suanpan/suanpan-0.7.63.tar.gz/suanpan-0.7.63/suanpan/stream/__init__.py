...
class Handler(Component):
    ...
    def run(self, steamObj, message, *arg, **kwargs):
        ...
        results = self.runFunc(steamObj, context)
...
class Stream(interfaces.HasBaseServices, interfaces.HasLogger, interfaces.HasDevMode):
    ...
    def streamCall(self, message):
        ...
        try:
            ...
            outputs = self.currentHandler(self, message) or {}
            endTime = time.time()
    ...
    def getHandler(self, message):
        ...
        if not handler or not isinstance(handler, Handler):
            raise Exception("Unknown stream handler: {}.{}".format(self.name, streamCall))
        return handler
    ...
    def send(self, results, queue=None, extra=None):
        ...
        return self.sendSuccessMessage(self.currentMessage, outputs, queue=queue, extra=extra)

    def sendError(self, msg, queue=None, extra=None):
        return self.sendFailureMessage(self.currentMessage, msg, queue=queue, extra=extra)
