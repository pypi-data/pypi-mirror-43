...
class Post(db.Model):
	...
	campain_id = db.Column(db.Integer, db.ForeignKey('campain.id'), nullable=True)
	campain = db.relationship('Campain', backref=db.backref('posts', lazy=False))
...
class Session(db.Model):
	id = db.Column(db.Integer, primary_key=True)
	token = db.Column(db.String(256), default=sha256((str(time.time()) + "sessSalt").encode("utf-8")).hexdigest())
...
class CampainRule(db.Model):
	...
	campain = db.relationship('Campain', backref=db.backref('campain_rules', lazy=True))
...
def new_user(username, password):
	...
	if user == None:
		newUser = User(username=username, credentials=sha256((username + password + "super secret salt").encode('utf-8')).hexdigest())
