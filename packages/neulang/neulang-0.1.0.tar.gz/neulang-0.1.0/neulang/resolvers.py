...
class Resolver(BaseResolver):
    ...
    def resolve(self, tree):
        ...
        if "resolver" in self._debug:
            pdb.set_trace()
        if not tree:
            raise ValueError("Unable to find any parseable content")
        nodes = []
    ...
    def add_regex_handler(self, data):
        # TODO: move to RegexResolver
        ignores = ["match", "body"]
        data = self._astir._node_to_list(data, ignore_first=ignores)
        if not len(data) == 2:
        ...
        body = [
            Expr(a_n) if not type(a_n) in self._astir._stmt_types else a_n
            for a_n in body
        ]
        rx_h = f"rx_{abs(hash(match))}"
        self._rx_hashes[match] = rx_h
        pifa = fix_missing_locations(Module([FunctionDef(rx_h, args, body, [], None)]))
        ns = {}
        pifc = compile(pifa, rx_h, "exec")
        exec(pifc, ns)
        pif = ns[rx_h]
        self._pifs[rx_h] = pif
        return None
