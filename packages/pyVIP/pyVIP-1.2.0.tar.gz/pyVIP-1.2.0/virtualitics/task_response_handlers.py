...
def generic_callback(response_bytes, payload, log_level, figsize):
    ...
    if (response["AuthStatus"] == "Success") and (response["VersionStatus"] == "Success"):
        ...
        for task_response in response["TaskResponses"]:
            if task_response["TaskStatus"] != "Success":
        ...
        if len(task_failures) > 0:
            i = 0
            while i < len(task_failures):
                if task_failures[i]['TaskStatus'] != 'Failed':
                    continue
                if 'Error' in task_failures[i]:
                    break
                i += 1
            task_type = task_failures[i]['TaskType']
            error = task_failures[i].get('Error') or '(unknown reason)'
            error = error.replace('\n', '').replace('<b>', '').replace('</b>', '').strip()
            if "Note" in task_failures[i]:
                note = task_failures[i]["Note"].replace('\n', '').replace('<b>', '').replace('</b>', '').strip()
                raise (exceptions.VipTaskExecutionException("Task '%s' failed because '%s' Note:"
                                                            "'%s'" % (task_type, error, note)))
            else:
                # also print 'Note' attribute if it exists
                raise (exceptions.VipTaskExecutionException("Task '%s' failed because '%s'" % (task_type, error)))
        if len(results) == 0:
            return None
        elif len(results) > 0:
...
def _plot_mapping_export_callback(task_response, payload, log_level):
    ...
    if "Pulse" in task_response.keys() and task_response["Pulse"] is not None:
        plot.pulse = task_response["Pulse"]
    ...
    if "ZNormalization" in task_response.keys() and task_response["ZNormalization"] is not None:
        plot.y_normalization = task_response["ZNormalization"]
...
def _export_callback(task_response, payload, figsize):
    ...
    resolution of the plots shown is controlled by the 'imsize' parameter in the function calls. The default is
    (16, 16).
    :return: None; displays the returned capture.
    """
...
def _ml_routine_callback(task_response, payload):
    ...
    """
    if "Note" in task_response.keys():
        print(task_response["Note"] + "\n")
    if "ReturnData" in task_response.keys():
        if task_response["ReturnData"]:
            start = task_response["BytesStartIndex"]
            size = task_response["BytesSize"]
