...
class VipPlot:
    ...
    def __init__(self, data_set_name=None, plot_type="scatter", x=None, y=None, z=None, color=None, size=None,
                 shape=None, transparency=None, halo=None, pulse=None, playback=None, arrow=None, groupby=None,
                 ...
                 hist_volume_by=None, x_bins=None, y_bins=None, z_bins=None, log_level=0, name=None):
        """
    ...
    def _get_dim_info(self):
        ...
        if self.pulse is not None:
            dim_info.append({"Dimension": "Pulsation", "Column": self.pulse})
        if self.playback is not None:
    ...
    def pulse(self):
        """
        Pulse dimension (Extra dimension). Should be set to :class:`str` of the feature name to be mapped to pulse
        dimension in VIP. If a :class:`pandas.Series` is passed, the `name` attribute will be used and must match a
        feature name in the specified dataset in VIP. Default is :class:`None` which implies that pulse dimension will
        not be mapped.
    ...
    def pulse(self, val):
        ...
        if not self.is_spatial_dimension_used():
            utils.raise_invalid_argument_exception(str(type(val)), "pulse",
                                                   "One of the spatial dimensions (X, Y, or Z) must be mapped "
                                                   "before the other dimensions can be used.")
        self._pulse = utils.get_name(val)
        if self.log_level >= api.LOG_DEBUG_LEVEL:
            print("'pulse' set to '{}'".format(self._pulse))

    @pulse.deleter
    def pulse(self):
        """
        Sets pulse dimension to :class:`None` which implies that the pulse dimension will not be mapped.

        :return: :class:`None`
        """
        if self.log_level >= api.LOG_HELP_LEVEL:
            print("Attribute 'pulse' is set to 'None'; 'None' value implies that nothing is to be mapped to the "
                  "pulse dimension. ")
        self.pulse = None
    ...
    def z_scale(self, val):
        ...
        if self.is_geospatial_plot():
            utils.raise_invalid_argument_exception(str(type(val)), "z_scale",
                                                   "'z_scale' is not applicable for geospatial plots. ")
    ...
    def color_type(self, val):
        ...
        if self._plot_type == "LINE_PLOT" and val not in ["COLOR_BIN", "COLOR_PALETTE"]:
            utils.raise_invalid_argument_exception(str(type(val)), "color_type",
                                                   "when 'plot_type' is 'LINE_PLOT', 'color_type' must "
                                                   "be set to 'COLOR_BIN' or 'COLOR_PALETTE'")
    ...
    def x_bins(self):
        """
        X bins setting. Must be a :class:`int` between 1 and 250. X bins is only applicable if the 'plot_type' has
        ...
        :return: :class:`int` between 1 and 250
        """
        return self._x_bins
    ...
    def x_bins(self, val):
        ...
        :param val: :class:`int` between 1 and 250
        :return: :class:`None`
        """
        if val is None:
            self._x_bins = None
        ...
        if not isinstance(val, int) or not (1 <= val <= 250):
            utils.raise_invalid_argument_exception(str(type(val)), "x_bins",
                                                   "must be `int` and between 1 and 250. ")
    ...
    def y_bins(self):
        """
        Y bins setting. Must be a :class:`int` between 1 and 250. Y bins is only applicable if the 'plot_type' has
        been set to "HISTOGRAM". If there are multiple spatial dimensions (X, Y, Z) mapped, there are additional
        constraints for permissible values.
    ...
    def z_bins(self):
        """
        Z bins setting. Must be a :class:`int` between 1 and 250. Z bins is only applicable if the 'plot_type' has
        been set to "HISTOGRAM". If there are multiple spatial dimensions (X, Y, Z) mapped, there are additional
        constraints for permissible values.
    ...
    def show_points(self):
        ...
        is set to one of the surface plot types: {"CONFIDENCE_ELLIPSOID", "CONVEX_HULL", "SURFACE"}
    ...
    def show_points(self):
        ...
        if self.log_level >= api.LOG_HELP_LEVEL:
            print("Attribute 'show_points' has been set to 'None'; VIP will use the default setting for showing points"
                  " with surface plots ('CONFIDENCE_ELLIPSOIDS', 'CONVEX_HULL', 'SURFACE'")
