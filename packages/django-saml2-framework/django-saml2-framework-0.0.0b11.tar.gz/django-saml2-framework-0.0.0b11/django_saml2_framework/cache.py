from saml2.cache import Cache, CacheError, ToOld
import shelve
import six
...
class PersistentCache(Cache):
    def __init__(self, filename=None):
        ...
        self.prefix = 'sp_cache'

    def key(self, key):
        # todo: mixin because reused from ModelIdentDB
    ...
    def delete(self, name_id):
        ...
        """
        self.cache.delete(self.key(code(name_id)))
    ...
    def get_identity(self, name_id, entities=None, check_not_on_or_after=True):
        ...
        if not entities:
            try:
                cni = code(name_id)
                entities = self.cache.get(self.key(cni)).keys()
            except KeyError:
                return {}, []
    ...
    def get(self, name_id, entity_id, check_not_on_or_after=True):
        ...
        """
        cni = code(name_id)
        (timestamp, info) = self.cache.get(self.key(cni))[entity_id]
        info = info.copy()
    ...
    def set(self, name_id, entity_id, info, not_on_or_after=0):
        ...
        cni = code(name_id)
        cni_info = self.cache.get(self.key(cni)) or {}
        # if cni not in self._db:
        #     self._db[cni] = {}
        cni_info[entity_id] = (not_on_or_after, info)
        self.cache.set(self.key(cni), cni_info)
    ...
    def entities(self, name_id):
        ...
        """
        cni = code(name_id)
        return list(self.cache.get(self.key(cni)).keys())
