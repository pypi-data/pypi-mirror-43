...
def log_data_age(r):
    ...
    if 'Last-Modified' in r.headers:
        age = datetime.datetime.now() - datetime.datetime.strptime(r.headers['Last-Modified'],
                                                                   '%a, %d %b %Y %H:%M:%S %Z')
    ...
    logger.info('ESI data is {} seconds olf, new ESI data in {} seconds'.format(expires, age))
    ...
    if 200 <= r.status_code < 300:  # is 2xx
        ...
        if fetch_pages and total_pages > 1:
            ...
            for page_request in pages:
                page_data = page_request.get(blocking=True, timeout=60)  # block to get result
