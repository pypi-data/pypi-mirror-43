...
@datasets.command('pull-path',
                  help='Downloads a part of a dataset. Argument is account/dataset/path/to/download.')
@click.argument('path', type=str)
@click.option('-v', '--version', type=str, help='The version of the dataset. If none is provided, latest is used.')
@click.option(
    '-t', '--threads',
    type=int,
...
@click.pass_context
@login_required
def partial_download(ctx, path, version, threads, yes):
    account_uid = path[:path.find('/')]
    ...
    synchronizer = FileSynchronizer(download_path, dataset_path, s3_wrapper, master, dataset_path + path)
