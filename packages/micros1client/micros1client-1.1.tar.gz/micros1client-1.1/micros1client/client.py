...
#         print(r_dict)  # for displaying from the cli  print in cli parser
    ...
    def upload_xl(self, filepath):
        ...
        service_endpoint = self.url_to_connect + api_route
        headers={'X-Auth-Token': token}
#         files = {'file': ('micros1.xlsx', 
#                           open('/mnt/c/mydev/temp/micros1.xlsx', 'rb'), 
...
#         print(r_dict)  # for displaying from the cli  print in cli parser
    ...
    def list_invoices(self, field_name, field_value):
        token = self.tlClient.get_token().get('auth_token')
...
#         print(r)  # for displaying from the cli  print in cli parser
    ...
    def delete_invoices(self, field_name, field_value):
        token = self.tlClient.get_token().get('auth_token')
        api_route = '/invoice/delete/{}/{}'.format(
            field_name, field_value)
        service_endpoint = self.url_to_connect + api_route
        headers={'X-Auth-Token': token}
        try:  
