...
class MQTTSnipsApp(SnipsApp):
    ...
    def __init__(self, config=None, assistant=None):
        ...
        self.mqtt = Client()
        self.mqtt.on_connect = self.subscribe_topics
...
class HermesSnipsApp(SnipsApp):
    ...
    def register_callbacks(self):
        ...
        for name in dir(self):
            ...
            self.check_callback(callable_name, 'intent_not_recognized')
            self.check_callback(callable_name, 'intents')
            self.check_callback(callable_name, 'session_ended')
            self.check_callback(callable_name, 'session_queued')
            self.check_callback(callable_name, 'session_started')

    # TODO: Make private?
    def check_callback(self, method, event):
