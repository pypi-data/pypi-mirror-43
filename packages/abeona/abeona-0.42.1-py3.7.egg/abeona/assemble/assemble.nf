...
process secondaryAssemblyTransabyss {
    conda 'transabyss'
    publishDir 'secondary', mode: 'copy', overwrite: true

    input:
    set gid, file(graph), file('reads') from skipped_gid_ch
        .join(gid_assigned_reads_for_secondary_assembly_ch)
        .map{ tuple(it[0], it[2], it[3])}
        .view()

    output:
    set gid, file('tabyss/transabyss-final.fa') into gid_tabyss_transcripts_ch

    """
    transabyss --se reads* --pe\
        --outdir tabyss\
        --length $params.max_read_length\
