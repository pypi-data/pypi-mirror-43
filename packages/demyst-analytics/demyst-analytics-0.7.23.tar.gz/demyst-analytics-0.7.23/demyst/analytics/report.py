...
def attribute_cleanup(attributes, data):
    clean = (
        ...
        )[['connector', 'attribute', 'max_nested', 'match_rate', 'fill_rate', 'nunique', 'column_0']]
    )
...
def attribute_display(attributes):
    ...
    for c in kept_connectors:
        ...
        display(Markdown("*%0.0f%% match rate  --  $%0.3f per record  --  %d attributes*" % (
                    sub.match_rate[0]*100, sub.cost[0], len(sub))))
        sub = sub.drop(columns=['index', 'connector', 'cost', 'match_rate', 'column_0'])
