...
def generate(layers, weights=lambda: (random.random()-0.5)*2, biases=lambda: (random.random()-0.5)*2):
    ins = layers[0]['count']
    outs = layers[-1]['count']
    nodes = []
    edges = []
    ...
    for l in layers:
        for a in range(l['count']):
            if 'type' in l.keys():
                nodes.append({'bias':biases(), 'type':l['type']})
            else:
                nodes.append({'bias':biases(), 'type':Type.RELU})
