...
def get_config_path(user_file=None):
    if user_file:
        if path.exists(user_file):
            logger.info('Using config in {0}'.format(user_file))
            return user_file
        else:
            logger.error('Config file {0} not found.'.format(user_file))
    else:
        env = os.environ.get('FORGE_CONFIG')
        if env:
            if path.exists(env):
                logger.info('Using config in {0}'.format(env))
                return env
            else:
                logger.error('Config file {0} not found.'.format(env))
