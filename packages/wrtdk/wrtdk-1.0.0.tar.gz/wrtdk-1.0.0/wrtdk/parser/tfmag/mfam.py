...
def test_m3xg():
    m = mx3g()
    f = r'C:\Users\reynolds\Documents\1606_navsea\data\no_i2c.dat'
    f = r'C:\Users\reynolds\Documents\1704_madunit\data\20190307_mfam_madunit\mfam_log_3.dat'
    msg = get_msg(f,10)
    for mm in msg:
        print('len: %d msg:%s\nhex:%s' % (len(mm),mm,mm.hex()))
        m.parse(mm)
        print(m.getData())
...
'''
    ...
    with open(filename,'rb') as f:
        ...
        while len(payload) < n:
            ...
            payload.append(f.read(wrapper.getLength()))
            pos = pos + wrapper.getLength()
            pos = pos + 1
        return payload
        '''
