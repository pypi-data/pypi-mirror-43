...
class ServerSideBotProcessManager:
    ...
    async def startBot(self, aiName, fromLevel=False, nick='', team=None):
        ...
        if self.server is None:
            self.connectingToGame = Event(['result'])
            ...
            connectingToGame = self.connectingToGame
            self.connectingToGame = None
            connectingToGame.execute(None)
        elif self.connectingToGame:
            await self.connectingToGame.waitOrRaise()
...
def main():
    ...
    if os.name == 'nt':
        initLogging(logFile='log{}.txt'.format(logPrefix))
    else:
