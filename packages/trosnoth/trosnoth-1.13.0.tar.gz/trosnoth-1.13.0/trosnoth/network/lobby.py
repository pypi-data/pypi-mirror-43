...
class AuthGame(Game):
    def __init__(self, lobby, server, id, name):
        self.lobby = lobby
        self.server = server
        self.id = id
        self.name = name
    ...
    def datagramReceived(self, datagram, address):
        ...
        if datagram == b'%s:GetGames' % (MULTICAST_PROTOCOL_NAME,):
            for game in self.getGames():
                self.transport.write(b'%s:Game:%s' % (MULTICAST_PROTOCOL_NAME,
                        repr(game),), address)
...
class UDPMulticastGameGetter(DatagramProtocol):
    ...
    def getGames(self, timeout=1):
        ...
        d= defer.Deferred()
        try:
            self.transport.write(b'%s:GetGames' % MULTICAST_PROTOCOL_NAME,
