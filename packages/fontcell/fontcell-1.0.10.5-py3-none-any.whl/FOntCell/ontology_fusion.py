...
def circular_graphing(data, name, directory, result=True, color=None, parallelization=False, g1=None, equiv=None):
    ...
    if not parallelization:
        ...
        project_path = root_dir + 'fontcell_files'
        os.remove(root_dir + os.sep + 'graph.gv')
        os.remove(root_dir + os.sep + 'graph.gv.pdf')
        os.remove(root_dir + os.sep + 'graph.gv.plain')
        os.remove(project_path + os.sep + 'graph_viz_coord.ods')
    return


def generate_subgraphs(ontology, nodes, windows):
    subgraphs = np.zeros(len(nodes), dtype=object)
    if nodes.__len__() > 100:
        scaled_nodes = len(nodes)/100
    else:
        scaled_nodes = len(nodes)
    pbar = tq.tqdm(total=scaled_nodes)
    for j in range(len(nodes)):
        subgraphs[j] = treeComp.subGraphGenerator(ontology, nodes[j], windows)
        if float(scaled_nodes) % float(j+1) == 0:
            pbar.update(10)
    pbar.close()
    return subgraphs
