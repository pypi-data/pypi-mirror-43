...
def fontcell(path_to_config):
    ...
    ontology2_name, del_old_trials_files, onto1_fuseclasses, onto2_fuseclasses, onto1_restriction, \
    onto2_restriction, onto1_list_clear, onto2_list_clear, synonyms, text_process, split_from1, split_since1, \
    split_from2, split_since2, windowsize, globalthreshold, localthreshold, topological_test, \
    constriction_threshold, topological_cosine, topological_pearson, topological_euclidean, topological_threshold\
        = rc.read_conf(path_to_config)

    if path_to_config == proyect_path + os.sep + 'demo_configurations.txt':
        input_folder = proyect_path + os.sep
        output_folder = proyect_path + os.sep + 'results' + os.sep
        file_clean_ontology1 = proyect_path + os.sep + 'file_clean_CELDA.txt'
    ...
    if parallelization:
        ...
        doc.save_csv(ontology1, proyect_path + os.sep + 'onto1_for_parallel.csv')
        doc.save_csv(ontology2, proyect_path + os.sep + 'onto2_for_parallel.csv')
        list_of_variables = [onto_name1, onto_name2, file1, onto1_fuseclasses, onto2_fuseclasses, onto1_restriction,
                             ...
                             parallelization, topological_test, constriction_threshold, topological_cosine,
                             topological_pearson, topological_euclidean, topological_threshold]
        list_of_variables_str = list(map(lambda a: str(a)+'\n', list_of_variables))
        doc.save_txt(list_of_variables_str, proyect_path + os.sep + 'parallelization_config.txt')

        # launch a subprocess with mpirun and the parallelization (adding proc variable)
        cmd = "mpirun -np " + str(proc) + " python3 " + config_path
        os.system(cmd)
