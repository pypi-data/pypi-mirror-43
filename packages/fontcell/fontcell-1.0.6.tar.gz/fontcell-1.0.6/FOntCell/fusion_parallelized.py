...
def import_and_delete():
    ...
    r_path = proyect_path + os.sep
    list_of_variables = doc.import_txt(r_path + 'parallelization_config.txt')
    ontology1 = doc.import_csv(r_path + 'onto1_for_parallel.csv')
    ontology2 = doc.import_csv(r_path + 'onto2_for_parallel.csv')
...
def ontology_fusion_parallelized():
    ...
    if rank == 0:
        root_dir = os.path.dirname(os.path.realpath(__file__))
        proyect_path = root_dir + os.sep + 'fontcell_files'
        os.remove(proyect_path + os.sep + 'parallelization_config.txt')
        os.remove(proyect_path + os.sep + 'onto1_for_parallel.csv')
        os.remove(proyect_path + os.sep + 'onto2_for_parallel.csv')
        os.remove(proyect_path + os.sep + 'graph.gv')
        os.remove(proyect_path + os.sep + 'graph.gv.pdf')
        os.remove(proyect_path + os.sep + 'graph.gv.plain')
        os.remove(proyect_path + os.sep + 'graph_viz_coord.ods')
    return
