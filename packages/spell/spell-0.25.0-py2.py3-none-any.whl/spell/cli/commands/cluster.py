...
def create(ctx, profile):
    ...
    try:
        ...
        return
    click.echo("""This command will help you setup
    - an S3 bucket to store your run outputs in
    - a VPC which Spell will spin up workers in to run your jobs
    - Subnets in the VPC in multiple availability zones
    - a Security Group providing Spell SSH and Docker access to workers
    - an IAM role allowing Spell to spin up and down machines and access the S3 bucket""")
    ...
    # TODO commit role_arn, external_id vpc_id, subnets, security_group, and bucket_name to DB
...
def get_vpc(ec2):
    ...
    click.echo("Finished creating subnets for your new VPC. Your VPC is ready to go!")
    return vpc
...
# TODO add s3 read only buckets for mounting access. Probably both to the this subcommand and a new command
def get_role_arn(iam, bucket_name, external_id):
    from botocore.exceptions import ClientError

    response = click.prompt("Would you like to make a new IAM Role or use an existing one. We recommend making "
    ...
    bucket_objects_arn = "arn:aws:s3:::{}/*".format(bucket_name)
    policy = json.dumps({
        "Version": "2012-10-17",
        "Statement": [
            {
    ...
    try:
        iam_policy = iam.create_policy(
            PolicyName="SpellAccess",
            PolicyDocument=policy,
            Description="Grants Spell EC2 and S3 access")
    except ClientError as e:
        raise ExitException("Unable to create new IAM policy. AWS error: {}".format(e))
