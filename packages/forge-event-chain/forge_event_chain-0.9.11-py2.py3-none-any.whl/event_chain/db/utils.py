...
def mkdir_p(path):
    try:
        ...
        if exc.errno != errno.EEXIST or not os.path.isdir(path):
            raise FileNotFoundError("path doesnt exist!{}".format(path))
...
def create_connection():
    ...
    """
    if not path.exists(config.app_path):
        mkdir_p(config.app_path)
    try:
...
def insert_ticket(conn, ticket, event, owner, create_hash=None, exchange_hash=None):
    if exchange_hash:
        conn.execute(
                ''' INSERT INTO tickets
...
def select_tickets(conn, owner=None, event=None):
    ...
    if owner and event:
        c.execute(
                '''select * from tickets
                            WHERE owner = ? and event_address=?''', (owner,
                                                                     event),
        )
    elif owner:
        c.execute(
