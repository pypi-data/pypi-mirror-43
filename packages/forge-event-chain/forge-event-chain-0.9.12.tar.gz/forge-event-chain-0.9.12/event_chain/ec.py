...
def mobile_buy_ticket(event_address):
    if request.method == 'GET':
        ...
        url_base64_encoded=base64.urlsafe_b64encode(new_tx.SerializeToString())

        g.logger.debug("Sent tx base64 encoded: {}".format(base64_encoded))
        g.logger.debug("Sent tx base64 url encoded: {}".format(url_base64_encoded))
        ...
        if ticket_address:
            return json.loads(json.dumps({'ticket:': ticket_address}))
        else:
