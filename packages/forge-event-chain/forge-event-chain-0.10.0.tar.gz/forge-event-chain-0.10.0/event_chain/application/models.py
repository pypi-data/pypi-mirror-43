...
class EventInfo:
    ...
    def gen_consume_tx(self):
        ...
        if res.code != 0 or res.tx is None:
            ...
            logger.debug("Consume tx generated for event {}".format(self.title))
            return res.tx
...
class EventAssetState:
    ...
    def buy_ticket(self, wallet, token):
        ...
        if exchange_hash:
            logger.info("Exchange Hash: {0}".format(exchange_hash))
            logger.info(
    ...
    def buy_ticket_mobile(self, buyer_address, buyer_signature):
        ...
        logger.debug(
            "buy_ticket_mobiel is done with addresss{0}".format(address))
        return address
