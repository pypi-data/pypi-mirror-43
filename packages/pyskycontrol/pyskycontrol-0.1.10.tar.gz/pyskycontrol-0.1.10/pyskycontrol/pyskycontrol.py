...
class Sky:
        ...
        if self.hosts is None:
            ...
            try:
                while True:
                    ...
                    url = re.search("(?P<url>https?://[^\s]+)",
                                    data_text).group("url")
                    if "Sky" in data_text:
                        if addr[0] not in device_tmp:
                            device_tmp[addr[0]] = []
                            ip_address.append(addr[0])
                        device_tmp[addr[0]].append(url)
            ...
            for a in ip_address:
                ...
                for i in range(10):
                    ...
                    device_tmp[a].append(url.format(a, i))

        for url in device_tmp:
            for IP in ip_address:
                if IP in url:
                    device.append({IP: device_tmp[url]})
                ...
                for url in i[IP]:
                    try:
                        ...
                        request = requests.get(url=url, data=None,
                                               headers=headers)
                        response = BeautifulSoup(request.text, 'lxml-xml')
                        base_url = response.find('URLBase').string[:-1]
    ...
    def current_program(self, host, ctrl_url):
        ...
        if prog_type == 'Channel':
            ...
            p_list = response["schedule"]["events"]
    ...
    def sendcommand(self, host, port, command):
        ...
        reply = s.recv(12)  # cheive handshake
        s.send(reply)  # send handshake
        reply = s.recv(2)  # cheive 2 bytes
        s.send(reply)  # send 1 byte
        reply = s.recv(24)  # cheive 24 bytes
