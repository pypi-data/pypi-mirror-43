...
def relative_x_directions(contour, target_amount, img_size=(320, 240), fov=60):
    """
    Action: returns the relative x directions of the contours, based on angle and horizontal field of view
    :param contour: the contour (or a list of contours)
    :param target_amount: the wanted amount of contours
    :param img_size: the dimensions of the image (width, height)
    :param fov: the horizontal field of view of the camera
    :return: the directions 0 to 2000
    """
    width = img_size[0]
    ratio = 2000 / fov
    if not validate(contour, target_amount):
    ...
    for currentContour in contour:
        ...
        x_sum += x
    x_res = x_sum / len(contour)
    angle = Geometry.horizon_angle(x_res, 60, width)
    return ratio * angle


def relative_y_directions(contour, target_amount, img_size=(320, 240), fov=35):
    """
    Action: returns the relative x directions of the contours, based on angle and  vertical field of view
    :param contour: the contour (or a list of contours)
    :param target_amount: the wanted amount of contours
    :param img_size: the dimensions of the image (width, height)
    ...
    for currentContour in contour:
        ...
        y_sum += y
    y_res = y_sum / len(contour)
    angle = Geometry.horizon_angle(y_res, 60, width)
    return ratio * angle
