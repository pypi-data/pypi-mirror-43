...
from .Geometry import distance_between_points as dbp, distance_from_frame as dff
...
def image_center_filter(contour_list, image_dimensions=(320, 240), max_dist=0.7):
    """
    Action: filters out contours that their center is not close enough
    ...
    for current_contour in contour_list:
        contour_center = Geometry.get_contour_center(current_contour)
        distance_ratio = dbp(contour_center, image_center) / float(dff(contour_center, image_dimensions))
...
def distance_filter(contour_list, point, min_dist=0, max_dist=50):
    ...
    for current_contour in contour_list:
        contour_center = Geometry.get_contour_center(current_contour)
        distance_from_center = dbp(contour_center, point)
...
def abs_distance_filter(contour_list, max_dist=50, min_dist=0, image_dimensions=(320, 240)):
    """
    Action: filters out contours that their center is not close enough
            to the center of the image
    :param contour_list: a list of contours to be filtered
    ...
    for current_contour in contour_list:
        contour_center = Geometry.get_contour_center(current_contour)
        distance_from_center = dbp(contour_center, image_center)
        if max_dist >= distance_from_center >= min_dist:
            output.append(current_contour)
            ratio.append(distance_from_center)
    return output, ratio
