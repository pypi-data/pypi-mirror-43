...
def display_image(image, win_name='image', delay=None, resizable=False):
    """
    ...
    if type(image) is str:
        ...
        # If image is an array, this opened already
        if resizable:
            cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
        ...
        # if image is a list of images , open it and read each image and open if needed
        image[0] = cv2.imread(image[0]) if type(image[0]) is str else image[0]
        shape = image[0].shape
        width = shape[0]
        height = shape[1]
        blob = image[0]
...
class Vision(object):
    ...
    def __init__(self,
        ...
        :param kwargs: The optional key word arguments are:
                       parameters/params: for specific parameters for filter functions,
                       morph: morphological functions to be applied on the mask,
                       morph_params: parameters for morphological functions,
                       image_filters: functions to be applied on the images taken,
                       image_params: parameters for the image filters
                       use_camera: a bool that specifies if the high performance ovl.Camera should be used
        ...
        if "parameters" in kwargs:
            ...
            if len(self.params) < len(self.filters):
                if type(self.params) is tuple:
                    self.params = self.params + ((),) * (len(self.filters) - len(self.params))
                elif type(self.params) is list:
                    self.params = self.params + [[]] * (len(self.filters) - len(self.params))
        elif "params" in kwargs:
            self.params = kwargs['params']
        else:
            self.params = ((),) * len(filters)
        ...
        if connection_dst == Vision.NO_CONNECTION:
            ...
            if type(port) in (tuple, set, list):
                self.socket = Serial(port[0], baudrate=port[1])
    ...
    def apply_morphs(self, mask):
        ...
        """
        if type(self.morph) not in (tuple, list, set):
            return mask
    ...
    def send_to_destination(self, data):
        ...
        if self.connection_type == Vision.NT_CONNECTION:
            ...
            self.socket.sendto(bytes(data), (self.connection_address, self.network_port))
    ...
    def apply_all_filters(self, use_results=True, suppress=False):
        """
        Action: Applies all of the filters on the
        :param use_results: A Boolean that determines whether or not apply all filters.
        ...
        for idx, filter_func in enumerate(self.filters):
            if self.params is not None and self.params[idx] not in ((), []):
                _, ratio = self.apply_filter(filter_func,
                                             apply_results=use_results,
    ...
    def get_contours(self, img, color=None, save=True, return_hierarchy=False):
        """
        Action: Gets a list of all the contours within the Color or MultiColor range that was given
    ...
    def __filter_list(self):
        ...
        """
        return str([f.__name__ for f in self.filters]).replace("'", '')
    ...
    def __str__(self):
        ...
        """
        filters_for_print = (self.__return_name(i) for i in self.filters)
        ...
        filters = zip(filters_for_print, self.params)
        ...
        first_part = numerical_replace(string,
                                       ...
                                       self.width,
                                       self.height,
                                       self.directions.__name__ if self.directions is not None else None,
                                       self.target_amount)
    ...
    def camera_setup(self, port=0, img_width=None, img_height=None, camera=False):
        ...
        :param img_width: the width of the images to be taken, -1 does not set a width
        :param img_height: the height of the images to be taken, -1 does not set a height
    ...
    def frame_loop(self, apply_all=True, one_loop=False, print_results=True,
        ...
        while True:
            ...
            for frame_num in xrange(30):
                ...
                if return_val:
                    self.contours = self.get_contours(new_img)
                    self.apply_all_filters(use_results=apply_all)
    ...
    @staticmethod
    def translate_parameter_list(param_list):
        """
        ...
        """
        param_copy = copy.copy(param_list)
    ...
    def json_serialize(self, pass_camera=False, pass_network=False, pass_calibration=False):
        ...
        converted_filters = []
        for func in converted_filters:
            if type(func) is str:
                converted_filters.append(func)
            else:
                converted_filters.append(func.__name__)

        translation = {'color': color_object,
                       'filters': converted_filters,
                       'parameters': self.translate_parameter_list(self.params),
                       'directions_function': self.directions.__name__,
                       'target_amount': self.target_amount,
                       'width': self.width,
                       'height': self.height,
                       'image_filters': self.__image_filter_list(),
                       'image_params': self.translate_parameter_list(self.image_params),
                       'morph': self.__morph_list(),
                       'morph_params': self.translate_parameter_list(self.morph_params),
    ...
    def json_deserialize(json_string):
        ...
        if nw is True:
            ...
            roborio_name = d['roborio_name']
        nw = d['network_information']
        morphs = None if 'morphs' not in d else d['morphs']
        morph_params = None if 'morph_params' not in d else d['morph_params']
        image_filters = None if 'image_filters' not in d else d['image_filters']
        image_params = None if 'image_params' not in d else d['image_params']
        if nw:
            return Vision(color=color,
                          filters=filters,
                          directions_function=directions,
    ...
    def display_contours(self, img, amount=0, delay=None, color=None, save_path=None):
        """
    ...
    def grab_and_process(self, apply_image_filters=True, color=None, save_contours=True, suppress=False):
        """
        Action: takes an image, gets contours and applies all filters returns: contours, image, mask and
                any returned ratios from the filters
        :param apply_image_filters: if image filters should be applied.
        :param color: if a different color than the default should be used.
        :param save_contours: if the conoturs gotten should be saved.
        :param suppress: if before and after filters should be suppressed.
        :return: the contours found the image the mask and the ratios returned from the filter functions
        """
        if self.camera is None:
            raise MissingResourceError("Must have an image source in order to grab and process")
        image = self.get_applied_image() if apply_image_filters else self.get_image()
        mask = self.get_color_mask(image, color)
        contours = self.get_contours_mask(mask, save_contours)
        ratios = self.apply_all_filters(use_results=save_contours, suppress=suppress)
        return contours, image, mask, ratios
    ...
    def rasses_vanegev(self, amount, delay=3, name_convention=NC_TIME, return_images=False, path=False):
        """
        Easter Egg Function :3 (identical to Vision.Vision.photo_array)
        Action: Takes a series of images with a delay between shots and saves them.
