...
from email_master.util import EmailUtil, EmailParser


class EmailMasterParser(EmailParser):
    def _get_parser(self, filename, base64_data, file_parsers=None, **kwargs):
        if not file_parsers:
        ...
        for file_parser in file_parsers:
            try:
                return file_parser(filename, base64_data, **kwargs)
            except IOError as e:
    ...
    def __init__(self, filename, email_data, parser_classes=None, **kwargs):
        """
        ...
        self.parser = self._get_parser(filename, email_data, file_parsers=parser_classes, **kwargs)
