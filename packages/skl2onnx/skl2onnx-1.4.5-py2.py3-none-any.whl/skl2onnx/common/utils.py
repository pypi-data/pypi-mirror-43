...
def convert_to_python_value(var):
    if isinstance(var, numbers.Integral):
        ...
        raise TypeError('Unable to convert {0} to python '
...
def convert_to_python_default_value(var):
    if isinstance(var, numbers.Integral):
        ...
        raise TypeError('Unable to find default python value for '
                        'type {0}'.format(type(var)))
...
def convert_to_list(var):
    if isinstance(var, numbers.Real) or isinstance(var, str):
        ...
        flattened = []
        if all(isinstance(ele, np.ndarray) and
               len(ele.shape) == 1 for ele in var):
            ...
            for ele in var:
                for i in range(max_classes):
                    if i < ele.shape[0]:
                        ...
                        flattened_one.append(
                            convert_to_python_default_value(ele[0]))
            flattened += flattened_one
            return flattened
        elif all(isinstance(v, numbers.Real) or
                 isinstance(v, str) for v in var):
            return [convert_to_python_value(v) for v in var]
        else:
...
def check_input_and_output_numbers(operator, input_count_range=None,
    ...
    :param input_count_range: A list of two integers or an integer.
        ...
        minimal/maximal number of inputs. If it's an integer, it is
        equivalent to specify that number twice in a list. For infinite
        ranges like 5 to infinity, you need to use [5, None].
    ...
    if min_input_count is not None and len(operator.inputs) < min_input_count:
        raise RuntimeError(
            ...
            'required but we got %s input(s) which are %s' % (
                operator.full_name, operator.type, min_input_count,
                len(operator.inputs), operator.input_full_names))
    ...
    if max_input_count is not None and len(operator.inputs) > max_input_count:
        raise RuntimeError(
            ...
            'supported but we got %s output(s) which are %s' % (
                operator.full_name, operator.type, max_input_count,
                len(operator.inputs), operator.input_full_names))

    if (min_output_count is not None
            and len(operator.outputs) < min_output_count):
        raise RuntimeError(
            'For operator %s (type: %s), at least %s output(s) is(are) '
            'produced but we got %s output(s) which are %s' % (
                operator.full_name, operator.type, min_output_count,
                len(operator.outputs), operator.output_full_names))

    if (max_output_count is not None
            and len(operator.outputs) > max_output_count):
        raise RuntimeError(
            'For operator %s (type: %s), at most %s outputs(s) is(are) '
            'supported but we got %s output(s) which are %s' % (
                operator.full_name, operator.type, max_output_count,
                len(operator.outputs), operator.outputs_full_names))
...
def check_input_and_output_types(operator, good_input_types=None,
    ...
    :param good_input_types: A list of allowed input types (
        e.g., ``[FloatTensorType, Int64TensorType]``)
        or ``None``. *None* means that we skip the check of the input types.
    :param good_output_types: A list of allowed output types. See
        good_input_types for its format.
    """
    if good_input_types is not None:
        for variable in operator.inputs:
            if type(variable.type) not in good_input_types:
                raise RuntimeError(
                    "Operator '%s' (type: %s) got an input '%s' with a wrong "
                    "type %s. Only %s are allowed" % (
                        operator.full_name, operator.type, variable.full_name,
                        type(variable.type), good_input_types))

    if good_output_types is not None:
        for variable in operator.outputs:
            if type(variable.type) not in good_output_types:
                raise RuntimeError(
                    "Operator '%s' (type: %s) got an output '%s' with a wrong "
                    "type %s. Only %s are allowed" % (
                        operator.full_name, operator.type, variable.full_name,
                        type(variable.type), good_output_types))
