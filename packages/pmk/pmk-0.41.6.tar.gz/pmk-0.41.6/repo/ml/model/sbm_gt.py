...
class WSBM_gt(SbmBase):
    ...
    def compute_wsim(self, theta=None, phi=None, **kws):
        ...
        ws = np.array([ theta[i].dot(phi).dot(theta[j]) for i,j,w in self.data_test])
        ...
        #edge_filter = g.get_edge_filter()[0]
        self._nnz1 = 0
        self._nnz0 = 0
        self.wsim0 = 0
        self.wsim1 = 0
        w_max = self.data_test[:,2].max()
        print('wmax: %d, nnz_test: %d' % (w_max, nnz))
        count = 0
        from  time import time
        for i,j,w in self.data_test:
            begin = time()
            Ew = 0
            e = g.add_edge(i,j)

            #for w_i in range(1, w_max+10):
            for w_i in range(1, 50):
                g.ep.weights[e] = w_i
                #new_state = state.copy(g=g)
                new_state = state.copy(g=g, recs=[g.ep.weights], rec_types=['discrete-poisson'])
                log_pij = entropy - new_state.entropy()
                print(entropy-new_state.entropy())
                print('%s %s\t\t\r' % (w_i, np.exp(log_pij)), end='')
                Ew += w_i * np.exp(log_pij)

            if w == 0:
                self.wsim0 += np.abs(w-Ew)
                self._nnz0 += 1

            else:
                self.wsim1 += np.abs(w-Ew)
                self._nnz1 += 1

            g.remove_edge(e)
            print('roc percentage: %.2f\r' % (count/nnz), end='')
            print('50 pt done in %s' % (time() - begin))
            count += 1
...
class WSBM2_gt(SbmBase):
    ...
    spec_map = dict(B_min='K', B_max='K',
                    ...
                    state_args=lambda self:{'recs':[self.frontend.data.ep.weights], 'rec_types': ["discrete-poisson"]})
