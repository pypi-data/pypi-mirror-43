...
class Credentials:
    ...
    def load_credentials(self):
        """Loads credentials from disk, creating the credentials file if it
        does not already exist, and populates self.username, self.uuid, and
        self.phrase"""
        ...
        # credentials[0] is username
        # credentials[1] is uuid
        # credentials[2] is phrase
        if credentials[1] == '':
            self.gen_random_uuid()
            self.save_credentials()
        else:
            if self.validate_uuid4(credentials[1]):
                self.set_username(credentials[0])
                self.set_uuid(credentials[1])
                self.set_phrase(credentials[2])
            else:
                self.init_credentials_file()
                self.load_credentials()
    ...
    def register(self, force_registration = False):
        ...
        create an API key. Will return immediately if there's already a secret
        phrase populatedin self.phrase (indicating a complete API key), unless
        force_registration is set to True. Saves the key to disk upon
