...
ci_data_path = path_util.make_and_return_path_from_path_and_folder_names(
    path=cosma_path, folder_names=['data', ci_data_type, ci_data_model, ci_data_resolution[batch_id]])
...
def mask_function_for_bug(shape, frame_geometry):
    return msk.Mask.from_cosmic_ray_image(shape=shape, frame_geometry=frame_geometry, regions=[(0, 330, 0, shape[1])])

from workspace_jam.pipelines import parallel_x3_species

pipeline = parallel_x3_species.make_pipeline(phase_folders=[ci_data_type, ci_data_model,
                                                            ci_data_resolution[batch_id]],
