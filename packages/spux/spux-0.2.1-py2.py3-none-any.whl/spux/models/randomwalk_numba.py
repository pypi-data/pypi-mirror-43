...
# TODO: rvs array get invalidated for some reason..
@numba.njit
def walk (t_start, t_end, position, trend, stepsize, rvs):
...
class Randomwalk (Model):
    ...
    def run (self, time):
        ...
        # TODO: numba does not work properly for some reason, see above
        # self.position = walk (self.time, time, self.position, self.trend, self.stepsize, rvs)
        
        # update position (e.g, perform walk)
        directions = numpy.where (rvs < self.trend, 1, -1)
