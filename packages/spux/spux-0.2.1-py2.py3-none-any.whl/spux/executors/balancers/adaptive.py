...
class Adaptive (Balancer):
    ...
    # distribute initial particles to particle ensembles for each worker - ensembles = groups to workers, I believe
    def ensembles(self, indices, workers):
        ...
        count = int(math.ceil(float(len(indices)) / workers))
        ensembles = [
            indices[i : i + count]
            for i in range(0, max(len(indices), workers * count), count)
        ]

        return ensembles

    # compute routings from current particle ensembles and specified indices
    # TODO: also allow to pass 'runtimes' parameter with predicted runtime information?
