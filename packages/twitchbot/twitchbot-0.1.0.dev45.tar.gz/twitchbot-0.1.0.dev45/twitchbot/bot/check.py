...
class UserCoolDown(Check):
    ...
    async def check(self, cmd: 'Command', ctx: Context):
        if ctx.author.login not in cmd._user_executions:
            return True

        if datetime.datetime.now() < (cmd._user_executions[ctx.author.login] + self._cool_down):
            self.description = f'command is on cool down ({round(((cmd._user_executions[ctx.author.login] + self._cool_down) - datetime.datetime.now()).total_seconds(), 2)} seconds) for you'
            return False
        return True
