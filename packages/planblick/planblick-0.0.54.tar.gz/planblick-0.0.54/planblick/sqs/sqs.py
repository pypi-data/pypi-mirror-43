...
class Sqs:
        ...
        while (1):
            for message in self.queue.receive_messages(MaxNumberOfMessages=MaxNumberOfMessages):
                if getMessageObject == False:
                    key = self.getKeyFromString()
                    cipher_suite = Fernet(key)
                    message_parameter = cipher_suite.decrypt(message.body.encode("utf-8")).decode("utf-8")
                else:
    ...
    def getKeyFromString(self, string):
        key = json.loads(os.getenv("CLUSTER_CONFIG")).get("cipher-key")
        print("SQS CIPHER KEY!!!!", key)
        if key is None:
