...
class OpenstackClient(OpenstackBase):
    ...
    def tenant_network_is_exist(self, network_name):
        ...
        for network in network_list:
            network_name_list.append(network[u'name'])
    ...
    def create_user(self, tenant_id, user_name, user_pwd):
        ...
        if user is not None:
            ...
            for role in roles_list:
                ...
                elif role.name == u'heat_stack_owner':
                    self.keystone_client.roles.add_user_role(user=user, role=role, tenant=tenant)
        return user
    ...
    def update_neutron_quota(self, tenant_id, networks, subnets, ports, routers):
        ...
        neutron_quota_value = neutron_quota[u'quota']
        neutron_quota_value[u'subnet'] = subnets
        neutron_quota_value[u'network'] = networks
        neutron_quota_value[u'router'] = routers
        neutron_quota_value[u'port'] = ports
        neutron_quota[u'quota'] = neutron_quota_value
    ...
    def create_flavor(self, name, id, ram, root_disk, vcpu, flavor_keys, cpu_policy):
        ...
        for keypair in flavor_keys_list:
            ...
            uni_key = unicode(keypair_dict[0])
            uni_value = unicode(keypair_dict[1])
            metadata.setdefault(uni_key, uni_value)
        metadata.setdefault(u'hw:cpu_policy', unicode(cpu_policy))
        try:
