...
from automl.client.core.common.onnx_convert.operator_converters import (       # noqa: E402
    PrefittedVottingClassifierConverter)
from automl.client.core.common.onnx_convert.operator_converters import (       # noqa: E402
    PrefittedVottingRegressorConverter)
...
class OnnxConverter:
    ...
    def __init__(self, logger=None, version='', is_onnx_compatible=False, *args, **kwargs):
        ...
        self.col_name_to_idx_in_input = {}      # type: Dict[Union[str, int], Any]

        # The column name to index map, in the generated initial types.
        self.col_name_to_idx_in_initial_tp = {}     # type: Dict[Union[str, int], Any]
    ...
    def initialize_input(self, X, x_raw_column_names=None, model_name='', model_desc=None):
        ...
        try:
            ...
            # e.g.  [('input1', FloatTensorType(shape=[1, 1])), ('input2',
            # Int64TensorType(shape=[1, 1]))].
            #
            header_exists = False
            for c in X.columns:
                if isinstance(c, str) and not header_exists:
                    header_exists = True
                self.col_name_to_idx_in_input[c] = X.columns.get_loc(c)

            # Unbatched initial type, for each column we init a tuple (column_name, onnx_type).
            for i_col, column in enumerate(X.columns):
                ele = X[column].iat[0]
                cur_type = type(ele)
                onnx_type = self._convert_np_to_onnx_type(cur_type)
                if header_exists:
                    assert(isinstance(column, str))
                    col_name = column
                else:
                    col_name = 'input_' + str(i_col)
    ...
    def _setup_initial_types_based_on_model(self, model):
        ...
        if trans is not None:
            ...
            if trans.mapper.built_default is None or trans.mapper.built_default is False:
                df_mapper_selected_column_indicies = {}
                for columns, transformers, options in trans.mapper.built_features:
                    col_idx = columns
                    col_name = columns
                    ...
                    if isinstance(col_idx, str):
                        ...
                        col_name = col_idx
                        col_idx = self.col_name_to_idx_in_input[col_idx]

                    df_mapper_selected_column_indicies[col_idx] = col_name

                new_idx = 0
                for idx, input_var in enumerate(self.individual_initial_types):
                    if idx in df_mapper_selected_column_indicies:
                        self.initial_types.append(input_var)
                        self.col_name_to_idx_in_initial_tp[df_mapper_selected_column_indicies[idx]] = new_idx
                        new_idx = new_idx + 1
            else:
                self.initial_types = self.individual_initial_types
                self.col_name_to_idx_in_initial_tp = self.col_name_to_idx_in_input
        else:
    ...
    def _convert_np_to_onnx_type(self, tp):
        if issubclass(tp, (float, np.floating)):
            return FloatTensorType
        elif np.issubdtype(tp, int) or np.issubdtype(tp, np.integer):
            return Int64TensorType
        elif np.issubdtype(tp, str):
            return StringTensorType
        else:
            raise OnnxConvertException("Unsupported data type [{}] for converting to ONNX.".format(tp))
    ...
    def _parse_data_transformer(self, scope, dt, inputs):
        ...
        for columns, transformers, options in dt.mapper.built_features:
            ...
            col_idx = self.col_name_to_idx_in_initial_tp[col_idx]

            # Add the col index (int) to selected set.
            df_mapper_selected_column_indicies.add(col_idx)
            input_var = inputs[col_idx]
            ...
            for j, trans_step in enumerate(transformers.steps):
                ...
                var_out_name = 'variable_c' + str(col_idx) + '_t' + str(j)
    ...
    def _parse_ensemble_estimator(self, scope, ensemble_estimator, inputs):
        ...
        if is_classification:
            virtual_ensemble_op = scope.declare_local_operator(PrefittedVottingClassifierConverter.OPERATOR_ALIAS)
            ...
            virtual_ensemble_op = scope.declare_local_operator(PrefittedVottingRegressorConverter.OPERATOR_ALIAS)
    ...
    def _check_and_get_ensemble_estimator(self, mdl_obj):
        ...
        if SOURCE_WRAPPER_MODULE in mdl_obj.__class__.__module__:
            ...
            if est_cls.__name__ == mdl_obj.__class__.__name__:
                tmp_obj = copy.deepcopy(mdl_obj)
                cl_obj = est_cls(
                    estimators=tmp_obj.estimators,
                    weights=tmp_obj.weights,
                    flatten_transform=tmp_obj.flatten_transform
                )
                # Explicitly set these attributes.
                cl_obj.estimators_ = tmp_obj.estimators_
                cl_obj.le_ = tmp_obj.le_
                cl_obj.classes_ = tmp_obj.classes_
                return cl_obj
            est_cls = PreFittedSoftVotingRegressor
            if est_cls.__name__ == mdl_obj.__class__.__name__:
                tmp_obj = copy.deepcopy(mdl_obj)
                cl_obj = est_cls(
                    estimators=tmp_obj._wrappedEnsemble.estimators,
                    weights=tmp_obj._wrappedEnsemble.weights,
                    flatten_transform=tmp_obj._wrappedEnsemble.flatten_transform
                )
                return cl_obj
