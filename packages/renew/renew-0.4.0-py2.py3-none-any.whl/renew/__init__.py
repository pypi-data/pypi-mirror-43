...
class Mold(six.with_metaclass(_mold.MoldMeta, object)):
    ...
    def __repr__(self):
        try:
            return "".join(self.dispatch_repr(True, None))
        except Exception:
            return object.__repr__(self)

    def dispatch_repr(self, top_level, dep_list):
        ...
        if hasattr(self, "_cls_constructor_spec"):
            ...
            items_reprs = [(kw, list(_py_lang.dispatch_repr(value, top_level, dep_list))) for kw, value in params]
...
def reproduction(single_object):
    ...
    """
    return "".join(_py_lang.dispatch_repr(single_object, True, None))
