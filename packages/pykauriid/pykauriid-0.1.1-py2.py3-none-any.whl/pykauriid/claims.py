...
def validate_commitment(commitment: str) -> Jws:
    """
    ...
    # First, make the JWK verification key object.
    curve = verifier.header['alg']
    signing_key_data = {'kty': 'OKP', 'crv': curve,
                        'x': verifier.payload['iss']}
    signing_key = Jwk.get_instance(crv=curve, from_dict=signing_key_data)
    verifier.jwk = signing_key
    verifier.verify()
    return verifier
...
class ClaimSet:
    ...
    def __init__(self,
        ...
        :param signing_key: Key for JWS signature.
        ...
        if signing_key:
            self.sub = utils.bytes_to_string(signing_key.x)
        self.claims = []
    ...
    @signing_key.setter
    def signing_key(self, value: Jwk):
        """
        Set the signing key.

        :param: A JWK signing key.
        """
        self._signing_key = value
        if value:
            self.sub = utils.bytes_to_string(value.x)
        else:
            self.sub = None
    ...
    def get_commitment(self, *,
                       committer_key: Jwk = None) -> Union[str, None]:
        """
        Compute and return a commitment for the given claims.

        :param committer_key: Signing key for the committer of the attestation.
            If none is given, the the object's `signing_key` member will be
            used (default: None). This is to be used for an attester
            commitment, in which case the attester's signing key is provided.
            In that case the commitment label to be signed is adapted to
            reflect this.
        :return: Commitment as compact JWS signature serialisation, `None` if
            no (private) signing key is available.
        """
        role = 'attester' if committer_key else 'subject'
        committer_key = committer_key or self.signing_key
        if committer_key and committer_key.d:
            signer = Jws.get_instance(jwk=committer_key)
            commitment = '.'.join(self._get_commitment_elements())
            signer.payload = {
                'commitment': commitment,
                'sub': self.sub,
                'iss': utils.bytes_to_string(committer_key.x),
                'role': role}
            signer.sign()
    ...
    def validate_commitment(self) -> dict:
        ...
        :raises: nacl.exceptions.BadSignatureError on failed validation.
        """
        verifier = validate_commitment(self.commitment)
        if verifier.payload['sub'] != self.sub:
            raise RuntimeError('Subject in commitment mismatches the'
                               ' claim set subject.')
        pub_signing_key = utils.bytes_to_string(self.signing_key.x)
        if (verifier.payload['role'] == 'subject'
                and verifier.payload['iss'] != pub_signing_key):
    ...
    def load(self, binary_data: Union[bytes, str], object_key: Jwk):
        ...
        self._signing_key = Jwk.get_instance(crv='Ed25519')
        self._signing_key.x = utils.string_to_bytes(self.sub)
...
class ClaimSetKeys:
    ...
    def load(self, data: Union[bytes, str, dict]):
        """
