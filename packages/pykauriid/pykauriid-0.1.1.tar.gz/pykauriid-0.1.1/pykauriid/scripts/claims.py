...
def new_claim_set(claims_fd: io.TextIOBase,
    ...
    signing_key = Jwk.get_instance(crv='Ed25519', from_json=sig_key_fd.read())
    if not signing_key.d:
        raise ValueError('Signing key does not contain a private key.')
    claim_set = claims.ClaimSet(signing_key)
...
def main(args: argparse.Namespace):
    ...
    if args.operation == 'new':
        ...
        access_claim_set(args.claimsetkeys, args.index, args.list)
    else:
