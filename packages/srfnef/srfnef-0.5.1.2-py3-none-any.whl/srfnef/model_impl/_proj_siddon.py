...
from srfnef.data_types import Image, Listmode, LORs
...
@funcclass
class ProjectorSiddon(Projector):
    device: str = 'gpu'
    ...
    def __call__(self, image: Image, lors: LORs):
        if self.device == 'cpu':
            ...
            proj_siddon_cuda[griddim, blockdim](image.data, lors.data,
                                                image.size / image.shape, image.center,
                                                vproj)
            cuda.synchronize()
        else:
