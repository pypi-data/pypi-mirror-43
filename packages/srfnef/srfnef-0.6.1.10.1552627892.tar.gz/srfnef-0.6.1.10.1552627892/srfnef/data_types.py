...
from .templating import dataclass
...
class Image:
    ...
    size: typing.List[float] = attr.ib(converter = lambda x: np.array(x).astype(np.float32))
    name: str = attr.ib(default = 'image')
...
class EmapMlem(Image):
    name: str = attr.ib(default = 'emap_mlem')

    # TODO correct emap generations in ring mode.
    @classmethod
    def from_scanner(cls, scanner: PetCylindricalScanner, back_projector, mode = 'thin-ring'):
        emap = cls(np.zeros(back_projector.shape, np.float32), back_projector.center,
                   back_projector.size)

        # if not np.abs(scanner.blocks.unit_size[2] - back_projector.unit_size[2]) < _tiny:
        if mode == 'full':
            _emap = back_projector(Listmode.from_lors(Lors.from_scanner(scanner)))
            ...
            for i in _tqdm(np.arange(scanner.nb_rings)):
                for j in _tqdm(np.arange(scanner.nb_rings)):
                    ...
                    _lors.data[:, 5] += j * scanner.blocks.size[2] + j * scanner.gap - \
                                        scanner.axial_length / 2
                    _emap = back_projector(Listmode.from_lors(_lors))
                    if i == j:
                        emap = emap + _emap
                    else:
                        emap = emap + _emap * 2
    ...
    @classmethod
    def from_scanner_full(cls, scanner: PetCylindricalScanner, back_projector):
        _emap = back_projector(Listmode.from_lors(Lors.from_scanner(scanner)))
        return cls(_emap.data, _emap.center, _emap.size)
