...
class BackProjectorDistanceDriven(BackProjector):
    shape: list = attr.ib(converter = converter = lambda x: np.array(x).astype(np.int32))
    center: list = attr.ib(converter = converter = lambda x: np.array(x).astype(np.float32))
    size: list = attr.ib(converter = converter = lambda x: np.array(x).astype(np.float32))
