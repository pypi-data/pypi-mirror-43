...
from .data_types import Image, Listmode, EmapMlem, Lors
from .templating import funcclass, Saver
...
class Mlem:
    ...
    is_tqdm: bool = attr.ib(default = False)
    ...
    def __call__(self, listmode: Listmode, *, x = None, labels = []):
        if x is None:
            x = Image(np.ones(self.emap_mlem.shape, dtype = np.float32), self.emap_mlem.center,
                      self.emap_mlem.size)
        range_ = range(self.n_iter) if not self.is_tqdm else _tqdm(range(self.n_iter))
        for ind in range_:
