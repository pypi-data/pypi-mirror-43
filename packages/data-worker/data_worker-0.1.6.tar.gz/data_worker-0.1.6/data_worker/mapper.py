from data_worker.defined_wrappers import wrappers, api_meta_data


def get_wrapper(target_wrapper):
    sources_string = '\n'
    for wrapper_name in wrappers:
        wrapper_alt_names = wrappers[wrapper_name]['alt_names']
        if target_wrapper in wrapper_alt_names:
            return wrappers[wrapper_name]['class'](wrapper_name, wrapper_alt_names, api_meta_data[wrapper_name])
        else:
            sources_string += ''.join(
                '  {}: {}\n'.format(wrapper_name, ', '.join(
                    ['\'{}\''.format(x) for x in wrapper_alt_names]))
            )

    raise Exception(
        '\'{}\' is not a valid source. Must be one of:\n{}'.format(
            target_wrapper, sources_string)
    )
...
def get_wrapper_method(wrapper, target_method):
    ...
    methods_string = '\n  '.join(
        ['{}()'.format(x.__name__) for x in wrapper_methods])
    methods_string = '\n  ' + methods_string + '\n'

    raise Exception(
        '\'{}\' is not a valid {} method. Must be one of:\n{}'.format(
            target_method, wrapper.name, methods_string)
    )
