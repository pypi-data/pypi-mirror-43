from data_worker.defined_wrappers import wrappers, meta_data
...
def get_wrapper(target_wrapper):
    ...
    for wrapper_name in wrappers:
        ...
        if target_wrapper in wrapper_alt_names:
            return wrappers[wrapper_name]['class'](wrapper_name, wrapper_alt_names, meta_data[wrapper_name])
        else:
            sources_string += ''.join(
                '  {}: {}\n'.format(wrapper_name, ', '.join(['\'{}\''.format(x) for x in wrapper_alt_names]))
            )
